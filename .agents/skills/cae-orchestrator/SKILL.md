...
| `ausreizen` | **Die Auslegung ins Gleichgewicht bringen — der Optimierer über `leistung`.** Sucht den Entwurf, der ALLE Grenzen ausnutzt, und sagt danach, **was noch brachliegt und welcher Griff es heben würde** — gemessen, nicht aus einer Faustregel. **Wichtig für das Verständnis:** Gleichgewicht ist kein Ziel, sondern ein Symptom. `--ziel leistung` maximiert nur P_max und lässt jede nicht bindende Grenze liegen, wo sie ist (das Joch einer gemessenen Maschine steht bei 25 % — es dünner zu machen bringt kein Watt). `--ziel dichte` (Vorgabe) maximiert P_max/Masse; dann ist eine brachliegende Grenze *entfernbares Material*, und das Gleichgewicht stellt sich von selbst ein. Gesucht wird **deterministisch** (Musterschritt, kein Sprachmodell): derselbe Payload ergibt denselben Entwurf, und das Protokoll nennt jeden Griff mit seinem Gewinn. `--frei p,slotDepth,…` grenzt die beweglichen Parameter ein (Vorgabe: alle 14 aus `FREE_PARAMS`), `--budget` deckelt die Auswertungen (gemessen 0,28–0,57 s je Stück). Das Layouttor und alle vier Grenzen aus `leistung` laufen mit — ein unbaubarer Kandidat fällt durch, statt zu gewinnen. Der Sieger ist ein **Vorschlag**, kein Ergebnis: nachgerechnet wird er mit `run analyse`. Exit 0 = besserer Entwurf gefunden, 1 = keiner |
| `umrichter` | **Mehrere Leistungselektroniken: A1..Ak, B1..Bk, C1..Ck statt A, B, C.** Das Erste, was dazu zu sagen ist: **Aufteilen macht die Maschine nicht stärker** — das Moment hängt an den Amperewindungen, und die liegen durch Nut, Stromdichte und Kühlung fest. Die gesamte Scheinleistung bleibt gleich und verteilt sich auf k Module zu je 1/k. Was man kauft: **kleinere Halbleiter**, **n-1-Betrieb** statt Totalausfall, verteilte Verlustwärme und — bei Winkelversatz — Auslöschung der 5. und 7. Harmonischen (die wird hier **nicht** gerechnet, der FDM ist magnetostatisch). `inverterVdc`/`inverterImax` sind die Werte **eines Moduls**; jedes System bekommt 1/k der Leiter, also N/k Windungen, woraus zwingend folgt: Spannung je Modul E/k, Strom unverändert. **Von der Maschine aus wirken k Module wie EIN Umrichter mit k·V_Modul bei gleichem Strom** — mehr Module kaufen Spannungsreserve (Eckdrehzahl), nicht Strom. Das steht in `ema_analysis.umrichter`, der einen Quelle der Grenzen, also erben es Kennfeld und die neun Module, die durch sie lesen; k=1 lässt jede Altrechnung Ziffer für Ziffer gleich. **Ein Wickeltor** sagt, ob k überhaupt darstellbar ist (`slots % 3k`; sektoriert zusätzlich ganze Polzahl je Sektor; für den Versatz `q ≥ k`) — und **was statt dessen ginge**. `verschachtelt` und `sektoriert` ergeben dieselben Modulkennwerte; sie unterscheiden sich im **Fehlerfall**: verschachtelt symmetrisch, sektoriert mit **einseitigem Magnetzug auf die Lager**, der hier nicht gerechnet ist und als Warnung dabeisteht. `--bilder` legt den Querschnitt mit den Systemen und die n-1-Kennlinie nach `<projekt>/charts/`. Im Browser: Karte **⚡ Leistungselektronik** im Betrieb-Reiter (sie rechnet nicht selbst, sie ruft `POST /umrichter` — die Wickelregel bleibt in Python). Exit 0 = teilbar, 1 = nicht teilbar |
...
### Rastmoment — wenn nach Drehgenauigkeit gefragt wird

Es läuft in **jedem** Paarvergleich als Spalte `Rast [%]` mit und steht in `sicherheit`. Drei Hebel, in dieser Reihenfolge:

| Hebel | Wirkung | Wie |
|---|---|---|
| **Nut-/Polzahl** | kgV hoch = rastet fein. 30N/10P (kgV 30) ist unbrauchbar, 24N/10P (kgV 120) brauchbar, 27N/10P (kgV 270) sehr gut | `--set slots=…` |
| **Schrägung** | eine ganze Nutteilung löscht die Grundwelle **exakt** aus, eine halbe lässt noch 13 % stehen | `--set skew_deg=…`, Achse `schraegung` |
| **Nutschlitz** | der stärkste Hebel — **dieses Werkzeug kann ihn nicht**: es zeichnet offene Nuten, `nut_breite` am Bohrungsrand *ist* die Öffnung | — |

**Sag immer dazu, dass es geschätzt ist.** Analytisch nach Zhu/Howe, Amplitude auf etwa Faktor 2. Gemessen wird es nicht: den Läufer im FDM zu drehen gibt bei einer vollen Rastperiode ±8 Nm Unterschied ohne Konvergenz — Rasterrauschen der gedrehten Geometrie. Der *Vergleich* zweier Auslegungen trägt, die absolute Zusage nicht.
...
Die 175 Routen sind über `raw` erreichbar; `routes --grep` findet sie.
