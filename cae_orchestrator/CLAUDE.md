...
`test_aster.py` (Code Aster als dritter Löser: gleiche Knoten-/Elementnummern wie
ccx und Z88, wortgleicher Lastfall, Dichte in t/mm³, und die Verschiebung NICHT aus
`getValues()` — dort stehen die Lagrange-Multiplikatoren mit drin; `--echt` rechnet
zusätzlich wirklich gegen CalculiX),
...
`test_auftrag.py` (``AUFTRAG.md``: die Vorlage entsteht bei der Projektanlage — an EINER Stelle —, eine vorhandene Datei wird NICHT ueberschrieben, `ergaenzen` haengt an und laesst Bestehendes Zeichen fuer Zeichen stehen, Schluessel und Ueberschrift treffen denselben Abschnitt, ein unbekannter wird angelegt statt der Eintrag verworfen, und der Auftrag erreicht Steckbrief und Agentenkopf — geschrieben und unerreichbar waere dasselbe wie nicht geschrieben), `test_chat_werkzeuge.py` (Chat und Aehnlichkeit, ohne Ollama und ohne Netz — das Sprachmodell wird gestellt: der Verlauf liegt anhaengend im Projekt und eine Kennung aus der URL fuehrt nicht heraus, die Werkzeugschleife fuehrt einen Aufruf aus und reicht das Ergebnis zurueck, laesst aber einen bloss ERWAEHNTEN Aufruf in Ruhe, ein gescheitertes Werkzeug wird gemeldet statt verschwiegen, die Schleife endet nach `max_runden` und SAGT es statt Protokoll zu zeigen, die Deckungspruefung findet die erfundene Zahl und meldet keine Projektkennung, und die Aehnlichkeit ist gegen eine 2,5-fach skalierte Maschine massstabsunabhaengig, benennt die Dublette und behauptet ohne Payload nichts), `test_ablage.py` (Papierkorb: ohne `bestaetigt` passiert NICHTS und der Aufruf ist die Rueckfrage, Entsorgtes liegt im Korb und kommt zurueck, Zurueckholen ueberschreibt nichts sondern landet daneben, ein Pfad ausserhalb wird am AUFGELOESTEN Pfad abgewiesen — direkt, ueber `..`, ueber einen Symlink und die Wurzel selbst —, jede Entsorgung steht in der Zeitleiste, endgueltig braucht ein zweites Ja, und die Modulfunktionen, die frueher `rmtree` riefen, gehen wirklich hindurch), `test_bericht.py` (Berichtserweiterung: festgehaltene Ansichten samt ihren
Einstellungen, der Elmer-Loeserstand aus der `case.sif` GELESEN, und der selbst
geschriebene Bericht — Pfade ausserhalb des Projekts abgewiesen, HTML mit
laufendem Video, PDF mit ehrlichem Standbild),
`test_paramstudy.py` (Parameterstudie: ein Ordner je Studie statt eines
gemeinsamen, EINE CSV-Quelle, wirkungslose Parameter werden BENANNT — ohne
Server, ohne Loeser),
`test_fluidx3d.py` (FluidX3D/LBM: `defines.hpp` setzen, Marker aus FluidX3Ds
`\r`-Fortschrittszeile holen, Speicherschaetzung gegen die gemessenen 572 MB,
Fensterlage im Wickelkopf-UEBERHANG statt in den geraden Staeben, Oelsaeule
ausserhalb der Muendung, Lizenzkennzeichnung des erzeugten Falls — ohne GPU,
...
ähnlich bei stehendem Luftspalt), `test_zyklen.py` (Fahrzyklus-Wahl und Sicherheitskriterien: der frische Payload waehlt den Zyklus NICHT selbst, eigene Zyklen ueberleben in der DB, `anwenden` setzt Zyklus UND Fahrzeug, die Zyklustemperaturen zaehlen mit, die Magnetgrenze kommt aus der Werkstofftabelle, Bericht und Werkzeug faellen EIN Urteil), `test_maschinenart.py` (Maschinenart als Tor und die ASM analytisch: eine nicht getragene Art wird ABGEWIESEN statt ersetzt — in Pipeline, Vorauswahl und Paarvergleich; die Normierungsbruecke `ema_asm.k_norm` wird ueber die **Momenterhaltung** nachgerechnet, der Magnetisierungsstrom gegen `_analytical_Barm` zurueckgeprueft; Kaefigauswahlregel, Tiefe/Breite-Deckel, Schlupf im Nennbereich, quadratische Stegspannung; und die 14. Achse liefert alle gezaehlten Kennzahlen, laesst `T_rel_pct` weg und `magnet_kg` stehen), `test_feldbild.py` (Feldlinienbilder: Alphakanal im fertigen PNG, Deckkraft steigt streng mit |B|, EIN Löserlauf für alle Querschnitte, der Schnitt nimmt Material WEG statt es zu übermalen, der Längsschnitt ohne 3-D-Ergebnis sagt es im Bild, und die Kachel im Agentenreiter bekommt das Schachbrett), `test_steckbrief.py` (Steckbrief, Ablage und der Rueckweg in fruehere Laeufe: ein Verbergebnis ueberlebt das Fenster und traegt seinen Aufruf mit, zwei Rechnungen in derselben Sekunde ueberschreiben sich nicht, der Steckbrief rechnet NICHTS und fuehrt die Herkunft aus `ema_db.HERKUNFT` mit, die Laufuebersicht liefert dasselbe wie das volle Einlesen ohne es zu tun, `lauf_lesen` deckelt und weist Kennungen aus der URL ab, eine abgelegte Rechnung erscheint in der rechten Spalte OHNE Zutun des Agentenkopfs, und der gemessene ACP-Fehler wird benannt statt ueberspielt), `test_studio.py` (der dritte Kopf und die Beitraege: drei Koepfe in der Registrierung, der Auslegungsauftrag liegt bei PI und NICHT beim Studio-Kopf, die Projektakte des laufenden Kopfes wird nicht ueberschrieben, die Tokenwache schuetzt genau `/studio` und `kopf=studio` und laesst `pi`/`hermes` offen, Schnittvorschlaege und `schnitt.sh` zeigen dieselben Sekunden, gerundete Zahlen sind gedeckt und erfundene werden gemeldet, X bricht um statt abzuschneiden, und beide Agentenseiten benutzen `agent_gemein.js` statt einer Abschrift — samt `node --check` ueber geteilte Datei plus Seite zusammen; die Fussleiste traegt keinen Ablagepfad mehr und zeigt acht Lampen), `test_werkzeugstand.py` (der Werkzeug-Fingerabdruck: stabil ueber zwei Abrufe, ein Byte in einer `PHYSIK`-Datei bewegt ihn und eine Datei ausserhalb nicht, `abweichung` NENNT die Datei, der Kopf meldet eine Aenderung waehrend des Laufs genau einmal in den Strom, und die Regel steht in `AGENTS.md`, `SKILL.md` und im stehenden Auftrag aller drei Koepfe), `test_asm_grenze.py` (das Recht auf „Nein": der Ventilatorfall liefert `erreichbar=False` mit den gemessenen 1686 A gegen 800 A statt 2,8e20 W, die abgeleiteten Groessen sind `None` und nicht 0,0, der Kaefig faellt nicht auf den Fertigungsboden zurueck, die Traktionsklasse bleibt Ziffer fuer Ziffer unberuehrt, und der Geltungsbereich benennt Speisung und Leistungsband), `test_bruecke.py` (die Bruecke Agentenkopf ↔ Geometrie-Reiter und die Abzweige: die Marke sieht `vehicle.getriebe`/`customLegs` und kommt aus `meta.json`, eine Notiz oder eine andere Schluesselreihenfolge bewegt sie NICHT, das Formular des Menschen hat Vorrang, und zurueckgehen loescht die Stufen dazwischen nicht), `test_laeufer_cad.py` (die Laeufer im CAD, OHNE FreeCAD: das erzeugte Skript wird mit einem Stellvertreter-FreeCAD wirklich AUSGEFUEHRT und gezaehlt, was dabei entsteht — eine Bauart ohne Magnete bekommt keine Magnetkoerper und keine Taschenschnitte, der Kaefig traegt `n_stab x Schraegungssegmente` Staebe und zwei Ringe, die Segmentwinkel mitteln sich zu 0, der Schleifringlaeufer traegt Wicklung + drei Ringe + drei Buersten und KEINEN Kaefig daneben, ein nicht auslegbarer Kaefig wird abgewiesen, die Nuttiefe trifft die Stromdichtevorgabe, der Anlasswiderstand verdoppelt den Schlupf ohne die Laeuferwicklung zu heizen, und CAD und Querschnittsbild lesen dieselbe `ema_asm.kaefig` Ziffer fuer Ziffer), `test_bilddaten.py` (Bilddatensatz: Zeichner-
...
**Stage 0 — Grenzen (`_gate_grenzen`, `ema_grenzen.py`).** Drei Groessen, die keine Auslegungsentscheidung sind und trotzdem kein Tor hatten: der **Luftspalt** liegt zwischen 0,1 und 2,0 mm (`ema_grenzen.LUFTSPALT_MM`, jetzt die EINE Quelle — er war an vier Stellen verschieden gebunden: `ema_design_ai` 0,5–3,0 · `ema_optimize` 0,1–3,0 · `ema_mobil` 0,3–5,0 · `ema_analysis` mit einer stillen Klemme auf 0,3; und weil `statorID`/`rotorOD` zwei unabhaengige Schemafelder sind, ging ein 10-mm-Spalt unbeanstandet durch); der **Wickelkopf** bleibt unter dem Statoraussendurchmesser (gemessen: 280/190, 6 Leiter, 4° Spreizung liegt 0,5 mm darunter, 6° steht 9,8 mm darueber, mit Isolierhuelse 26 mm — gezeichnet wurde es trotzdem); die **Nut** frisst das Rueckenjoch nicht auf (`ema_em3d.slot_rects` deckelt auf 1 mm Restjoch, `ema_freecad` deckelt NICHT — die beiden rechnen dann verschiedene Maschinen). **Tore, keine Warnungen**; wer eine ausdruecklich will, setzt `geom.luftspaltFreigabe` / `hairpinFreigabe` / `nuttiefeFreigabe`, dann steht die Ueberschreitung als ⚠ im Protokoll statt unbemerkt durchzugehen. Laeuft in `build_cad_preview` UND `run_pipeline`, `fatal=not partial` wie die Rotor-Tore. Erreichbar ohne Lauf ueber `cae_cli.py rotor-check`.

**Rastmoment (`ema_rastmoment.py`).** Die Groesse hinter „sehr praezise auch von der Drehgenauigkeit", und sie fehlte. In ``compute_performance`` stand ``T_cogging_est = Br_NdFeB * R_gap * L_ax * 0.05 / lcm * 1000``: die Remanenz des WERKSTOFFS statt des Luftspaltfelds, ein Beiwert 0,05 ohne Herkunft, und vor allem **ohne die Nutoeffnung** — den staerksten Hebel ueberhaupt. **Gemessen wird es nicht**, und der Grund steht im Modulkopf: den Laeufer im FDM ueber eine Rastperiode zu drehen ergibt bei 24N/10P (Periode 3,0 Grad) T(0)/T(3) = 8,70/0,30 bei N=500, 0,90/4,10 bei N=700, 5,50/3,70 bei N=900 — nach einer vollen Periode muss derselbe Wert stehen, es konvergiert nicht. Ursache ist die Treppung der gedrehten Rastergeometrie; ein Rastmoment braucht ein koerperangepasstes Netz. Also analytisch nach Zhu/Howe, und **getrennt** nach Belastbarkeit: EXAKT sind ``ordnung`` (kgV, Rastfaktor nach Gieras) und ``schraegungsfaktor`` — eine Schraegung um eine Nutteilung loescht die Grundwelle exakt aus, das ist ein Integral; GESCHAETZT (Faktor ~2) ist die Amplitude. ``_abklingen`` war dabei noetig und ist keine Kosmetik: reine Rechteck-Fourierkoeffizienten fallen nur wie 1/m, und dann lag eine 27-Nut-Auslegung (kgV 270) kaum besser als eine 24-Nut (kgV 120) — im stromfreien Spalt klingt eine Stoerung der Wellenlaenge ``Teilung/m`` aber wie ``exp(-2*pi*m*x/Teilung)`` ab (Laplace). **Nebenbefund, der bleibt:** dieses Werkzeug zeichnet OFFENE Nuten — es gibt keinen Nutverschluss, ``nut_breite`` am Bohrungsrand IST die Oeffnung (4,03 mm ueber 0,70 mm Spalt). Das steht in jeder Bewertung.

**Drei neue Paarvergleichsachsen (20 insgesamt): `schraegung`, `spannung`, `strom`.** Die Schraegungsachse faehrt Anteile EINER Nutteilung, nicht runde Gradzahlen — zehn Grad sagen ohne die Nutzahl nichts. ``T_rast_pct`` ist eine ``ZUSATZSPALTEN``: immer angezeigt, aber ``zaehlt=False``. Beides mit Absicht — zaehlen hiesse zu entscheiden, wie wichtig Drehgenauigkeit ist (das entscheidet der Einsatz), weglassen hiesse eine Schraegungsachse zu zeigen, unter der „bewegt NICHT: alles" steht. Dafuer gibt es die Zeile „bewegt aber (ungewichtet)". ``_setz_umrichter`` setzt neben der Grenze auch ``umrichterBezug="wicklung"``: ohne das meinte die Option mit dem Vorgabewert etwas anderes als ihre Nachbarn, und 800 A lieferten weniger Moment als 400 A.

**Umrichterspannung und -strom sind einstellbar — und die Windungszahl ist der Schluessel dazu.** ``INVERTER_V_DC``/``INVERTER_I_MAX`` waren Modulglobale, die niemand aendern konnte; ``aufgabe`` sagte „FEST verdrahtet" an, gerechnet wurde trotzdem mit 800 V. Jetzt ``geom.inverterVdc``/``geom.inverterImax`` (Klemmenwerte, Grundebene des Schemas), gelesen ueber die eine Stelle ``ema_analysis.umrichter(geom)``. Neun Module lasen die Globalen direkt — ``ema_asm``, ``ema_eesm``, ``ema_synrm``, ``ema_paarvergleich``, ``ema_thermal``, ``ema_mobil``, ``ema_referenz`` und zweimal ``ema_analysis`` selbst; alle gehen jetzt durch den Zugriff. **Kein einziger der 20 Aufrufer von ``estimate_dq_currents``/``power_envelope`` musste angefasst werden**, weil alle ohnehin ``geom`` uebergeben und die Funktionen die Grenzen selbst lesen.

**Warum das mehr ist als zwei Zahlen durchreichen:** das ganze elektrische Modell rechnet mit **einer Windung je Nut** (``N_ph = slots/3``); ``conductorsPerSlot`` geht in Kt, psi, Ld/Lq und das Kennfeld **gar nicht** ein, nur in Kupfermasse und Verluste. Die 800 V/800 A gehoerten also zu einer gedachten Einwindungswicklung, und eine wirkliche Klemmenspannung ist darin ohne die Windungszahl nicht darstellbar: bei fester Geometrie und festem Moment liegen die AMPEREWINDUNGEN fest, und N tauscht Strom gegen Spannung (``Kt ~ N``, ``i ~ 1/N``, ``u ~ N``). Ein Umrichter mit 24 V und 200 A treibt dieselbe Maschine wie einer mit 800 V und 6 A — nur die Wicklung ist eine andere. ``umrichter`` rechnet zwischen beidem um (``v_dc_1t = u/N``, ``i_max_1t = i*N``) und nimmt N aus ``ema_wicklung.windungen`` (``turnsPerSlot``, ersatzweise ``conductorsPerSlot``) — **kein zweiter Schluessel**, das ist dieselbe Quelle, aus der schon Strangwiderstand und Kupfermasse lesen.

**Worauf sich die Grenzen beziehen, sagt ``umrichterBezug``** und nicht ihr Wert. Vorgabe ``"einwindung"``: N = 1, jede Altrechnung bleibt Ziffer fuer Ziffer dieselbe (nachgeprueft in ``test_zyklen``). ``"wicklung"``: Klemmenwerte, umgerechnet mit der wirklichen Windungszahl. Der erste Entwurf schloss aus „weicht von der Vorgabe ab" auf „ist eine Klemme" — das brach in der Paarvergleichs-Stromachse sichtbar auf (800 A weniger Moment als 400 A). Wer eine Grenze setzt und den Bezug vergisst, wird von ``sicherheit`` beanstandet. ``umrichter_passt`` sagt, ob sie passt, und ``ema_sicherheit`` macht daraus das Kriterium ``umrichter``: die Gegen-EMK bei ``rpm_to`` soll zwischen 40 % und 85 % der Klemmenspannung belegen. Darueber erreicht die Maschine die Drehzahl nicht, darunter bleibt der Umrichter ungenutzt und es fliesst unnoetig Strom — beides lief vorher klaglos durch. Gewaehlt wird die Windungszahl **nicht** selbst: die Meldung nennt ``--set turnsPerSlot=<n>``, welche Wicklung gebaut wird, entscheidet nicht das Rechenwerkzeug. Wo N > 1 ist, stehen die KLEMMENwerte neben den bezogenen (``Isc_klemme_A``, ``v_dc_1t_V``/``i_max_1t_A`` im Kennfeld) — sonst liest sich ein Kurzschlussstrom von 1452 A wie eine Aussage ueber die gebaute Maschine.

**Der Lastfall ist ZWEIERLEI, und die Frage danach kommt zuerst.** Ein ``Fahrzyklus`` ist v(t) plus Fahrzeug (Masse, Rad, Uebersetzung); ein ``Lastspiel`` ist n(t) und T(t) der Welle, ohne Fahrzeug — fuer Roboter- und Werkzeugachsen, Spindeln, Pumpen, Luefter, Winden, Pruefstaende. Beide laufen durch dieselbe Verlust-, Thermik- und Energierechnung, die haengt ohnehin nur an ``rpm_motor``/``T_motor``; der einzige Unterschied ist, ob die beiden aus einem Fahrzeugmodell gerechnet oder direkt angegeben werden (``compute_drivetrain`` verzweigt in Zeile 1). Die Art steht in den PUNKTEN (zwei Spalten Fahrt, drei Spalten Lastspiel), nicht in einer Datenbankspalte — keine zweite Quelle, keine Wanderung fuer schon abgelegte Zyklen. Weg, Verbrauch je 100 km und v_max sind beim Lastspiel **None** und nicht 0: eine 0 liest sich wie ein Messwert. **Gemessen am 06.09.2026:** ein Roboterarm-Antrieb, der Agent erkannte richtig, dass kein abgelegter Zyklus passt, und baute sich einen — konnte das aber nur in km/h. Um „2200 1/min" zu schreiben, erfand er Rad 0,12 m und Uebersetzung 4; der Lauf meldete fuer ein Gelenk **14,2 km bei 345 kWh/100 km**. ``cae_cli.PFLICHTPUNKTE['lastfall']`` fragt deshalb ZUERST „faehrt sie oder dreht sie nur?" und listet den Bestand **nach Art getrennt** — „abgelegte Lastspiele: keines" ist die Auskunft, die vorher fehlte, weil alle fuenf angebotenen Zyklen Fahrzeuge waren.

**Der Luftspalt im FDM-Raster war IMMER 2,5 mm.** ``AIRGAP_MIN_MM = 2.5`` oeffnete das Luftband unter der Bohrung auf mindestens 2,5 mm, indem es **Rotorrandeisen wegnahm** (das Statoreisen bleibt ausdruecklich stehen). Gemeint war eine Aufloesungshilfe — geeicht an der 280-mm-Ur-Maschine, wo 2,5 mm gut vier Pixel sind; der Kommentar sagte das sogar. Nur ist eine Millimeterangabe an eine Maschinengroesse gebunden und nicht an ein Netz. Gemessen an einem 75-mm-Antrieb mit 0,7 mm Spalt: der Laeufer endete im Feldmodell bei **r = 25,50 statt 27,30 mm**, der Spalt war **2,50 statt 0,70 mm**, und zwar bei JEDER Netzfeinheit gleich — kein Aufloesungsfehler, eine andere Maschine. Weggefressen wurde genau der Bereich, in dem Steg und Taschenkappe liegen; im Feldbild standen die gezeichneten Kreise fuer rotorOD und statorID danach 1,8 mm ausserhalb des Eisens, das sie meinen. An der Ur-Maschine waren dieselben 1,8 mm 1,9 % des Laeuferhalbmessers, an einem 27-mm-Laeufer sind es 6,6 %. Jetzt ``AIRGAP_MIN_PX = 1.0``: **ein** Bildpunkt ist die einzige Forderung, die wirklich aus dem Verfahren kommt (Rotor- und Statoreisen duerfen sich nicht beruehren, sonst ist der magnetische Kreis kurzgeschlossen); alles darueber ist Modellwahl — 4 px waeren bei N=180 ganze 7,8 mm. ``luftspalt_im_netz(geom, N)`` gibt gezeichneten und wirklichen Spalt heraus, ``run_em_analysis`` traegt sie im Ergebnis, die Pipeline warnt, und ``ema_grenzen.cad_gegen_feld`` fragt jetzt den **Rasterer** statt ein zweites Mal dieselben zwei Durchmesser — vorher verglich genau die Probe, die dafuer gebaut war, den Spalt mit sich selbst und meldete „gleich". Wirkung auf die Zahlen: T_maxwell 20,5 -> 6,3 Nm am 75-mm-Antrieb (Auslegungspunkt 6 Nm); **B_gap und Kt aendern sich nicht**, die kommen aus der Formel, nicht aus dem Raster. ``test_fdm_golden.json`` ist neu gesetzt.

**Stage 0 — Grenzen, zweiter Teil.** Der **Hairpin** unterschreitet 3 x 3 mm nicht (`ema_wicklung.HAIRPIN_MIN_M` — er ist ein gebogener Rechteckstab, kein Draht; der Runddraht darf duenner bleiben). `nutgeometrie` klemmte bisher auf 1,5 x 2,0 mm und rechnete klaglos weiter: eine Wicklung, die nicht in ihre Nut geht, kam als Kupfermasse, Widerstand und Verlustleistung heraus. `passt` prueft jetzt **beide** Richtungen (zu tief UND zu breit) und `passt_grund` sagt welche — vorher stand bei einem zu breiten Leiter „0,0 mm zu hoch" da. **Folge:** eine 22-mm-Nut traegt vier Hairpins, keine sechs (25,6 mm noetig); drei Testgeometrien standen auf sechs und waren damit nicht wickelbar.

**Offene Magnettasche (`geom.magTascheOffen`).** Der Steg zwischen Tasche und Rotorrand haelt den Polschuh UND kurzschliesst den Magneten. `ema_topology.taschenoeffnung`/`stegbreite_mm` sind die eine Quelle fuer den WIRKLICHEN Steg; `nein` (Vorgabe) aendert nichts. Offen laesst der Speichenmagnet bis an den Luftspalt laufen — gemessen **B_gap +37 %** an einem 10-poligen Laeufer. Dafuer musste `_analytical_Bgap` zwei Dinge lernen: die konzentrierende Flaeche ist die radiale Laenge DES MAGNETEN aus `magnet_legs` (dort stand `r_rot - r_shaft - 2*1.0`, also der ganze Ringraum abzueglich eines hartkodierten 1-mm-Stegs — 19,3 mm gegen 12,5 mm wirklich gezeichneten Magneten), und die Streuung `k_leak` ist AUFGETEILT in Stirnstreuung `K_LEAK_STIRN`=0,92 und Stegkurzschluss `K_LEAK_STEG`; ihr Produkt ist exakt die alte 0,85, geschlossene Auslegungen aendern sich um keine Stelle. `rotor_layout_check` nimmt eine ausdrueckliche Oeffnung an und sagt dafuer, was mechanisch an die Stelle des Stegs treten muss — **die Fliehkraftpruefung rechnet sie NICHT**, und die Paarvergleichs-Achse `taschenoeffnung` schreibt genau das unter jede offene Option.

**Ein gerissenes Tor schlaegt Auswege vor, statt nur abzusagen**
(`ema_rotorcheck.entlastung`, `ema_pipeline.AuslegungReisst`, seit 12.09.2026).
Gemeldet als: „wenn die Festigkeit nicht reicht, startet er nicht, er sagt nur
'Analyse fehlgeschlagen'." Das stimmte — und bei genau diesem Tor ist eine nackte
Absage besonders aermlich, denn die Ringspannung ist eine **geschlossene Formel**
(`sigma ~ rho*omega^2*((1-lam)a^2 + (3+lam)b^2)`): jeder Hebel darin laesst sich
nach dem zulaessigen Wert aufloesen. `entlastung` rechnet vier durch —
Hoechstdrehzahl (`n ~ 1/sqrt(sigma)`, exakt, der staerkste Hebel),
Rotorblech (noetige Fliessgrenze, und ob eines der hinterlegten sie hat),
Rotoraussendurchmesser und Wellendurchmesser (beide per Halbierung) — und
**jeder Weg wird gegen DASSELBE Tor nachgeprueft, bevor er vorgeschlagen wird**:
einer, der es erneut reissen laesst, ist keiner. Gerundet wird dabei auf die
sichere Seite (gemessen: auf 50 aufgerundet lag die Sicherheit wieder bei 1,297
statt 1,30). Die Bohrung taucht bei der Beispielmaschine bewusst NICHT auf —
ihr Term traegt dort 1,7 % der Spannung, und ein wirkungsloser Vorschlag waere
schlechter als keiner.

**Vorgeschlagen ist nicht gewaehlt.** Jeder Weg nennt seinen Preis (weniger
Drehzahl heisst weniger Eckleistung, kleinerer Aussendurchmesser weniger Moment,
ein festeres Blech mehr Ummagnetisierungsverluste), die Ausnahme traegt sie mit,
`/status` reicht sie durch, und die Oberflaeche macht Knoepfe daraus, die **nur
das Formular setzen** — gestartet wird nichts. Beim `rotorOD` zieht die Uebernahme
die Statorbohrung mit, sonst reisst beim naechsten Anlauf das Luftspalt-Tor und
es saehe aus, als haette der Vorschlag nichts genuetzt. Dieselbe Haltung wie
`ema_asm`s „nicht erreichbar, weil …": ein begruendetes Nein ist ein Ergebnis.
Test: `test_rotorcheck.py`.
...
Exit 1 when it tears. See `BEFUNDE.md`. **And since 19.09.2026 it also runs the
machine-TYPE gate** (`_gate_laeufer`): `rotor_layout_check` inspects magnet
pockets, and a cage, salient-pole or armature rotor has none — its narrow spot is
a different one (the web over the cage slot, the pole foot with its centrifugal
force, the armature slot, the commutator). The verb reported „Layout OK" for a
separately-excited machine while the run refused it at the pole attachment; the
remedy is the same as for the centrifugal gate — call the pipeline's own gate, not
a second copy. The verdict is read from „ABGELEHNT" and **not** from the warning
sign: a ⚠ can be a finding that forbids nothing (a commutator longer than the
stack, a damper pitch close to the slot pitch), and conflating the two means
reading a note as a no. **Not covered yet:** balance-bolt holes and flux barriers
— a passing gate does not rule out a breakthrough from those.
...
### Eigener Rechensatz, zweiter und dritter Löser, Topologieoptimierung

`ema_deck.py` · `ema_z88.py` · `ema_aster.py` · `ema_topopt.py` — ein zweiter Weg zur
Rotor-Festigkeit **ohne FreeCAD**. Der bestehende FreeCAD/ccx-Pfad bleibt unverändert und ist weiter
...
**Der Löservergleich.** Gleiches Netz, gleiche Last, gleiche Größen auf allen Seiten
...
Und mit **Code Aster** als drittem, an einem 5.745-Knoten-Vollrotor (21.822 Tet4,
12.000 min⁻¹, `--solver alle`):

| Größe | CalculiX | Z88 | Code Aster | max. Abw. |
|---|---:|---:|---:|---:|
| σ_v Spitze | 153,620 MPa | 153,640 MPa | 153,620 MPa | 0,013 % |
| σ_v P99 | 105,870 MPa | 105,880 MPa | 105,870 MPa | 0,009 % |
| Ringspannung Bohrung | 117,080 MPa | 117,090 MPa | 117,080 MPa | 0,009 % |

**Dass ccx und Aster auf die zweite Nachkommastelle gleich herauskommen, ist die
richtige Erwartung und kein Verdacht** — der Tet4 ist ein Element konstanter Dehnung,
seine Spannung folgt exakt aus den Knotenverschiebungen, und zwei korrekte
Direktlöser auf EINEM Netz müssen deshalb auf Rundungsniveau übereinstimmen.
Nachgemessen Element für Element: größte Abweichung der Vergleichsspannung
**6,0·10⁻⁵ MPa bei 157,6 MPa Spitze = 3,8·10⁻⁵ %**, `u_max` beide 23,084 µm. Der
**Verdachtsfall wäre eine BITgleiche Zahl** gewesen (dann läse ein Löser die Ausgabe
des anderen); `test_aster.py --echt` prüft beides — klein genug UND nicht null.
...
`struct_solver` kennt jetzt `freecad` | `ccx` | `z88` | `aster` | `beide` (ccx+z88)
| `alle` (alle drei). **Den Polsektor kann nur CalculiX**: Z88 kennt weder zyklische
Symmetrie noch schiefe Symmetrieebenen, und Aster bekommt die `*EQUATION`-Paarung in
der `.mail` nicht geschrieben — sobald ein zweiter Löser mitläuft, wird deshalb der
volle Rotor vernetzt, und dann sehen alle bitgleich dasselbe Netz.
...
3. **`rpm_base` an `estimate_dq_currents`.** `render_preview_frame` setzte
   `rpm_base=rpm` — dann liegt jeder Punkt per Definition an der Eckdrehzahl und
   es gibt nie Feldschwächung. Seit dem 15.09.2026 gibt es dort `rpm_base=None`,
   und `estimate_dq_currents` rechnet die Eckdrehzahl über
   `ema_analysis.eckdrehzahl` selbst (EINE Quelle, s. `BEFUNDE.md`).
...
daraus kommt.
**Die Baulaenge kommt aus der Geometrie** (`stapellaenge_m`, seit 12.09.2026).
`compute_performance`, `compute_advanced_em` und `run_em_analysis` nahmen die
Blechpaketlaenge als **optionales** `axial_mm` mit einem stillen Rueckfall auf
**feste 80 mm** — und vier Produktivstellen gaben sie nicht mit: der schnelle
Bewerter (`ema_optimize._eval_geom`, an dem Zielwertoptimierung,
Parameterstudie, Magnet-Feinoptimierer und die Vorsortierung der KI-Entwuerfe
samt Trainingslabels haengen), die Feldbilder der Parameterstudie, die
statische EM-Stufe der Pipeline samt Drehzahl-Sweep und der Handy-Pfad.
`psi_pm = p·(2/π)·B_gap·R_gap·L_ax` und `Kt = 1,5·p·psi_pm` sind in L **linear**;
gemessen stand `Kt` ueber 40…120 mm auf 0,0410 Nm/A still, wo 0,021 / 0,041 /
0,062 richtig sind — bei 40 mm also um den **Faktor 2 zu hoch**. Unauffaellig
blieb es, weil 80 mm die Laenge des `--frisch`-Payloads und der meisten
Testgeometrien IST: am Auslegungspunkt stimmte die Zahl zufaellig, falsch wurde
sie erst, sobald jemand die Laenge aenderte — also genau in einer Laengenstudie
oder einer Optimierung, die an ihr dreht. Jetzt entscheidet **eine** Funktion:
ausdrueckliche Angabe → `geom["axialLen"]` → 80 mm; und `_apply_params`
schreibt die Laenge auch IN die Geometrie zurueck, damit `geom` und das daneben
gefuehrte `axial` nicht zwei Wahrheiten ueber dieselbe Groesse sind. **Das
bewegt Altbestand:** von 74 gespeicherten Projekten haben 50 eine Laenge ≠ 80 mm,
deren Kt/psi/EMK verschieben sich mit L/80. `test_fdm_golden.py` gibt `axial_mm`
seit jeher ausdruecklich mit und ist unberuehrt; zwei festgenagelte Erwartungen
in `test_zyklen.py` waren es nicht und sind korrigiert (der dortige 75-mm-Antrieb
traegt `axialLen=60`: Spannungsausnutzung 310 % → 231 %, passende Windungszahl
2 → 3). Voller Befund in `BEFUNDE.md`.
...
### Spritzoel als Lattice-Boltzmann (FluidX3D, GPU) — `ema_fluidx3d.py` / `fluidx3d_runner.py`

**Dritter Loeser auf demselben Spritzoel-Fall**, neben dem qualitativen Mantaflow-Pfad
(`ema_oilspray`) und dem quantitativen VOF-Pfad (`ema_cfd`). Er rechnet die freie
Oberflaeche als Lattice-Boltzmann auf der GPU und loest dabei die Duesenbohrung
tatsaechlich auf — die Stelle, an der der Mantaflow-Pfad bei 1-mm-Duesen an seine
Grenze kommt („Strahl unter-aufgeloest"). Die Einstellungen kommen aus **demselben**
`oil`-Satz wie 💧 und 🌊 (`_fx3dBuildPayload` baut auf `_oilBuildPayload` auf); ein
zweiter Satz Spritzoel-Felder waere die Stelle, an der die drei Loeser auseinander-
laufen, ohne dass es jemand merkt.

**Gemessen am Probelauf 12.09.2026** (RTX 3090, Wickelkopf-STL einer 305-mm-Maschine):
9,0 Mio. Zellen (208³) belegen **572 MB** und rechnen **10,4 ms Realzeit in 11,3 s**
bei **9.526 MLUPS** (2,9 TB/s effektiv), Massenerhaltung **−1,1 %** ueber 12.000
Schritte; der Strahl trifft nach 2,6 ms, die benetzten Zellen am Kupfer wachsen
0 → 763 → 880. Zum Vergleich: derselbe Fall ist im Mantaflow-Pfad ein CPU-Bake.

**Drei Grenzen, jede gemessen — sie tragen je eine Entwurfsentscheidung:**

1. **Die ganze Maschine geht nicht.** Ein 287-mm-Wuerfel bei 3 GB ergibt 0,94 mm je
   Zelle, die 1-mm-Bohrung waere **1,06 Zellen**. Drei Zellen darueber bei voller
   Maschine hiessen 1007³ = 1,0 Mrd. Zellen ≈ **65 GB** auf einer 24-GB-Karte.
   Deshalb rechnet das Modul grundsaetzlich ein **Fenster** um EINE Duese — nicht
   als Sparmassnahme, sondern weil die Alternative nicht existiert.
   `vollmaschine_kosten` rechnet die Zahl je Auslegung aus, statt sie zu behaupten,
   und sie steht in jedem Ergebnis und in der Maske.
2. **Es gibt keinen Dauerstrahl.** `mass` — die Erhaltungsgroesse des Freiflaechen-
   loesers — hat **keinen Wirtspuffer** (`lbm.cpp:149`, `allocate_host=false`), und
   ein `TYPE_E`-Rand traegt keine Masse in die Grenzflaeche ein: gemessen **1,94 mm³**
   Oel nach 12.000 Schritten gegen 21,8 eingesetzte, also 0,5 % des Duesenstroms.
   Gerechnet wird deshalb ein **Oelstoss** — die Saeule steht zum Zeitpunkt 0 in
   freier Luft **ausserhalb** des Spritzrings und fliegt los. Der erste Versuch legte
   sie nach INNEN, und dann lagen schon zum Zeitpunkt 0 dreizehn Zellen am Kupfer:
   das ist kein Strahl, sondern ein vorgefuellter Leiterzwischenraum
   (`test_saeule_startet_ausserhalb` nagelt das Praedikat fest). Weil die Saeule
   Platz hinter der Muendung braucht, geht ihre Laenge in die **Fensterlage** ein;
   reicht das Fenster nicht, wird der Stoss gekuerzt und das **gesagt**.
3. **Isotherm.** `SURFACE` und `TEMPERATURE` sind in keinem der 40 mitgelieferten
   Faelle kombiniert. Kein Temperaturfeld, kein Waermeuebergang — dieselbe Grenze wie
   `interFoam`. Dieser Pfad ersetzt den **qualitativen** 💧-Pfad, **nicht** `ema_cfd`,
   aus dem der HTC kommt.

**Die Fensterlage kommt aus der Geometrie, nicht aus einer Zahl** (`fenster_bestimmen`).
Der erste Probelauf setzte sie nach Augenmass auf z = 98 mm und traf die **geraden
Staebe** statt der Krone. Jetzt: Ueberhang = Ecken mit |z| > Paketlaenge/2, daraus
Kronenradius und Kronenhoehe; der Ausschnitt wird um −θ_c um die Motorachse gedreht,
bis er auf der +x-Achse liegt (dann ist „radial nach innen" = −x und „unten" = −y wie
im waagerechten Einbau von `ema_oilspray`), und die STL wird dafuer in Python gedreht
und neu geschrieben — alle Geometrieentscheidungen an EINER, pruefbaren Stelle statt
halb im C++.

**FluidX3D hat keine Eingabedatei: jeder Fall IST Quelltext** (`main_setup()` in
`src/setup.cpp`) und wird **neu uebersetzt** (gemessen 40 s). `fluidx3d_runner`
macht daraus denselben Ablauf wie bei jedem anderen Werkzeug hier, mit vier Dingen,
die nicht offensichtlich sind:

* **Gearbeitet wird in einer KOPIE** unter `$CAE_FLUIDX3D_HEIM` (Vorgabe
  `~/fluidx3d_cae`), ausserhalb des Repos wie `~/aster-build`. Der Quellbaum traegt
  die Faelle des Menschen — dort steht eine Laval-Duese —, und ein Werkzeug, das die
  ueberschreibt, ist keines.
* **`defines.hpp` wird gesetzt, nicht ersetzt** (`defines_setzen`, rein und getestet):
  die Zeilen tragen Zeilenendkommentare (`#define SURFACE // enables …`), ein
  `replace` auf die ganze Zeile greift deshalb nicht — derselbe Fallstrick wie beim
  Zeilenendkommentar in `.gitignore`.
* **Uebersetzt wird zwischengespeichert** (SHA1 ueber den erzeugten Fall): aendert
  sich nur die Drehzahl der Ausgabe, laeuft `g++` nicht noch einmal — dasselbe
  Argument wie der Netz-Cache in `ema_em3d`.
* **Ergebnisdaten gehen in DATEIEN, nicht durch die Leitung.** FluidX3Ds
  Fortschrittszeile wird mit `\r` ueberschrieben (`info.cpp:105`) und aus einem
  NEBENLAEUFIGEN Thread gedruckt (`main.cpp:159`); zeilenweises Lesen sieht die
  eigenen Marker deshalb mittendrin in einem sehr langen „Satz", vorn wie hinten von
  Fortschrittszeichen eingerahmt (gemessen). Im Strom bleiben nur
  `FX3D_STAGE`/`STEP`/`DONE`, und `marker_aus` holt sie an beliebiger Stelle heraus;
  Schnittbilder und Kennwerte schreibt das Setup nach `<work>/aus/`. Der **Schnitt**
  statt des Volumens: ein phi-Feld von 208³ sind 36 MB je Ausgabeschritt, der
  Schnitt daraus 43 kB — fuer Video und Kennwerte wird nichts anderes gebraucht.

**Lizenz — und warum sie in den Code gehoert.** FluidX3D (ProjectPhysX): keine
kommerzielle, keine militaerische Nutzung, **kein KI-Training auf dem Quelltext**,
der Lizenzhinweis bleibt stehen, und wer Ergebnisse einer **geaenderten** Fassung
veroeffentlicht, muss die geaenderte Quelle veroeffentlichen. Das erzeugte
`setup.cpp` IST eine solche Aenderung und traegt deshalb `ALTERED SOURCE VERSION`
samt Verweis im Kopf (`test_setup_code_vollstaendig_und_lizenztreu` prueft das);
`vorbereiten` legt eine `HERKUNFT.txt` neben die Kopie. Der Quellbaum bleibt
ausserhalb der Versionsverwaltung (`.gitignore`).

Server: `/fluidx3d` (503 ohne FluidX3D), `/fluidx3d/status|abort`, **`/fluidx3d/kosten`**
(was der Fall kostet, BEVOR er laeuft — gerechnet auf dem Server, nicht im Browser
nachgebaut, damit die Zahl aus derselben Quelle kommt wie der Lauf),
`/project/<id>/fluidx3d(/saved(/<rid>(/video|/delete)))`, Job-Typ `fluidx3d`,
Video ueber `video_subdirs["fx3d"]`. UI: Reiter **🌀 FluidX3D (LBM)**. Test:
`test_fluidx3d.py` (14 Pruefungen, ohne GPU, ohne Uebersetzer, ohne FreeCAD).
...
| `cae_cli.py` | **Agent-Kommandozeile** (Kern stdlib only, kein `requests`): **fuenfunddreissig** Verben — `health/status/geom/projects/results/run/wait/routes/raw` über HTTP auf `:5000`, dazu **`auftrag`** — die ABSICHT des Projekts zeigen und **ergaenzen** (s. `ema_auftrag.py`), **`papierkorb`** — was entsorgt wurde auflisten, zurueckholen, endgueltig leeren (s. `ema_ablage.py`), `steckbrief` (s. `ema_steckbrief.py`), `beitrag` (s. `ema_beitrag.py`), **`bericht`** — Berichte nachsehen und erzeugen (`--art liste|studie|reihe|elmer|eigen`): die fuenf Berichtswege hingen alle an Knoepfen der Oberflaeche, und ein Knopf, den nur sie hat, ist fuer den Agenten nicht vorhanden. EIN Verb mit `--art` statt fuenf Verben — ein oertliches Modell haelt keine fuenf Schemata auseinander, die alle „Bericht" heissen. `--art eigen --bausteine steckbrief,getriebe,…` erzeugt den Gesamtbericht aus gerechneten Abschnitten (PDF **und** HTML, weil ein PDF kein Video abspielt), `welle` (s. `ema_welle.py`), **`getriebe`** — Getriebeauslegung samt Einbau in der Welle (s. `ema_getriebe.py`), **`studie`** — EINEN Parameter von x nach y durchfahren und alle Kennwerte mitschreiben (`ema_paramstudy.run_study`), **`zielwert`** — die Zielwertoptimierung (`ema_optimize.optimize`), `paarvergleich`, `rotor-check`, `screen`, `bilddaten`, `struktur`, `topopt`, `db`, `lernen`, `recherche`, `maschinenart`, `aufgabe`, `zyklus`, `sicherheit`, `feld2d` (s. `ema_em2d_harm.py`), `feld3d` (s. `ema_em3d_harm.py`) und `feldbild` (s. `ema_feldbild.py`), die **lokal** rechnen (und dafür erst beim Aufruf `ema_deck`/`ema_z88` importieren, damit der Rest schlank bleibt). Gedacht als *Skill* für eine Agent-Harness (PI), nicht als MCP-Server: ein lokales Modell kann 175 Routen nicht als 175 Werkzeugschemata halten. Drei Eigenschaften tragen das: Base64-Nutzlasten (`*_b64`, PNG/VTP/PDF) werden **immer** herausgefiltert und die Ausgabe bei 12 000 Zeichen gekappt (sonst ist der Kontext nach einer Antwort voll), der **Exit-Code** trägt den Zustand (0 ok · 1 Gegenstelle · 2 Bedienfehler · 3 Server aus · 4 Zeitüberschreitung) — eingeordnet wird ein oertlicher Fehlschlag an EINER Stelle, `_oertlicher_fehlschlag`, und die trennt zwei Faelle, die vorher zusammenfielen: ein **fehlendes Python-Modul** ist kein Befund ueber die Auslegung (`gmsh` wird erst im Aufruf importiert und liegt nur in der venv — der `ModuleNotFoundError` fiel deshalb mitten in den Fang, der fuer „gmsh kommt mit dieser Geometrie nicht zurecht" gebaut ist, und meldete „Vernetzung fehlgeschlagen" mit Exit 1 = Gegenstelle; jetzt Exit 2 samt `sys.executable` und dem Aufruf, der geht, und dem ausdruecklichen Satz, dass an der Auslegung nichts geprueft wurde), waehrend jeder andere Fehler Exit 1 samt Fehlertyp bleibt. An den beiden Feldstufen stand dafuer die **nackte Zahl 4** statt einer benannten Konstante — in dieser CLI die Zeitueberschreitung, was zum Vergroebern des Netzes einlaedt und nichts hilft; es waren die einzigen zwei Literale im ganzen Verbsatz (s. `BEFUNDE.md`, 2026-09-11), und `run --set KEY=WERT` ändert einen geerbten Payload parameterweise statt ihn neu zu schreiben. **`--frisch`** traegt seit dem Fahrrad-Fall auch `cycle="off"`, `cycle_csv` und das ganze `vehicle`-Dict (s. `ema_zyklen.py`) — vorher kannte der Payload den Fahrzyklus nicht, die Pipeline nahm still `wltp3` samt Autobahn-Volllast am 1600-kg-Pkw, und `--set cycle=off` wurde abgewiesen, weil der Schluessel nirgends stand. `--set KEY=@datei` liest den Wert aus einer Datei (fuer `cycle_csv`, das hunderte Zeilen hat). **`--frisch`** (alle acht Payload-Verben) ist die Gegenrichtung: ein neutraler Grundpayload aus `ema_text2ema.SCHEMA` statt eines Altprojekts. Er musste sein, weil es vorher nur `--payload` (90 Schlüssel von Hand), `--payload-file` und `--from-project` gab — praktisch also nur `--from-project last`, und damit erbte **jede neue Auslegung Polzahl, Nutzahl, Magnetanordnung, Kühlung und Werkstoffe der vorigen**. Der Skill warnte in Prosa davor und zeigte in jedem Beispiel `last`, weil es nichts anderes gab; ein Modell folgt den Beispielen. Der frische Satz läuft durch `ema_screen.einpassen` — die rohen Schemavorgaben fallen sonst durchs Layouttor (gemessen: Taschenkollision 4,37 mm), weil `magDist`=2 mm die beiden V-Schenkel nie aneinander vorbeilässt. Das ist ein Defekt der **Vorgabe**, der genauso in `ema.html` steht (`GEOM.magDist: 2`); `frischer_payload` macht ihn deshalb nur für diesen einen Payload minimal auf (2 → 5 mm) statt den Startzustand der Oberfläche mitzuverschieben. `--set` prüft gegen `/param_schema` (Typ, ganzzahlig, `lo`/`hi`, Auswahllisten), sortiert nach `in_geom` selbst nach `geom` bzw. auf die obere Ebene ein, nimmt Punktpfade (`vehicle.mass_kg=…`) und **weist Grenzverletzungen ab, statt zu klemmen** — ein geklemmter Wert sieht für den Aufrufer wie ein angenommener aus. Unbekannte Namen fallen mit `difflib`-Vorschlag durch. `_ALIAS`/`_MIRROR` fangen die eine echte Namensabweichung: `/param_schema` spricht das Vokabular von `ema_text2ema.SCHEMA` (`axialLen`), die Pipeline liest `data["axial_len"]` (`ema_pipeline.py:1547`) — ohne die Tabelle landete der Wert in einem toten Schlüssel. Bei `run em3d`/`em3d_sweep` traegt der Payload zusaetzlich die **Projektkennung seiner Quelle** (`project_id` aus `--from-project`): ohne die nimmt `/em3d` das im Server zuletzt aktive Projekt, und dann liegen 2D und 3D derselben Maschine in zwei Projekten — `em3d.compare_2d` vergleicht zwei Fremde. `RUN_ROUTES` führt Start- **und** Statusroute je Stufe (`/cad_preview/status`, `/em3d/status` …), und `_wait` behandelt `idle` erst nach einem gesehenen Laufzustand als Abschluss — sonst meldet ein vierstündiger Lauf nach 0 s „fertig" |
...
| `ema_freecad.py` | FreeCAD script generators (rotor, full motor, rotor FEM); interior pockets + surface arc magnets from `magnet_legs`; parametric hairpin end-windings (`conductorsPerSlot`/`coilPitch`, collision-free radial-split crowns). **Die MASCHINENART entscheidet, was im Laeufer liegt** (seit 18.09.2026): `gen_magnets` wird mit `ema_maschinenart.hole(...).hat_magnete` verundet, und die Taschenschleife haengt an einem eigenen `HAT_MAGNETE` — wer die Magnete nur ausblendet (`genMagnets=False`, der stufenweise Aufbau), soll den Laeufer weiterhin mit seinen Taschen sehen; eine Asynchronmaschine hat dagegen gar keine. Vorher fragte der Erzeuger die Art nicht, und ein ASM-Laeufer haette Magnettaschen UND Magnete UND einen Kaefig bekommen. **Beide ASM-Laeuferbauformen** kommen aus `ema_asm`: der Kaefig (Nuten, Staebe, beide Kurzschlussringe) und der Schleifringlaeufer (Wicklungsbuendel, drei Ringe, drei Buersten). `_laeufernuten` ist der gemeinsame Nutbauer — eine Nut ist eine Nut, was darin liegt entscheidet spaeter —, und er zeichnet die **Schraegung in GERADE Segmente** zerlegt statt als tordiertes Prisma (dieselbe Entscheidung wie `ema_em3d`s Staffelung, dort gemessen); die Segmentwinkel mitteln sich zu 0, die Nut sitzt also im Mittel dort, wo sie ohne Schraegung saesse. **Ein nicht auslegbarer Kaefig wird abgewiesen, nicht gezeichnet** (`geom.kaefigFreigabe` ueberstimmt) — sonst faellt `kaefig` still auf den 2-mm-Fertigungsboden zurueck. Nur die ZEICHENmasse der Schleifringe werden eingebettet: das Urteil ueber die Umfangsgeschwindigkeit gehoert ins Protokoll, und als JSON-`true`/`null` waere es im erzeugten **Python**-Skript ohnehin ein NameError |
| `ema_agent.py` / `ema_agent.html` (Routen `/agent…`) | **PI im Browser: links der Agent, rechts was dabei herauskommt.** Startmaske (Projekt · Sitzung · Modell · erster Auftrag) und danach ein geteilter Bildschirm — links der Denk-/Antwortstrom mit Prompt-Feld, rechts Werkzeugausgaben und **Bilder**, die nach oben wegrollen. Gebaut fuer die Bildschirmaufnahme; im Terminal ginge es auf dieser Maschine gar nicht (kein `tmux`/`screen`/`zellij`/`dtach`, kein `chafa`/`timg`/Sixel — und ohne sudo nicht nachruestbar). **Angesprochen wird `pi --mode rpc`**, ein zweiseitiges Protokoll: EIN Prozess haelt die Sitzung, jeder Prompt ist eine Zeile auf stdin, das Modell bleibt geladen. Die Form stand nirgends und wurde gemessen: hinein `{"type":"prompt","message":…}` (mit `prompt`/`content`/`text` als Feldname scheitert es mit „Cannot read properties of undefined (reading 'startsWith')"), heraus NDJSON — links zaehlen `assistantMessageEvent.type` = `thinking_delta`/`text_delta`/`toolcall_start`, rechts `tool_execution_start`/`tool_execution_end` (`result.content[].text`, `isError`). **Zugende ist `agent_settled`**, nicht `turn_end` und nicht `agent_end` (die kommen davor) — erst danach darf die Eingabe wieder frei sein. Der Prozess laeuft aus der **Repo-Wurzel** (PI sortiert Sitzungen nach cwd) mit `~/.npm-global/bin` im PATH (dort liegt `pi`, nicht in `~/.local/bin`). **Neue Bilder ueber die Datei-Aenderungszeit** in `<projekt>/charts|cad_images`, nicht aus dem Werkzeugtext: welche Bilder ein Lauf erzeugt, steht dort gar nicht — die Pipeline schreibt sie nebenbei. Ein **Ringpuffer** (`?ab=`) laesst einen neu geladenen Browser dort wieder einsteigen, wo er war. Ein gebundenes Projekt geht als **Systemzusatz** herein (nicht als Prompt, den das Modell vergessen kann) und sagt ausdruecklich, dass es **keine Vorlage** ist. Der Aufseher fasst den Strom nur zusammen — zwei Woerter Antwort sind 75 `message_update`-Ereignisse — und rechnet nichts: der Agent ruft `cae_cli.py` wie im Terminal. Einstieg: **eigener Reiter 🤖 Agent** in `ema.html` (`panel-agent`, ein `<iframe>` auf `/agent` — eine Quelle, kein zweites Layout; die Quelle wird **erst beim ersten Oeffnen** gesetzt, sonst haengt schon beim Seitenstart ein offener NDJSON-Strom an einem Server-Arbeiter) sowie derselbe Knopf in der Kopfzeile; `↗ in eigenem Fenster` fuehrt auf die eigene Seite. Weil die Seite damit bei jedem Neuladen der Oberflaeche neu startet, fragt sie zuerst `/agent/status`: **laeuft schon einer, knuepft sie an** (`?ab=0` zeichnet den Verlauf aus dem Ringpuffer nach) statt die Startmaske zu zeigen, die `/agent/start` ohnehin nur abweisen wuerde. **Vier Dinge fuer die Aufzeichnung:** (1) **Mitlaufen je Spalte als gemerkter Zustand** (`FOLGT`), nicht als Abstandsfaustregel bei jedem Anhaengen — eine einzelne hohe Kachel oder ein Bild, dessen Hoehe erst mit `load` feststeht, liesse die Anzeige sonst mitten im Lauf stehen; wer hochscrollt, bekommt „⤓ Neues" statt weggerissen zu werden, und Bilder ziehen nach `load` nach. **Und die Spalte GLEITET ans Ende, sie springt nicht** (`gleitStart`/`gleitStopp`, `requestAnimationFrame`): das frühere `scrollTop = scrollHeight` stellte die Spalte mit der neuen Kachel schon wieder unten hin — in der Bildschirmaufnahme sah man dann nur, DASS sich etwas geändert hatte, nicht WAS, und gerade rechts ist eine Kachel ein ganzes Rechenergebnis oder ein Bild. Das Tempo ist `rest/GLEIT_TAU_S` (2,5 s Zeitkonstante), nach unten auf `GLEIT_MIN_PX_S`=260 px/s begrenzt: fest wäre eines von beidem falsch — bei einem Zeichen-Anhang links stünde die Spalte sekundenlang nach, bei fünf Kacheln auf einmal liefe sie eine halbe Minute hinterher. Zwei Fallen dabei: die EIGENE Bewegung darf nicht als Bedienung gelten (während des Gleitens steht die Spalte per Definition nicht unten — ohne den `erwartet`-Vergleich schaltete das Mitlaufen sich bei der ersten neuen Kachel selbst ab und „⤓ Neues" erschiene, ohne dass jemand gescrollt hat), und „⤓ Neues" selbst bleibt ein SPRUNG: das ist eine ausdrückliche Bitte, ans Ende zu kommen. Gilt für beide Köpfe, weil PI und Hermes dieselbe Seite bedienen. (2) **Stoppuhr** in der Kopfzeile: Gesamtlaufzeit **und** Dauer des laufenden Zuges, aus den Ereignis-Zeitstempeln (`start_ts` in `/agent/status`), also auch nach einem Neuladen richtig; jede Kachel traegt ihre Zeit. (3) **`sichern()`** schreibt `protokoll_<marke>.md` nach `<projekt>/agent/`, ohne Bindung nach `~/cae_projekte/_agent_laeufe/<marke>/` (fuehrender `_`, sonst taucht der Ordner als Projekt in jeder Liste auf) — **automatisch nach jedem Zugende** und beim Beenden, dazu der Knopf `💾 Sichern`. Bilder werden **verwiesen, nicht kopiert**. Quelle ist die **anhaengende Mitschrift** `ereignisse_<marke>.jsonl`, nicht der Ring: der Ring haelt nur `RINGGROESSE`=4000 Ereignisse und laeuft in einem langen Lauf ueber — ein aus ihm geschriebenes Protokoll verloere dann seinen Anfang. Aus demselben Grund ist `satz["i"]` eine **durchzaehlende Nummer** und nicht die Stelle im Ring, sonst wird `?ab=` nach dem ersten Ueberlauf mehrdeutig. Der **Ordner steht mit dem Start fest** (`zielordner()` haelt ihn), und eine schon laufende Aufnahme **zieht mit** (`Aufnahme.umziehen`, `os.rename` im selben Dateisystem — der offene Schreibgriff bleibt gueltig): wer die Aufnahme vor dem Agenten startet, hatte sonst Video und Protokoll in zwei Ordnern, die eine Minute auseinanderlagen. Die Kennung `<marke>` im Dateinamen haelt mehrere Laeufe **im selben Projektordner** auseinander. (4) **Bildschirmaufnahme** (`getDisplayMedia` + `MediaRecorder`, 5 B/s, 0,7 Mbit/s ≈ 5 MB/min): der Browser sammelt **nichts**, jedes Stueck geht alle 5 s per POST an `/agent/video/stueck` und wird dort angehaengt (`Aufnahme` in `ema_agent.py`) — flacher Speicherbedarf, die Datei ist waehrend des Laufs schon vollstaendig, und bei `VIDEO_MAX_MB` (800) endet sie von selbst. Im Reiter braucht der `<iframe>` dafuer `allow="display-capture"`. **Zwischenruf** (`merken`/`_hinweise_uebergeben`, Route `/agent/hinweis`, eigenes Feld unter dem Prompt): PI nimmt waehrend eines laufenden Zuges keinen zweiten Prompt an — wer aber zusieht, wie der Agent zwanzig Minuten in die falsche Richtung laeuft, soll das sagen koennen, ohne abzubrechen und ohne den Moment abzupassen. Der Ruf wird gemerkt, sofort quittiert und am naechsten Zugende **gesammelt als EIN Auftrag** uebergeben (drei Rufe waehrend eines Zuges sind ein Gedanke, keine drei Auftraege); wartet der Agent ohnehin, geht er direkt durch. Das Feld bleibt waehrend der Arbeit ausdruecklich bedienbar — sonst haette es keinen Zweck. **Video, Pause und Projektakte (nachgereicht):** die Aufnahme liegt an EINEM festen Ort (`VIDEO_ORDNER`, Vorgabe `~/Videos`, ueber `CAE_VIDEO_ORDNER` umstellbar) und nicht mehr beim Projekt — ein Bildschirmvideo ist kein Rechenergebnis; der Bezug zum Lauf steckt im **Dateinamen** (`agent_<marke>_<projekt>.webm`), und `sichern()` schreibt den Pfad ins Protokoll. `umziehen` entfaellt damit ersatzlos. **Waehrend der Server rechnet, wird die Aufnahme angehalten**: `/agent/status` traegt jetzt `rechnet`/`rechnet_was` (`server._rechnet` sieht die Modulglobalen nach dem Muster `_<name>_state` durch — ein neuer Zustand ist damit automatisch erfasst, eine Handliste waere beim naechsten still unvollstaendig), die Seite fragt alle 5 s und ruft `MediaRecorder.pause()/resume()`; `Aufnahme.pausieren` fuehrt nur Buch, damit die gemeldete Dauer die AUFGEZEICHNETE ist. Ohne das besteht ein vierstuendiger Lauf fast ganz aus einem stehenden Fortschrittsbalken und die 800-MB-Grenze ist erreicht, bevor etwas Sehenswertes im Bild war. **`projektakte_schreiben()`** schreibt `AGENTS.projekt.md` bei JEDEM Start — auch ohne Projektbindung. Vorher schrieb sie **nur** `start_agent.sh` und **nur** bei gebundenem Projekt: jeder Browser-Lauf las die Akte des letzten Terminallaufs als seine eigene (gemessen am 03.09. ein Flugzeugantrieb vom 02.09., ueberschrieben mit „Aktuelles Projekt“) — eine falsche Akte ist schlimmer als keine, weil das Modell keinen Anlass hat, an ihr zu zweifeln. **Der Denkprozess ist einstellbar — aus / niedrig / hoch / Standard** (`DENKSTUFEN`, Auswahlfeld in der Startmaske, `denken` an `/agent/start`). Die vier Namen sind die des Menschen, die Uebersetzung steht an EINER Stelle; zwei Leitern nebeneinander waeren die naechste Abschrift. **Die beiden Koepfe koennen es auf VERSCHIEDENEN Wegen, und der Unterschied ist gemessen:** `pi --help` kennt `--thinking off|minimal|low|medium|high|xhigh|max` — eine Flagge, die nur fuer diesen Lauf gilt und nichts hinterlaesst. **`hermes acp` kennt KEIN `--reasoning`** (das gibt es nur am Hauptbefehl, gemessen an `hermes acp --help`), und `hermes_constants.resolve_reasoning_config` liest ausschliesslich `cfg["agent"]["reasoning_effort"]` aus der `config.yaml`, die `HERMES_HOME` meint — kein Schluessel auf oberster Ebene, keine Umgebungsvariable. Genau die ist im Projekt aber auf die GEMEINSAME **verlinkt**; hineinzuschreiben verstellte den Terminalkopf mit und bliebe nach dem Lauf stehen. Deshalb wird sie bei gewaehlter Stufe zur **echten Datei, die bei JEDEM Start neu aus der gemeinsamen erzeugt wird** — dieselbe Disziplin, mit der `AGENTS.projekt.md` gegen Drift gebaut ist: eine Kopie, die nie altert, weil sie nie alt wird. Bei „Standard" bleibt es bei der Verknuepfung, und zurueck auf Standard wird sie wiederhergestellt. Geschrieben wird **ohne PyYAML** (das liegt nicht im venv, und eine Abhaengigkeit fuer eine Zeile waere der falsche Preis): `yaml_agent_setzen` setzt EINEN Schluessel im `agent:`-Block, behaelt die Einrueckung, zaehlt einen auskommentierten Schluessel NICHT als vorhanden und faellt bei Flow-Mappings aufs Anhaengen zurueck. **Was die Stufe wirklich bewirkt, ist durch PIs rpc-Strom gemessen** — also ueber den Weg, den der Kopf geht, nicht an der rohen Ollama-API: `off` 2173 Denkzeichen / 26,6 s · `low` 3620 / 29,9 s · `high` 13443 / 99,7 s. Die Leiter wirkt also stark (hoch = 6x Denken, 4x Zeit), aber **„aus" ist bei PI+Ollama keine Abschaltung, sondern die kleinste Stufe** — an der rohen API schaltet `think:false` das Denken wirklich auf null (0 Zeichen, 0,7 s statt 1,5 s), ueber PI nicht. Der Hinweistext in der Maske sagt genau das; ein erster Entwurf trug dort die API-Zahlen und waere irrefuehrend gewesen. **Nebenbefund, der dazu gehoert:** mit Denken verbraucht das Modell sein Antwortbudget zuerst im Denken — an der rohen API mit `num_predict=1200` kamen bei `low` und `high` gemessen **0 Zeichen Antwort** heraus, bei `think:false` 3886. Das ist derselbe Grund, aus dem `ema_report.call_ollama` seit jeher `think:false` sendet. Die Terminalwege brauchten nichts: `start_agent.sh` und `start_hermes.sh` sammeln unbekannte Flaggen in `ARGS` und reichen sie durch, also gehen `--thinking high` bzw. `--reasoning high` dort schon. Der Studio-Kopf erbt es von `PiKopf`. Die Stufe steht im Startereignis und damit im `protokoll_*.md` des Laufs — womit gedacht wurde, gehoert zum Ergebnis. Test: `test_agent.py`. **Zweiter Agentenkopf: Hermes (Reiter 🪽 neben 🤖 PI).** `class Lauf` ist in eine Basis `Kopf` (Ringpuffer, Mitschrift, Bilder, Zwischenruf, Protokoll, Aufnahme, Projektakte — alles Gemeinsame) und zwei Adapter `PiKopf`/`HermesKopf` zerlegt; `LAUF`/`HERMES`/`KOEPFE`/`kopf(name)` sind die Auswahl, und **die fuenfzehn `/agent/…`-Routen bleiben EINE Menge**: `server._agent_kopf()` liest `?kopf=` (bei POST auch aus dem Rumpf) und faellt ohne Angabe auf PI zurueck, sodass eine aeltere Seite unveraendert weiterlaeuft. Ebenso ist `ema_agent.html` **eine** Seite fuer beide (`const KOPF` aus der Abfragezeichenkette, jede Adresse durch `K()`) — ein zweites HTML waere die Kopie, die beim ersten Fehlerbericht auseinanderlaeuft, genau der Grund, aus dem PI und Hermes schon jetzt EINE `SKILL.md` lesen. Angesprochen wird **`hermes acp`** (Agent Client Protocol), nicht `hermes serve` — dessen eigene Weboberflaeche auf :9119 waere ein Fremdkoerper ohne Stoppuhr, Protokoll, Projektbilder und Aufnahme. Die Form ist gemessen: zeilengetrenntes **JSON-RPC 2.0 auf stdout, Protokoll auf stderr** (deshalb `_stderr_ziel()` = `PIPE` und **nicht** `STDOUT` wie bei PI — eine eingemischte Logzeile zerlegt den Strom; durchgereicht wird nur `[ERROR]`/`[CRITICAL]`/`Traceback`, sonst stuenden 55 Zeilen Plugin-Registrierung je Start links); `initialize` → `session/new {cwd, mcpServers}` → `result.sessionId`; danach `session/prompt`, und **die JSON-RPC-Antwort auf genau diese Anfrage IST das Zugende** (`stopReason`) — der eine Punkt, an dem ACP besser ist als PIs zu erratendes `agent_settled`. Waehrend des Zuges `session/update` mit `agent_thought_chunk`/`agent_message_chunk`/`tool_call`/`tool_call_update` (nur `completed`/`failed`, nicht der Zwischenstand)/`usage_update`. **Freigabe-Rueckfragen wurden 0 mal gemessen**, werden aber beantwortet UND in den Strom geschrieben: unbeantwortet stuende der Zug still hinter einer weiterlaufenden Uhr, und stillschweigend zuzustimmen waere schlimmer als die Rueckfrage. Zwei Dinge sind bei Hermes anders und stehen deshalb in der Maske: **kein `--append-system-prompt`** (`KANN_SYSTEMZUSATZ=False`) — der stehende Stand geht ueber `AGENTS.projekt.md` hinein, das der Kopf ohnehin bei jedem Start schreibt; und **sein Gedaechtnis haengt am Projekt** — `HERMES_HOME` wird von `hermes acp` befolgt (gemessen: `state.db` landet dort), also zeigt `_umfeld()` es auf `<projekt>/_agent/hermes` (geteiltes `config.yaml`/`.env`/`skills` **verlinkt, nicht kopiert**, sonst arbeiten Terminal- und Browserkopf nach zwei plausibel aussehenden Konfigurationen). Daraus folgt `projektpflicht` in `/agent/auswahl`: die Startmaske laesst Hermes ohne Projektwahl nicht los — im falschen Projekt serviert er das an einer anderen Auslegung Gelernte als Tatsache, derselbe Grund, aus dem `start_hermes.sh` am TTY zuerst die Projektmatrix zeigt. `sitzungen()` bleibt bei Hermes **leer**: seine Sitzungen liegen in der `state.db`, ACP kann sie erst NACH dem Start auflisten, und ein nachgebautes fremdes SQL-Schema waere die naechste stille Kopie. **Durchsichtige Feldbilder in der rechten Spalte:** die Bilder aus `ema_feldbild` tragen einen Alphakanal, und `.kachel img` hat den Vorgabegrund **weiss** — ohne eigene Regel laege die Durchsicht also auf Weiss und waere nicht als solche zu erkennen. `kachelBild` haengt Dateien mit dem Praefix `feld_` die Klasse `durchsicht` an (dunkles Schachbrett). Gilt fuer beide Koepfe, weil beide dieselbe Seite bedienen und die Bilder ueber denselben Bilderpfad (`<projekt>/charts`, Aenderungszeit) hereinkommen. Test: `test_agent.py`  **Frühere Läufe sind jetzt wieder aufrufbar.** `sichern()` schrieb nach jedem Zug ein `protokoll_*.md` und eine `ereignisse_*.jsonl` — gelesen hat das nie jemand, es gab keinen Weg zurück; für den, der davorsitzt, ist „geschrieben, aber unerreichbar" dasselbe wie „nicht gespeichert". `laeufe_liste()`/`lauf_lesen()` + die Routen `/agent/laeufe`, `/agent/lauf` und `/agent/steckbrief` sind der Gegenweg; die Seite spielt einen alten Lauf über **dieselben** Zeichenfunktionen wie den lebenden ab (`SPALTEN` schaltet nur das Ziel um). Die Übersicht liest eine Mitschrift dabei **nie ganz ein** (`_lauf_ueberblick`, ein Durchgang, JSON nur für die Zeilen, die sich als interessant ausweisen) — eine gemessene Mitschrift ist 9,4 MB mit 140.872 Ereignissen; `lauf_lesen` deckelt auf `RINGGROESSE` und schneidet **vorn** ab. **`_rechnungen_melden()`** ist der zweite, belastbarere Weg in die rechte Spalte: was `cae_cli.py` nach `<projekt>/rechnungen/` legt, wird über die Änderungszeit gefunden — genau wie die Bilder in `charts/`. Das musste sein, weil der erste Weg bei Hermes **gemessen unterbrochen** ist: ruft das Modell in EINEM Zug mehrere Werkzeuge auf, schickt `hermes acp` v0.20.5 je ein `tool_call`, aber **kein einziges** `tool_call_update` — nachgestellt mit einem eigenen ACP-Klienten (1 Werkzeug → Aufruf + Update; 3 Werkzeuge → 3 Aufrufe, 0 Updates). Der Lauf vom 04.09. zeigt es: 1.562 Ereignisse, 3 Werkzeugaufrufe, 0 Ergebnisse, rechte Spalte leer. `_offene_werkzeuge_abschliessen()` benennt am Zugende, was ohne Rückmeldung blieb, statt zu schweigen oder etwas zu erfinden. **Und die Ergebnisse werden nachgelesen, statt nur beschriftet zu werden:** verloren sind sie nämlich nicht — Hermes schreibt jedes Werkzeugergebnis in `<HERMES_HOME>/state.db` (`messages`, `role='tool'`, Inhalt als JSON mit `output` bzw. `content`), denn das Modell bekommt sie ja auch. `_ergebnisse_nachlesen()` holt am Zugende genau die dieses Zuges von dort (**nur lesend**, `mode=ro` + Zeitgrenze — die Datei gehört dem laufenden Hermes) und füllt die stummen Kacheln mit dem **echten** Text; `_offene_werkzeuge_abschliessen()` fällt nur noch dort auf den ehrlichen Platzhalter zurück, wo auch die Ablage nichts hergibt. Zugeordnet wird der **Reihe nach**, nicht über die Kennung: ACP vergibt `tc-…`, die Ablage `call_…` — zwei Nummernkreise. `_zerlegen()` liest dabei mit `JSONDecoder.raw_decode` in der Schleife, weil eine Zeile gemessen **mehrere** hintereinander geschriebene Objekte tragen kann (ein einzelnes `json.loads` scheitert mit „Extra data“) und weil hinter dem letzten Objekt Modellkontext stehen kann (`[Subdirectory context discovered: …]` samt ganzer CLAUDE.md, gemessen 8.109 Zeichen) — der gehört nicht in die Ergebniskachel, wird aber als Zeile gemeldet statt verschluckt. **Tempo:** `HermesKopf.tempo()` liest `session_model_usage` (exakte `output_tokens` je Sitzung, ohne den `title_generation`-Eintrag) und bildet aus zwei Abfragen die **gemessene** Rate; `Kopf.tempo()` gibt nichts zurück, weil sich aus einem Textstrom Zeichen zählen lassen, aber keine Token — die Seite zeigt dann ihr Zeichentempo und schreibt „Z/s“ daran. **Ein Zug, der nie endet, ist jetzt sichtbar und lösbar.** Gemessen am 04.09.: Hermes schickte auf `session/prompt` keine Antwort mehr — kein Text, kein Werkzeug, kein Fehler. `beschaeftigt` blieb damit stehen, `fragen()` wies jede weitere Eingabe mit „Der Agent arbeitet noch“ ab, und der einzige Ausweg war, den ganzen Lauf zu beenden und die Sitzung zu verlieren; die Zustandspille behauptete unbeirrt „arbeitet“, während die Arbeitsleiste daneben korrekt „nichts läuft“ zeigte. `Kopf.letztes_ts`/`zug_ab` führen jetzt mit, wann zuletzt IRGENDETWAS kam — ohne diese Marke ist ein hängender Zug von einem langen nicht zu unterscheiden — und `zustand()` liefert `still_sek`, `zug_sek` und `prozess_lebt` an die Leiste. Die zeigt eine eigene **Agent**-Leuchte („arbeitet · 0:42“ bzw. bernsteinfarben „still seit 8:13“), korrigiert die Pille oben und bietet ab 450 s `POST /agent/freigeben` an. Die Schwellen (120 s Warnung, 450 s Freigabe) liegen bewusst **über** der Dauer eines langen Werkzeugaufrufs — während `sleep 180 && status` kommt zu Recht nichts, und Hermes lässt ein Werkzeug bis 420 s laufen. `freigeben()` beendet den Agenten **nicht**: der Prozess läuft weiter, eine später doch noch eintreffende Antwort erscheint im Strom, und genau das steht auch im Verlauf — ein stiller Neustart des Zuges wäre schlimmer, dann liefen zwei nebeneinander, ohne dass es jemand weiß. **Die Bildschirmaufnahme richtet sich jetzt nach der ERGEBNISSPALTE, nicht danach, ob der Server rechnet.** Die alte Regel („Server rechnet → Pause“, begründet damit, am Bild ändere sich dann nichts außer einem Fortschrittsbalken) ist gemessen falsch: im Lauf vom 04.09. kamen **mitten im Rechenlauf fünf Bilder** in die rechte Spalte (Querschnitt, Seitenansicht, Luftspalt, Feldbild, Feld unter Last) — angehalten wurde also genau während der einzigen Momente, die aufzuheben sich lohnt. Jetzt setzt jede Kachel, jedes Bild, jeder Auftrag und jedes Scrollen in der Ergebnisspalte die Uhr zurück (`rekTaetig`), und beim Erscheinen wird SOFORT fortgesetzt statt erst beim nächsten Wächterlauf. **Wichtiger noch: es wird mitgeschrieben, WANN was geschah.** `Aufnahme.marke()` sammelt je Ereignis die **Videosekunde** (verstrichene Zeit *minus* Pausen — die Wanduhr läge mit jeder Pause weiter daneben; der Browser schickt seine eigene Rechnung mit, weil er den `MediaRecorder` genauer kennt), und `beenden()` legt daneben eine `.marken.tsv` **und ein ausführbares `.schnitt.sh`**: benachbarte Marken werden zu einem Stück verschmolzen (`VOR_S`/`NACH_S`/`VERSCHMELZEN_S`), jedes Stück wird geschnitten und alle werden aneinandergehängt. Bewusst **neu kodiert statt `-c copy`** — kopierend schnitte ffmpeg an Schlüsselbildern und träfe den Moment um Sekunden daneben. Damit ist die Pause nur noch Platzersparnis und kein Zwang mehr: die Schaltfläche „Leerlauf auslassen“ schaltet sie ab, dann wird alles aufgenommen und hinterher geschnitten. **Zweiter gemessener ACP-Fehler, gleicher Tag:** `skill_view("cae-orchestrator")` antwortet in `hermes acp` v0.20.5 mit *Skill 'cae-orchestrator' not found* — **obwohl** `hermes skills list` ihn zeigt (Quelle `local`, Trust `local`, `skills.trusted_project_dirs` enthält den Repopfad) und derselbe Aufruf in einem gewöhnlichen Python-Prozess mit demselben `HERMES_HOME` und demselben Arbeitsverzeichnis gelingt; mit einem eigenen ACP-Klienten nachgestellt, also nicht vom Server verursacht. Die Ursache liegt in Hermes' Projekt-Skill-Auflösung (`agent.skill_utils.get_project_skills_dirs` -> `find_project_root` über `TERMINAL_CWD`/`Path.cwd()`, während die ACP-Sitzung ihren cwd in einer eigenen Contextvar hält, `agent/runtime_cwd.py`) und wird hier nicht geflickt. Statt dessen nennt **jede** Startunterlage den Dateipfad `.agents/skills/cae-orchestrator/SKILL.md` ausdrücklich (`AGENTS.md`, die im Browser erzeugte `AGENTS.projekt.md`, beide Startskripte): eine Datei lesen statt suchen — ein Agent, der den Skill für abwesend hält, rechnet ohne Verben, Laufzeiten, Exit-Codes und Fallen los.|
| `ema_studio.html` / `agent_gemein.js` (Routen `/studio…`, Reiter 📱 Studio) | **Dritter Agentenkopf, einspaltig, vom Handy aus bedienbar.** `ema_agent.html` ist fuer den Schreibtisch und die Bildschirmaufnahme gebaut: zwei grosse Spalten und ein Ziehgriff auf Mausereignissen — auf einem Handy unbedienbar, derselbe Befund, aus dem `ema_mobil.py` entstand. Diese Seite zeigt **alles untereinander in der Reihenfolge des Geschehens** (Frage rechts, Antwort links, Denken einklappbar, Werkzeugzeile schmal, Ergebnis und Bild als Karte **an ihrer Stelle im Verlauf** statt in einer zweiten Spalte — zwei Spalten sind zwei Zeitachsen, und man liest unwillkuerlich die eine als Fortsetzung der anderen). **Geteilt wird nicht das Layout, sondern das Verhalten:** `agent_gemein.js` traegt Kopfklammer (`KOPF`/`K`/`$`/`esc`), Ereignisstrom (`lauschen`/`verarbeiten`/`verarbeitenStumm`), Stoppuhr, Mitlaufen und Gleiten, Archiv, Arbeitsabfrage und die **ganze Bildschirmaufnahme** — rund 350 Zeilen, die als zweite Abschrift beim ersten Fehlerbericht auseinandergelaufen waeren (dasselbe Argument, aus dem alle Koepfe EINE `SKILL.md` lesen). Die Grenze ist ein Kriterium und keine Geschmacksfrage: **in der geteilten Datei steht, was VERHALTEN ist; in der Seite, was AUSSEHEN ist.** Sie ruft deshalb in die Seite zurueck (`links`, `kachelText`, `kachelBild`, `zustand`, `denkenUm`, `ordnerZeigen`, `gesichertZeigen`, `arbeitZeigen`), und beide Seiten sind klassische Skripte im selben globalen Gueltigkeitsbereich — eine zweite `let`-Deklaration desselben Namens ist ein Syntaxfehler, den keine Datei fuer sich zeigt, weshalb `test_studio.py` `node --check` ueber **geteilte Datei plus Seite zusammen** laufen laesst. **Der Kopf** (`ema_agent.StudioKopf`, eine Zeile in `KOEPFE`) ist ein eigener `pi`-Prozess mit eigenem Ring, eigener Sitzung und eigener Mitschrift: er steht ausdruecklich NEBEN einem rechnenden PI. Zwei Dinge fallen erst durch dieses Nebeneinander auf und sind hier behoben bzw. benannt: **`AGENTS.projekt.md` wird nicht mehr ueberschrieben, solange ein anderer Kopf laeuft** (der zweite Kopf taeuschte dem ersten sonst beim naechsten Lesen ein fremdes Projekt vor; sein eigener Stand reist ohnehin ueber `--append-system-prompt`, und wo das nicht geht, wird es **gesagt** statt verschwiegen), und **`VIDEO` ist EINE Aufnahme je Server** und nicht je Kopf — die Seite meldet ehrlich, dass ein anderer Kopf schon aufnimmt. **Zugang:** als einzige Agentenseite steht sie absichtlich im Heimnetz und traegt deshalb die Tokenwache des Handy-Pfads (`ema_mobil.token`, dasselbe Geheimnis — ein QR deckt beide Wege): `/studio` und `/agent/…?kopf=studio` sind von `127.0.0.1` offen und von aussen nur mit `?t=`; `pi`/`hermes` bleiben unberuehrt, sie waren nie von aussen gemeint. Die Wache sitzt in **einem** `before_request` und nicht an fuenfzehn Routen — die naechste neue Route waere sonst die, die jemand vergisst. Abgewiesen wird die SEITE mit einer Seite (Eingabefeld fuers Token), nicht mit einem JSON: der Startbildschirm-Eintrag (`mobil/studio.webmanifest`) startet unter `/studio` ohne `?t=`, denn ein Token gehoert nicht in ein Manifest, das jeder lesen kann. `/studio/zugang` ist schaerfer — **nur localhost**, sonst holte sich jeder im WLAN das Token ab —, liefert URL, LAN-Adresse und die **QR-Modulmatrix** (`ema_mobil.qr_matrix`, `segno`/`qrcode` wenn vorhanden, sonst `None` und die Adresse im Klartext: ein halb richtiger QR scannt scheinbar und fuehrt ins Leere). **Hochformat mit fester Aufloesung — nur hier.** Ein Reel und ein Short sind 1080x1920; nimmt man ein Fenster in Fensterform auf, entsteht ein 16:9-Video, aus dem der Hochkantausschnitt erst hinterher geschnitten wird — und dann entscheidet sich **beim Schneiden**, was im Bild ist. Deshalb steht der Verlauf in einer Buehne von genau dieser Pixelgroesse (`FORMATE`: 9:16 als Vorgabe, dazu 4:5, 1:1 und „frei"), die per `transform: scale()` ins Fenster passt: die Anordnung ist damit unabhaengig von der Fenstergroesse, und der Zuschnitt steht **vor** der Aufnahme fest. Skaliert wird die ganze Buehne und nicht ihre Schriftgroessen — sonst saehe jede Aufnahme anders aus; die feste Buehne hat deshalb einen **eigenen Schriftsatz** (`#buehne.fest …`, rund doppelt so gross wie eine Bedienoberflaeche, weil ein Reel auf einem Telefon gelesen wird). Unter `FEST_AB_PX`=760 entfaellt das: dort IST der Schirm schon hochkant, und die Seite sagt das (`📱 fuellt den Schirm`), statt ein wirkungsloses Auswahlfeld stehen zu lassen. **Was wirklich aufgenommen wird, steht daneben:** die Buehne ist 1080x1920 gross, sitzt aber verkleinert auf dem Schirm, und aufgenommen werden dessen Bildpunkte — ein 1000 px hohes Fenster liefert gemessen 527x937, die der Zuschnitt auf 1920 hochrechnet. Die Pille sagt das mit Zahl und Warnfarbe, statt dass hinterher jemand raetselt, warum das Reel weich aussieht. **Und der Zuschnitt wird nicht geraten:** beim Aufnahmestart schreibt die Seite ueber den Rueckruf `nachAufnahmeStart` eine Marke mit Buehnenrechteck, Fenster- und Aufnahmegroesse; `ema_beitrag.zuschnitt` macht daraus `crop=…,scale=1080:1920` — **nur** wenn sich Aufnahme und Fenster in beiden Achsen im selben Verhaeltnis abbilden (Reiter- oder Fensteraufnahme). Wurde der ganze Bildschirm aufgenommen, sitzt das Fenster irgendwo darin, das laesst sich nicht wissen — dann steht keine Zuschnittzeile da, sondern der Satz, warum nicht. **Der gelbe Werkzeugkasten bricht um, statt hinauszulaufen:** `white-space:pre` mit `overflow-x:auto` in einem Flex-Element mit `min-width:auto` durfte nicht schmaler werden als sein Inhalt und lief rechts hinaus statt zu scrollen — der Befehl war abgeschnitten, und in einer Aufnahme ist das endgueltig, weil dort niemand nachscrollt. **Die Ergebniskachel geht den umgekehrten Weg: sie behaelt ihre SPALTEN und passt die SCHRIFT an.** Umbrechen zerstoert eine Kennwertetabelle, scrollen versteckt sie, ein Hoehendeckel schnitt mitten hinein (gemessen: der Steckbrief war nach `safety_factor_fem` zu Ende). `preAnpassen` misst deshalb die Zeichenbreite der Schrift **einmal am DOM** (statt 0,6 em zu raten) und rechnet den Schriftgrad so, dass die **ausgerichteten** Zeilen hineinpassen — erkannt an der Spaltenluecke (zwei oder mehr Leerzeichen mitten in der Zeile, ein Tabulator, oder eine Zeile, die mit einem Rohrzeichen anfaengt und aufhoert). Drei Anlaeufe waren noetig, jeder wegen einer Messung: das 90. Hundertstel ueber ALLE Zeilen fiel auf die 228 Zeichen Prosa der `welle`-Ausgabe herein und drueckte die Kachel auf den Boden; ein blosses `includes('|')` hielt denselben Satz fuer eine Tabelle, weil `(|B| p95 …)` darin steht; und ohne ein Zeichen Luft brach eine 94 Zeichen breite Tabelle um, weil nach dem Abrunden 93 passten. Prosa DARF umbrechen — gemeldet wird nur, wenn die ausgerichteten Zeilen selbst nicht passen (der `paarvergleich` ist gemessen 418 Zeichen breit und passt in kein Hochformat; dann steht das an der Kachel statt einer stillen Umbruchwueste). **Und die Antwort wird als Markdown gesetzt** (`markdown()` zwischen `<<MD-START>>`/`<<MD-END>>`, von `test_studio.py` mit `node` gegen feste Beispiele geprueft — dasselbe Verfahren wie beim JS-Spiegel in `test_topology.py`): Ueberschriften, Fett/Kursiv, Code, Listen, Zitate, Trennlinien und **Tabellen**. Die Tabelle ist der eigentliche Grund: eine Feldanalyse IST eine Tabelle, und `| B_gap | 0,799 T |` als Rohtext ist keine. Vier Dinge sind daran nicht beliebig: zerlegt wird der **rohe** Text und `esc()` laeuft pro Stueck (erst escapen, dann zerlegen, hiesse `>` ist `&gt;` und ein Zitat kein Zitat mehr); ein weicher Zeilenumbruch des Modells wird zum **Leerzeichen** und nicht zu `<br>` (sonst franst die rechte Kante dort aus, wo das Modell bei achtzig Zeichen umbrach) waehrend zwei Leerzeichen am Zeilenende weiter ein harter Umbruch sind; Kursiv nur an Wortgrenzen, sonst zerlegt `em_field_load.png` sich selbst; und gesetzt wird je **Bild** (`requestAnimationFrame`) statt je Delta — PI schickt fuer eine Antwort gut hundert davon. Der ROHE Text bleibt dafuer an der Blase (`dataset.roh`) haengen: aus gerendertem HTML laesst er sich nicht zurueckgewinnen. `agent_gemein.js` ruft dafuer den Rueckruf `blaseFuellen`; wo es ihn nicht gibt (`ema_agent.html`), bleibt es beim Klartext. **Die Fussleiste ist zwei Zeilen hoch und zeigt, wer gerade was macht.** Sie hatte vier: Arbeitsanzeige, Zwischenrufhinweis, Eingabe — und eine vierte nur fuer den Ablagepfad, der als abgeschnittene Zeichenkette dauerhaft Platz belegte fuer eine Angabe, die man einmal am Tag braucht. Der Pfad sitzt jetzt am Sicherungsknopf (`title`, immer abrufbar) und erscheint bei einer Sicherung **von Hand** als `sys`-Zeile im Verlauf, wo er chronologisch hingehoert; `#p_gesichert` gibt es nicht mehr, und `agent_gemein.js` fasst kein seitenspezifisches Element mehr an (`sichernJetzt` meldet Erfolg UND Fehler ueber `gesichertZeigen`, mit `hand: true` fuer die Sicherung von Hand). Das Haekchen „Denken zeigen" zog an das rechte Ende der Arbeitszeile — damit faellt die vierte Zeile weg und die Eingabezeile bekommt die volle Breite fuer 📣, Feld und ➤. Statt einer einzeiligen Zeichenkette mit Auslassungspunkten steht dort jetzt dieselbe **Lampenzeile** wie am Schreibtisch, `lampe(an, symbol, kopf, text, klasse)` mit gleicher Signatur in beiden Seiten: 🤖 Agent (immer, solange der Kopf laeuft — `arbeitet · m:ss` / `wartet auf dich` / bernstein `still seit …` mit `🔓 Sperre loesen` ab `STILL_FREIGABE`), dazu ⚙ Rechnung, 🌐 Recherche, 🧮 Loeser, 🖥 GPU, 🧠 Modell, ⚡ Tempo und 🔧 Werkzeug (s. `ema_werkzeugstand.py`) — hier **nur wenn taetig**, waehrend am Schreibtisch alle stehen: dort hat die Spalte Platz und eine dunkle Lampe ist selbst eine Aussage, hier ist die Buehne 1080 px breit. Es gibt weiterhin **keine Lampe „Modell denkt"** — Ollama meldet ueber `/api/ps` nur, was GELADEN ist. Auf der festen Buehne haben die Lampen ihre eigene Groesse (`#buehne.fest .lampe`); unter `FEST_AB_PX` bleiben sie **eine wischbare Zeile** (`flex-wrap:nowrap` + `overflow-x:auto`) — umbrechend nahmen sie am Handy gemessen sechs Zeilen und schoben das Gespraech aus dem Bild, und dort sass die Leiste vorher richtig. **Bilder haben ihre Groesse zurueck.** Gemeldet als „die Bilder werden nicht dargestellt, da sind nur Linien“ — und genau das war es: `#strom` ist eine SPALTEN-Flexbox, deren Kinder per Vorgabe `flex-shrink:1` tragen. Sobald der Verlauf laenger wird als die Buehne, staucht der Browser die Kacheln in der Hoehe, statt zu scrollen, und ein `<img>` mit `height:auto` hat keine feste Inhaltshoehe, die das aufhielte: es wird zuerst und am staerksten zusammengedrueckt, uebrig bleibt ein weisser Strich. `#strom > *{flex:0 0 auto}` — nichts im Strom schrumpft, gescrollt wird, dafuer ist `overflow-y` da. Die Schreibtischseite war nie betroffen, ihr `.strom` ist ein gewoehnlicher Block; das ist zugleich die Gegenprobe zur Ursache. Dazu die Groesse selbst: **die Kachel gibt die BREITE vor, die Hoehe folgt dem Seitenverhaeltnis** (`width:100%;height:auto`) — beides ist noetig, `width:100%` allein liess ein 700 px breites Diagramm auf der 1080-px-Buehne klein und verloren stehen, `height:auto` allein liesse es verzerren, sobald der Hoehendeckel der Buehne greift; der traegt deshalb `object-fit:contain`. Ein Bild, das NICHT kommt, war von einem gestauchten nicht zu unterscheiden (beides ein weisser Strich) und sagt jetzt, was fehlt. **Und die fertige Aufnahme ist abspielbar:** sie endete bisher mit zwei Zeilen, die den PFAD nannten — dieselbe Lage wie bei den Agentenlaeufen vor `/agent/laeufe`, und am Handy gibt es kein Dateisystem zum Nachsehen. `agent_gemein.js` ruft beim Ende den (optionalen) Rueckruf `nachAufnahmeEnde(a)`, beide Seiten haengen daraus eine Kachel mit `<video controls>` in den Verlauf, und `GET /agent/video/datei/<name>` liefert sie aus `VIDEO_ORDNER` — auf `_safe_name` und die zwei Endungen begrenzt, die der Recorder ueberhaupt schreibt, und mit `conditional=True`, damit man springen kann, ohne 80 MB vorher zu laden. Bild- und Videoadresse gehen durch `K()`: vom Handy aus tragen sie damit Kopf und Token wie jede andere Adresse. **Aufgenommen wird die BUEHNE, nicht der Reiter.** Ein Reiter-Mitschnitt ist ein Bild in Fensterform, in dem die Buehne irgendwo sitzt — der Hochkantausschnitt entstuende erst hinterher beim Schneiden, und bis dahin weiss niemand genau, was im Bild landet. Der Browser kann das selbst: `CropTarget.fromElement($('buehne'))` + `track.cropTo` (Region Capture) schneidet eine SELBSTaufnahme auf ein DOM-Element zu. Zwei Bedingungen haengen daran und stehen deshalb in der Seite: die Aufnahme muss der eigene Reiter sein (`aufnahmeOptionen()` setzt `preferCurrentTab` + `selfBrowserSurface`, und `surfaceSwitching:'exclude'`, weil ein Wechsel den Zuschnitt ungueltig machte), und der Zuschnitt muss VOR dem ersten aufgezeichneten Bild stehen — `agent_gemein.js` **wartet** deshalb auf `nachAufnahmeStart` und tut es zwischen `new MediaRecorder` und `REK.start()`: davor gaebe es keinen Recorder, dessen Marke `rekTaetig` schreiben koennte, danach laegen die ersten Sekunden ungeschnitten in der Datei. Was das NICHT aendert, ist die Aufloesung: zugeschnitten wird in den Bildpunkten, die der Reiter wirklich hat — eine auf 527x937 verkleinerte Buehne liefert 527x937, und die Pille sagt das weiterhin. Die Marke heisst dann `Bereichsaufnahme der Buehne` statt der Rechteckangabe, und `ema_beitrag.zuschnitt_lage` unterscheidet daraufhin **drei** Faelle (`fertig` · `berechnet` · `unbekannt`): schon zugeschnitten ist etwas anderes als „die Lage ist unbekannt“, auch wenn in beiden Faellen keine ffmpeg-Zeile herauskommt — sonst stuende unter einem perfekt zugeschnittenen Reel „ohne Zuschnitt“. Kann der Browser es nicht, sagt die Seite es und faellt auf den gerechneten Zuschnitt zurueck. **Zwei Verfahren, in dieser Reihenfolge:** `restrictTo` (Element Capture) nimmt NUR den Teilbaum auf — was darueber liegt, ist nicht im Bild; `cropTo` (Region Capture) schneidet das Reiterbild auf das Rechteck zu und laesst Verdeckendes drin. Also erst das schaerfere. Gepruefte wird an der SPUR und nicht an `MediaStreamTrack.prototype`: beide Methoden haengen an `BrowserCaptureMediaStreamTrack`, das nur eine Selbstaufnahme liefert — gemessen (Chromium 152) ist `MediaStreamTrack.prototype.cropTo` `undefined`, waehrend die Spur die Methode sehr wohl hat. **Und die Seite merkt, wenn sie veraltet ist:** ein Agentenlauf dauert Stunden, die Seite bleibt so lange offen, und eine Aenderung an ihr wirkt erst nach dem Neuladen. Gemessen am 08.09.2026: die Bereichsaufnahme lag um 10:19 auf der Platte, der Mitschnitt um 10:27 nahm trotzdem den ganzen Reiter auf, weil der Reiter seit 09:44 offen war — die Marke traegt die alte Rechteckform, und keine der beiden neuen Zeilen steht daneben. Nichts sagte es, und die Suche ging in den Code statt auf „Strg+R". `ema_werkzeugstand.stand_oberflaeche()` (eine EIGENE Liste neben `PHYSIK` — ein Knopfumbau darf nicht wie eine Modelaenderung aussehen) reist ueber `/agent/arbeit` mit; `seiteVeraltet()` merkt sich den Stand beim Oeffnen und sagt EINMAL Bescheid, wenn er sich aendert. Neu geladen wird nicht von selbst: mitten in einem Lauf ist das eine Entscheidung des Menschen. Test: `test_studio.py` |
| `ema_beitrag.py` (Verb `beitrag`, Route `/studio/beitrag`) | **Beitragsentwuerfe fuer Instagram und X aus dem, was gerechnet wurde.** Das Material kommt aus `ema_steckbrief.steckbrief` und nicht aus dem Gespraech: der Steckbrief weiss, welche Stufe eine Kennzahl geliefert hat und was fehlt, waehrend ein Beitrag aus dem Chatverlauf jede unterwegs geschaetzte Zahl uebernimmt — und **veroeffentlicht**. Aus demselben Grund wird die Regel „keine Zahl ohne Deckung im Material" nicht nur in den Prompt geschrieben, sondern hinterher **gemessen**: `ungedeckte_zahlen` vergleicht jede Zahl des Entwurfs mit dem Material (Runden ist gedeckt — 0,7994 → 0,80 —, Erfinden nicht; Hashtags und Jahreszahlen bleiben aussen vor) und haengt die Funde als Hinweis an. Eine Bitte an ein Sprachmodell ist keine Zusicherung. Die Formen sind gemessen und nicht dem Modell ueberlassen: X 280 Zeichen je Beitrag, Faden bis 3 — und **zu lang heisst umbrechen, nicht abschneiden** (im ersten echten Lauf endete Teil 1 bei 274 von 280 Zeichen mit „Achtung: Sicherheitskriterien…", also fiel ausgerechnet die Einschraenkung weg, wegen der man den Beitrag lesen sollte); Instagram 2200 Zeichen mit 8–15 Hashtags. Bilder holt `ema_agent.bilder_im_projekt` (**eine** Bildersuche fuer Agentenspalte und Beitrag), Reel-Vorschlaege entstehen aus der `.marken.tsv` der Bildschirmaufnahme ueber `ema_agent.stuecke_aus_marken` — **dieselbe** Gruppierung, aus der auch das `schnitt.sh` entsteht, sonst zeigte der Vorschlag andere Sekunden als geschnitten werden; die fertige `ffmpeg`-Zeile wird gezeigt, nicht ausgefuehrt. Jeder Entwurf traegt eine **Herkunftsfussnote** (welche Kennzahl aus welcher Stufe) und landet unter `<projekt>/beitraege/<zeit>_<kanal>.md` **und** ueber `ema_steckbrief.ablegen` in `rechnungen`/`evolution`, wie die uebrigen entscheidenden Verben. **Veroeffentlicht wird nichts** — kein Netzweg nach draussen, keine Zugangsdaten, kein Knopf dafuer; kopiert wird von Hand. Das Verb gibt es, damit der Agent dasselbe Werkzeug benutzt wie der Mensch: ein Knopf, den nur die Oberflaeche hat, ist fuer ihn nicht vorhanden — und ein Agent, der Beitraege von Hand schreibt, erfindet Zahlen. Test: `test_studio.py` |
| `ema_llm.py` | **Die EINE Stelle, an der dieses Werkzeug ein Sprachmodell anspricht.** Es hatte **sechs** eigene Anlaufstellen -- `ema_report` (`/api/generate`), `ema_chat`, `ema_text2ema`, `ema_optimize`, `ema_design_ai` (alle `/api/chat`) und `ema_experts` (`/api/generate`) --, jede mit eigenem `urllib`-Aufruf und fest auf Ollamas eigenes Protokoll verdrahtet. Ein im Katalog eingetragener API-Anbieter wirkte damit nur fuer die Agentenkoepfe; das Werkzeug selbst sprach weiter ausschliesslich mit Ollama. **Der Entwurf: der Ollama-Rumpf bleibt die INNERE Sprache.** Die sechs Aufrufer bauen ihre Anfrage weiter genauso und lesen die Antwort weiter genauso; `senden(anfrage, art)` schickt sie bei einem **lokalen** Anbieter **Byte fuer Byte unveraendert** weiter und uebersetzt nur fuer eine API, die Antwort zurueck in dieselbe Form. Das ist die risikoaermste Bauform: haette jeder Aufrufer eine neue Schnittstelle bekommen, waere jede der sechs Stellen eine eigene Gelegenheit gewesen, das lokale Verhalten zu verschieben (`test_llm.py` prueft die Byte-Gleichheit ausdruecklich). Uebersetzt wird nach dem `api`-Feld des Katalogs: `openai-completions` → `POST {basis}/chat/completions` mit `Authorization: Bearer`, `format:"json"` → `response_format`, `num_predict` → `max_tokens`; `anthropic` → `POST {basis}/messages` mit `x-api-key` + `anthropic-version: 2023-06-01`, **Systemrolle NEBEN den Nachrichten** und **`max_tokens` zwingend** (ohne beides ein 400). **Was verloren geht, wird GESAGT:** `num_ctx` und `think` gibt es in keiner fremden API -- lokal ist `num_ctx` tragend (65536; ein fehlender Wert klemmt still auf den Modelfile-Wert zurueck), also stehen sie in `letzte_info["verloren"]` statt stillschweigend zu fehlen. **Einbettungen bleiben IMMER lokal**, auch wenn das gewaehlte Modell zu einem API-Anbieter gehoert: keiner der Anbieter hat einen Embeddings-Endpunkt, und eine woanders erzeugte Einbettung waere mit den vorhandenen unvergleichbar -- der `ema_rag`-Index ist ein Vektorraum, kein Textspeicher. (Ein erster Entwurf nahm dort die Basisadresse des API-Anbieters und haette den Text hinausgeschickt; `test_llm.py` hat es gefangen.) Der Schluessel steht in keiner Auskunft, auch nicht in `letzte_info`. Test: `test_llm.py` (28 Pruefungen, ohne Netz und ohne Ollama) |
| `ema_modelle.py` | **Der Modellkatalog — gepflegt statt von Hand gefuehrt.** ``pi`` liest seinen Katalog aus ``~/.pi/agent/models.json`` und **entdeckt nichts von selbst**; gemessen am 11.09.2026 zeigte ``pi --list-models`` **zwei** Modelle, waehrend in Ollama **zwanzig** lagen — und eines der zwei (``qwen3.5:9b``) war gar nicht installiert. Die Startmaske bot damit ein Modell an, an dem der Start scheitert, und verschwieg neunzehn, die es gibt. Eine Handliste neben einer Wirklichkeit, die sich aendert, laeuft auseinander (dieselbe Sorte Fehler wie die zweite Geom-Tabelle in ``server.py`` oder die abgeschriebene ``magnetLegs``), also wird **abgeglichen**: ``sync_ollama`` traegt ein, was in ``/api/tags`` liegt. **Was im Katalog steht und nicht mehr installiert ist, wird BENANNT und nicht geloescht** — der Eintrag kann zu einem Ollama gehoeren, das gerade aus ist, und die Datei gehoert ``pi``, nicht diesem Werkzeug; ``katalog()`` gibt solchen Eintraegen ``da=False``, die Maske zeigt sie gesperrt mit „nicht installiert". Ist Ollama **aus**, aendert der Abgleich **nichts** und im Katalog gilt nichts als fehlend: ohne Auskunft laesst sich das nicht behaupten. ``anbieter_setzen`` traegt einen API-Anbieter ein (``BEKANNT`` fuehrt OpenRouter/OpenAI/Anthropic mit Basisadresse und ``api``-Form, damit niemand eine URL raten muss); ``fremde_modelle`` holt dessen Modellliste zum Auswaehlen — **OpenRouters Liste ist oeffentlich** (gemessen: 443 Modelle ohne Schluessel), man kann also erst sehen, was es gibt, und dann erst einrichten. **Preise stehen dort je TOKEN und im Katalog je MILLION** — ungerechnet uebernommen stuende an einem Modell fuer 5 $/Mio die Angabe „kostet nichts". **Der Schluessel verlaesst das Modul nie:** ``katalog()`` meldet nur ``schluessel: True/False``, geschrieben wird atomar mit **0600** und einer einmaligen ``.bak`` (eine zweite Aenderung ueberschreibt sie nicht — sonst waere sie nach zwei Fehlgriffen wertlos). ``ema_agent.modelle``/``anbieter_fuer`` delegieren hierher; es gibt keine zweite Liste. Test: ``test_modelle.py`` |
| Modellkatalog im Browser (Routen `/agent/anbieter…`, Abschnitt „⚙ Modelle & API-Zugang" in `ema_agent.html`) | **Einrichten, wo es sicher ist.** ``GET /agent/anbieter`` sagt, was im Katalog steht und was sich eintragen liesse (ohne Schluessel, auch aus dem Heimnetz lesbar — die Antwort traegt dann ``lokal: false``); ``POST /agent/anbieter`` (eintragen/entfernen), ``/agent/anbieter/sync`` (Ollama abgleichen) und ``/agent/anbieter/liste`` (Modelle eines Anbieters holen) sind **nur von 127.0.0.1 aus erlaubt** — dieselbe Wache wie ``/m/zugang`` und ``/studio/zugang`` und aus demselben Grund: die Studio-Seite steht absichtlich im Heimnetz, und ein Schluesselfeld, das von dort erreichbar waere, ist keines. Die Sperre sitzt im **Server**; die Maske sperrt ihre Felder zusaetzlich und sagt warum, verlaesst sich aber nicht darauf. Der Abschnitt laedt **erst beim Aufklappen** (er fragt Ollama UND liest den Katalog, das gehoert nicht in jeden Seitenstart), und nach dem Eintragen wird das Schluesselfeld sofort geleert — in der Datei steht er, im DOM hat er nichts verloren. **Ein EIGENER Anbieter laesst sich eintragen** (Eintrag „➕ eigener Anbieter“ in der Auswahl): Name, **Basisadresse**, **API-Form** (``openai-completions`` oder ``anthropic``) und wahlweise Modellkennungen **von Hand**. Die drei hinterlegten Dienste sparen nur das Tippen — gebunden war daran nie etwas, ``anbieter_setzen`` nimmt Adresse und Form ohnehin entgegen und die Route reichte sie schon durch; es fehlte allein der Weg dorthin. Fuer die Modellliste wird ``{basis}/models`` **versucht** (der OpenAI-Standardpfad, den fast jeder kompatible Dienst bedient), und wer ihn nicht hat, traegt seine Kennungen in das Feld daneben — ohne diesen zweiten Weg waere so ein Anbieter gar nicht eintragbar, und die Maske hoerte genau dort auf, wo sie gebraucht wird. ``katAnbieterFelder()`` ist die EINE Stelle, aus der Suchen UND Eintragen ihre Angaben holen; zwei Abschriften waeren beim ersten neuen Feld auseinandergelaufen. **Die Schluesseleingabe gibt es nur auf der Schreibtischseite**, nicht im Studio: die beiden ``modellFuellen``-Fassungen sind ohnehin schon auseinandergelaufen, und eine Abschrift mehr waere die naechste Fehlerquelle. Beide Seiten zeigen dafuer gesperrte Eintraege fuer Modelle mit ``da=false`` |
...
| `ema_werkzeugstand.py` / `BEFUNDE.md` | **Womit ist diese Zahl gerechnet worden — und hat sich das mitten im Lauf geaendert?** Die Agentenkoepfe sind Codieragenten (`pi --mode rpc`, `hermes acp`) mit Schreibrecht im Repo, und im Browserpfad wird keine Freigabe erfragt (`HermesKopf._freigabe` stimmt sogar ausdruecklich zu, weil ein unbeantworteter Zug sonst still stuende). Bei einer **Zielwertsuche** ist eine Aenderung an `ema_asm.py` deshalb der kuerzeste Weg zum Ziel — und der einzige, der die Zahl falsch macht, ohne dass es jemand sieht. **Verhindern laesst sich das nicht:** der Agent laeuft als derselbe Benutzer, ohne Sandkasten; wem eine Datei gehoert, der darf sie schreiben, und ein `chmod` zurueck kostet eine Zeile — ein Schloss, das man von innen aufschliessen kann, ist kein Schloss, sondern eine Behauptung. **Verbergen** laesst es sich sehr wohl verhindern, und das ist die ganze Absicht: `stand()` bildet einen Fingerabdruck ueber die **dreizehn** Module, deren Aenderung eine Kennzahl bewegt (`PHYSIK`, eine ausdrueckliche Liste — ein Glob zoege jede neue Seite mit hinein, und der Stand waere binnen einer Woche Rauschen, das niemand mehr liest), dazu den `git`-Stand und welche davon von HEAD abweichen. Er reist mit: im Kopf jeder abgelegten Rechnung (`ema_steckbrief.ablegen`), an jeder Evolutionsstufe (`ema_projekt.record_run`), als Zeile im Steckbrief — und wenn dort **mehrere** Staende stehen, sagt der Steckbrief ausdruecklich, dass diese Zahlen nicht ohne Weiteres vergleichbar sind. `Kopf.werkzeug()` vergleicht gegen den Stand des Laufstarts (ohne laufenden Kopf gegen `git HEAD`), `/agent/arbeit` traegt ihn ohne neue Route, und beide Agentenseiten haben die Lampe 🔧. `Kopf._werkzeug_pruefen()` haengt an `_rechnungen_melden` — dem Zugende, an dem die Zahlen landen — und meldet **jede Datei genau einmal** als `hinweis` in den Strom, damit es auch im `protokoll_*.md` des Laufs steht. Gelesen wird ueber mtime/Groesse zwischengespeichert, `git` hoechstens alle 30 s und weich (ohne Baum steht dort `None`). Dazu die **Regel im Klartext**, dort wo der Agent sie liest (`SKILL.md` „Was NICHT zu tun ist", `AGENTS.md`, `Kopf.WERKZEUGREGEL` im stehenden Auftrag ALLER drei Koepfe): die Rechenmodule sind der Massstab, nicht der Gegenstand; ist ein Ziel nicht erreichbar, ist „nicht erreichbar, weil …" die richtige und vollstaendige Antwort; haelt man das Werkzeug fuer falsch, schreibt man einen **Befund** nach `cae_orchestrator/BEFUNDE.md` (was beobachtet, wo gemessen, welche Fundstelle) statt still zu reparieren. Die **Zielwertsuche selbst** (`ema_optimize.py`) braucht nichts davon: das Modell schlaegt dort nur Parametervektoren vor, `_clamp` und `_fitness` halten sie im Rahmen (jede unzulaessige Loesung rangiert unter jeder zulaessigen), und der Quelltext ist fuer sie unerreichbar. Gefaehrdet ist der **Agentenpfad**, der von Hand mit `run --set` sucht. Test: `test_werkzeugstand.py` |
| `ema_asm` — das Recht auf „Nein" (`I_Q_MIN_ANTEIL`, `erreichbar`/`grund`) | **Ein begruendetes Nein ist ein Ergebnis, kein Fehlschlag.** Gemessen am 08.09.2026 (`BEFUNDE.md`): ein netzgespeister 230-V-Ventilatorantrieb (24 Nuten, p=1, Rotor Ø 88,6) braucht allein zum Magnetisieren auf 0,80 T einen Strom von **1686 A** gegen eine Umrichtergrenze von **800 A**; es bleibt kein momentbildender Strom. Das Modell rechnete trotzdem weiter und meldete `T_ist = 0,0 Nm` neben **2,8·10²⁰ W** — weil `verluste` das Statorkupfer mit `(I_s/max(i_q, 1e-9))²` skalierte. Eine Division durch fast Null ist kein Ergebnis, sondern die Stelle, an der die Rechnung haette aufhoeren muessen. Jetzt entscheidet `betriebspunkt` zuerst, ob es den Punkt **gibt**: unter `I_Q_MIN_ANTEIL` (ein **Tausendstel** der Stromgrenze, nicht `1e-9` — darunter liegt auch das Moment unter einem Tausendstel dessen, was die Maschine koennte, und jede abgeleitete Groesse waechst um sechs Groessenordnungen) kommt `erreichbar: False` samt Begruendung zurueck und `T_ist_Nm`/`i_q_A`/`schlupf` sind **`None`, nicht 0,0** — eine Null liest sich wie ein gerechnetes Ergebnis, und genau so wurde sie gelesen. `verluste` und `dauermoment` rechnen dann gar nicht erst; `kaefig` faellt nicht mehr still auf den Fertigungsboden von 2 mm zurueck (aus dem der 2-D-Lauf sonst einen breiten, flachen Rotor zeichnet — gemessen Carter 3,2 gegen angenommene 1,15 und 0,29 T Leerlauffeld statt 0,80 T, Moment auf ein Siebtel), sondern meldet `bemessung: "nicht auslegbar"`, und `ema_em2d_harm` weigert sich, daraus ein Netz zu bauen. Den einen Satz schreibt `nicht_erreichbar_text()`; `ema_paarvergleich` weist die Option ab wie eine verletzte Fliehkraftgrenze, `cae_cli feld3d` bricht mit Exit 2 ab. Test: `test_asm_grenze.py` |
| `ema_referenz.GELTUNG` / `geltung_pruefen` / `leistungsband` | **Der Geltungsbereich — die zweite Haelfte der Gewichtung einer Antwort.** Auf die Frage „muessen Antworten gewichtet werden" ist die Antwort hier **nicht** eine Vertrauenszahl (die waere selbst wieder eine erfundene Groesse), sondern die zwei Angaben, die eine Zahl waegbar machen und beide schon halb vorhanden waren: **Herkunft** (welches Verfahren — `ema_db.HERKUNFT`, seit jeher an jedem Kennwert im Steckbrief) und **Geltungsbereich** (liegt der Fall in der Klasse, auf die diese Kette geeicht ist). Es ist — wie `ART_BAND`, dessen Haltung es uebernimmt — **kein Tor**: ausserhalb heisst nicht falsch, sondern „dafuer ist diese Kette nicht geeicht, und keine der gerechneten Auslegungen liegt dort". Drei Pruefungen, jede gemessen statt behauptet: **Umrichter** (steht keiner im Payload, rechnet die Kette mit der Traktionsvorgabe 800 V / 800 A, und JEDE Stromaussage ist dann eine Aussage ueber diesen Deckel), **Speisung** (braucht die Magnetisierung mehr als die Grenze hergibt, ist die Maschine fuer diese Umrichterklasse zu gross — oder gehoert gar nicht an einen Umrichter; **Netzbetrieb 1~230 V ist strukturell nicht modelliert**, und das steht wortwoertlich in `GELTUNG["speisung"]`), und **Leistung** gegen das aus `ema_db` **ausgelesene** Band der tatsaechlich gerechneten Laeufe (gemessen 0,2–419 kW aus 57 Werten; die zwei Nulleintraege bleiben draussen — „nicht gerechnet" ist keine 0-kW-Maschine, und mit ihnen faellt nie ein Fall unten heraus). Ausgewertet an vier Stellen: der Steckbrief druckt einen Abschnitt „Geltungsbereich", `ema_asm`s `grund` benennt ihn, `sicherheit` listet ihn (nur echte Abweichungen — dass kein Umrichter GESETZT ist, ist der Normalfall dieses Bestandes und gehoert nicht in eine Pruefung, die sonst bei fast jedem Projekt mit Exit 1 endete), und `ema_beitrag` traegt ihn in die Aufforderung, damit ein Entwurf ueber eine Maschine ausserhalb der Klasse das dazusagt statt ihre Zahlen als belastbar hinzustellen. Test: `test_asm_grenze.py` |
| `ema_ausreizen.py` (Verb `ausreizen`) | **Der Optimierer ueber `leistung` — die Auslegung ins Gleichgewicht bringen.** Und die tragende Unterscheidung steht im Modulkopf: **Gleichgewicht ist kein Ziel, sondern ein Symptom.** Wer die LEISTUNG maximiert, laesst jede nicht bindende Grenze liegen, wo sie ist — das Joch der gemessenen Maschine steht bei 25 %, und es duenner zu machen bringt kein Watt, die Suche hat also keinen Grund dazu. Maximiert man die **Leistungsdichte** (`--ziel dichte`, Vorgabe), wird aus jeder brachliegenden Grenze *entfernbares Material*, und das Gleichgewicht stellt sich von selbst ein. Ein kuenstlicher Ausgleichsterm („belohne Ausnutzung nahe 100 %") taete das Gegenteil: er koennte eine SCHLECHTERE Maschine vorziehen, die zufaellig gleichmaessig ausgelastet ist. Gesucht wird **deterministisch** (Musterschritt, Schrittweite halbiert sich, wenn eine Runde nichts findet) — kein Sprachmodell in der Schleife, also ergibt derselbe Payload denselben Entwurf, und das Protokoll nennt jeden Griff mit seinem Gewinn. `ema_optimize.FREE_PARAMS`/`_clamp`/`_apply_params` bleiben die eine Quelle; dabei fiel auf, dass **`slotWidthRatio` dort fehlte** — der einzige Hebel, der die Saettigung bewegt, war fuer jede Suche unerreichbar (jetzt drin, 0,25…0,70). **`brachliegend()` ist der eigentliche Ertrag:** fuer jede Grenze unter der Schwelle wird GEMESSEN, welcher freie Parameter sie am staerksten hebt — ein Durchgang fuer alle Grenzen zusammen (2 x Parameterzahl Auswertungen), und die Empfehlung kommt aus derselben Rechenkette wie das Ergebnis statt aus einer Faustregel. Griffe, die die Grenze heben und dabei den Zielwert ruinieren, werden als solche gekennzeichnet. **Der erste echte Lauf war zugleich die schaerfste Pruefung des Modells** und fand prompt ein Loch: er fuhr `shaftD` auf 39,3 mm und `shaftBore` zugleich auf 190,1 mm — jeder Wert fuer sich zulaessig, die Kombination nicht, und `ema_screen` rechnete daraus **−49,05 kg** Wellenmasse, womit der Zielwert um Faktor 74 stieg. Ein Optimierer ist eben kein Verursacher, sondern ein **Messgeraet**: er findet zuverlaessig die schwaechste Stelle. Behoben ueber `ema_optimize._stimmig` (radiale Ordnung im gemeinsamen Bewerter, wie `_baubar` und `_saettigung`) und eine Klemme in `massen_und_kosten`; s. `BEFUNDE.md` 13.09.2026. Der Sieger ist ein **Vorschlag**, kein Ergebnis — gerechnet auf dem schnellen Bewerter, nachzurechnen mit einem Pipelinelauf. Test: `test_ausreizen.py` (35 Pruefungen ohne Loeser; die Suche laeuft gegen eine gestellte Zielfunktion mit BEKANNTEM Optimum, damit ein roter Test sagt, ob die Suche oder die Physik gemeint ist) |
| `ema_umrichter.py` (Verb `umrichter`, Route `POST /umrichter`, Karte ⚡ im Betrieb-Reiter) | **Mehrere Leistungselektroniken — A1..Ak, B1..Bk, C1..Ck statt A, B, C.** Der erste Satz des Modulkopfs ist der wichtigste: **Aufteilen macht die Maschine nicht staerker.** Das Moment haengt an den Amperewindungen, und die liegen durch Nutquerschnitt, Stromdichte und Kuehlung fest — vier Systeme mit je einem Viertel der Leiter liefern dieselbe Durchflutung wie eines mit allen. Gekauft werden vier andere Dinge: kleinere Halbleiter (1/k der Scheinleistung je Modul), **n-1-Betrieb**, verteilte Verlustwaerme und — bei Winkelversatz — Ausloeschung der 5. und 7. Harmonischen. Nur die ersten beiden rechnet das Modul; die Harmonischen braeuchten einen zeitabhaengigen Feldlauf (der FDM ist magnetostatisch), und das steht als Grenze in jeder Ausgabe. **Die Rechnung folgt zwingend aus dem festen Nutquerschnitt:** jedes System bekommt 1/k der Leiter, also `N/k` Windungen → Flussverkettung `psi/k` → **Spannung je Modul E/k**, waehrend `sum(N_j·I_j) = k·(N/k)·I = N·I` bleibt → **Strom je Modul unveraendert**. Von der Maschine aus wirken k Module zu je `V_Modul` also wie EIN Umrichter mit `k·V_Modul` bei demselben Strom: mehr Module kaufen **Spannungsreserve** (und Eckdrehzahl), nicht Strom. Das ist dieselbe Algebra wie die Windungszahlbruecke (`Kt ~ N`, `i ~ 1/N`, `u ~ N`), und deshalb steht die Modulzahl **in `ema_analysis.umrichter`** — der einen Quelle der Grenzen, durch die neun Module hindurchlesen; k=1 laesst jede Altrechnung Ziffer fuer Ziffer gleich (im Test festgenagelt). **`wickelbar()` ist ein Tor wie `rotor_layout_check`**: `slots % 3k`, bei `sektoriert` zusaetzlich eine ganze Polzahl je Sektor, und fuer den Winkelversatz `q ≥ k`. Es sagt **warum nicht und was statt dessen ginge** (36 Nuten: „moeglich waeren k = 2, 3, 4, 6"). Jeder Befund traegt seine **eigene Schwere** — der fehlende Versatz ist ein Hinweis und kein Ausschluss, und ein erster Entwurf faerbte alle Befunde nach dem Gesamturteil, womit ein Hinweis wie ein Nein aussah. **`verschachtelt` und `sektoriert` ergeben dieselben Modulkennwerte** — das wird ausdruecklich gesagt, statt einen Unterschied zu erfinden, den die Rechnung nicht hergibt; sie unterscheiden sich im Fehlerfall (symmetrisch gegen **einseitigen Magnetzug auf die Lager**, hier NICHT gerechnet und als Warnung dabei). `ausfall()` ist der eigentliche Grund fuer solche Antriebe und mit `power_envelope` eine Zeile Arbeit: beides skaliert auf `(k-1)/k`, Spannungsreserve und erreichbare Durchflutung. Zwei **Bilder** nach `charts/` wie beim Feldbild: der Querschnitt mit den Systemen (bei `sektoriert` Farbbloecke, bei `verschachtelt` abwechselnd — man sieht den Unterschied, an dem der Fehlerfall haengt) und die n-1-Kennlinie. Die Karte im Browser **rechnet nicht selbst**, sie ruft `POST /umrichter`: die Wickelregel haengt an der Wicklung, und eine JS-Abschrift waere die naechste `magnetLegs`-Drift. Test: `test_umrichter.py` (54 Pruefungen ohne Loeser — die Wickelregel gegen ausrechenbare Faelle, die Aufteilung gegen die Invariante „die Durchflutung bleibt") |
| `ema_saettigung.py` | **Die vierte Grenze — was das Eisen tragen MUSS, gegen das, was es kann.** Sie war die einzige der vier Auslegungsgrenzen, die gar nicht gerechnet wurde: der FDM ist linear (`MU_R_IRON=500`), `_analytical_Bgap` hat keinen Eisenterm, `B_SAT_IRON` wirkt nur im Anzeigepfad — das Moment waechst im Modell linear mit dem Strom weiter, als gaebe es kein Eisen. **Der naheliegende Weg scheitert, und das ist gemessen:** |B| im Statoreisen aus dem Feldlauf streut ueber N = 300…800 um **56…73 %** und nicht einmal monoton (s. `BEFUNDE.md`) — auch im Zahn und im Joch, nicht nur in den duennen Rotorstegen; eine Erosion in ZELLEN statt in Millimetern haette dabei fast zum falschen Schluss gefuehrt, weil sie bei verschiedenen N verschiedene Gebiete vergleicht. Gerechnet wird deshalb ueber die **Flusserhaltung** aus `B_gap` — der Groesse, die hier ausdruecklich aufloesungsunabhaengig ist: `Phi_pol = B_gap·D_gap·L/p`, `B_zahn = B_gap·tau_nut/(b_zahn·k_fe)`, `B_joch = Phi_pol/(2·h_joch·L·k_fe)`. Gegenprobe: Polfluss aus dem Luftspalt und Fluss durch die Zaehne stimmen auf **0,019 %**. Die Grenze kommt aus `LAMINATES[...]["B_sat_T"]` (1,65…2,0 T je Blechsorte), die Ankerrueckwirkung ueber `_analytical_Barm` und **vektoriell** statt als Betragssumme — bei MTPA ist `i_d` negativ und schwaecht den Magneten, wer addiert ueberschaetzt den Zahn gerade dort, wo die Feldschwaechung arbeitet. **Was das Ergebnis bedeutet, steht dabei:** eine Ueberschreitung ist KEINE Wand, an der die Maschine stehenbleibt — sie laeuft weiter und liefert nur weniger Moment als gerechnet; oberhalb sind Kt, Moment und Leistung zu optimistisch. **Ein Hebel, den die Messung sichtbar gemacht hat:** weil `nutgeometrie` die Nutbreite als festen Anteil der Nutteilung setzt, kuerzt sich die Teilung heraus und es gilt `B_zahn = B_gap/((1-slotWidthRatio)·k_fe)` — die **Nutzahl bewegt die Zahnflussdichte nicht** (24/36/48/60 Nuten geben alle 1,2500 T), `slotWidthRatio` dagegen von 0,962 auf 1,786 T. Angeschlossen im **gemeinsamen** Bewerter `ema_optimize._eval_geom` (Schluessel `B_zahn`/`B_joch`/`B_eisen`/`saettigung`/`engstelle`, rein additiv). **Dass der Bewerter sie fuehrt, heisst aber nicht, dass sie jemand ZEIGT** — auf die Frage „wo wird die Saettigung dargestellt?" war die ehrliche Antwort zuerst „fast nirgends": `ema_paramstudy._STUDY_METRICS` filtert ueber `ema_optimize.METRICS`, `/optimize/meta` liefert genau diese Tabelle an die Oberflaeche, und `ema_sicherheit`, `ema_steckbrief` und `ema_paarvergleich` kannten den Kennwert gar nicht. Angeschlossen sind jetzt alle: `METRICS` (Studienkurve + Browser-Zielgroesse und -Nebenbedingung), `ema_sicherheit` als zehntes Kriterium (**Schwere `hinweis`, nicht `verletzt`** — die Maschine bleibt nicht stehen, sie liefert weniger Moment als gerechnet), `ema_db.HERKUNFT` + `ema_steckbrief.KENNWERTE`, `ema_paarvergleich` als **Zusatzspalte** (mitgefuehrt, nicht gewichtet: ob eine Auslegung ins Eisen laeuft, ist eine Aussage ueber ihre GUELTIGKEIT und nicht ueber ihre Guete) samt neuer Achse `nutbreite`, und `run_pipeline` schreibt sie ins `summary`, womit Bericht und Ergebnisreiter sie ebenfalls haben. **Dazu zwei Bilder, analog zum Feldbild** (`charts/saettigung_eisen.png`: der Querschnitt ueber `render_cross_section` — dieselbe Zeichnung wie CAD-Schnitt und Bilddatensatz — mit Zahn- und Jochring nach ihrer Ausnutzung eingefaerbt; `charts/saettigung_kennlinie.png`: B_Zahn und B_Joch ueber dem Moment mit der Blechgrenze und dem markierten Knie, gemessen „ab 59 Nm laeuft der Zahn ins Eisen"). Sie liegen in `charts/`, weil die rechte Spalte beider Agentenkoepfe dort ueber die Aenderungszeit nachsieht — kein zweiter Meldeweg. Und in **beiden** Bildern steht, dass sie aus der Flusserhaltung kommen und kein geloestes Feld sind: ein Bild, das wie ein Feldbild aussieht und keines ist, waere die schlimmste Variante. Auf Zuruf ueber `leistung --bilder`, sonst bei jedem Pipelinelauf (gemessen 1,1 s). Test: `test_saettigung.py` (25 Pruefungen ohne Loeser — die Flusserhaltung laesst sich gegen sich selbst schliessen, was eine Formel wirklich prueft, waehrend ein festgenagelter Zahlenwert nur prueft, dass sich nichts geaendert hat) |
| `ema_leistung.py` (Verb `leistung`) | **Die umgekehrte Frage: was gibt diese Geometrie her, und WAS begrenzt sie?** Die Pipeline bekommt eine Last und rechnet, was herauskommt; `ema_sicherheit` sagt hinterher, ob das zulaessig war. Beides zusammen beantwortet nicht, was vor dem Auslegen zu wissen ist. Hier wird die Last **gesucht**: je Drehzahl Bisektion auf das groesste Moment, bei dem keine Grenze ueber 100 % geht. **Nichts davon ist neu gerechnet** — `power_envelope` bleibt die eine Quelle der elektrischen Huellkurve, `ema_optimize._eval_geom` der Bewerter (gemessen 0,01–0,09 s je Aufruf, weil der LU-Cache die Faktorisierung ueber alle Lasten haelt: `mu` haengt an der Geometrie, nicht am Strom — eine ganze Kennlinie kostet ~2 s), und die Grenzwerte werden aus `ema_sicherheit` GELESEN statt neu aufgeschrieben. **Der Ertrag ist der Ausnutzungsgrad:** jedes Kriterium dort fuehrt seit jeher `wert` UND `grenze`, geteilt hat sie nie jemand — eine Grenze bei 40 % ist verschenktes Material, und das ist die Angabe, aus der eine Auslegungsentscheidung folgt, nicht das bestandene Ja. Drei Faelle werden unterschieden und nicht vermischt: die Thermik bindet · der Umrichter bindet · **es reisst schon im Leerlauf** (dann binden die drehzahlabhaengigen Verluste und nicht das Moment, und „0 Nm moeglich" waere als Zahl richtig und als Auskunft nutzlos). `UNGEPRUEFT` steht unter JEDER Ausgabe, allen voran die **Saettigung** — der FDM ist linear (`MU_R_IRON=500`), `_analytical_Bgap` hat keinen Eisenterm, `B_SAT_IRON` wirkt nur im Anzeigepfad, das Moment waechst hier also linear mit dem Strom weiter, als gaebe es kein Eisen. Wer darauf einen Optimierer setzt, der „alle Punkte ausreizt", laeuft in genau diese Luecke — derselbe Fehler wie das fehlende Layouttor im Bewerter. **Und die Gegenprobe laeuft bei jedem Lauf mit:** `ema_thermal.rated_torque` (2·σ·V_rotor aus dem Kuehlungs-Preset) traegt im Kennfeld die Kurve „Dauer" und setzt `cont_limited_by="kuehlung"` — gemessen nennt es 122,6 Nm, waehrend dasselbe LPTN-Netz dort **277,6 °C** am Magneten gegen 80 °C Grenze meldet (Faktor 16 in der Leistung, s. `BEFUNDE.md` 13.09.2026). Aufgeloest wird das NICHT — gemeldet wird es, als „⚠ WIDERSPRUCH im Werkzeug" ueber der Kennlinie. Test: `test_leistung.py` (27 Pruefungen, der Kern ohne Loeser, weil `hoechstlast` seinen Bewerter uebergeben bekommt) |
...
| `ema_getriebe.py` (Verb `getriebe`) | **Das Getriebe wird gerechnet statt angenommen.** Es war zwei Konstanten: `DEFAULT_VEHICLE["gear_ratio"] = 9.5` und `["eta_drive"] = 0.95`. `compute_drivetrain` rechnete damit `rpm_motor = rpm_wheel·i` und `T_motor = T_wheel/(i·η)` — keine Stufenteilung, keine Zaehnezahlen, kein Modul, keine Tragfaehigkeit, keine Masse, keine Traegheit, und ein Wirkungsgrad, der bei Volllast dasselbe sagt wie im Schub. Ein Fahrrad-Nabenmotor und ein Traktionsantrieb bekamen dieselben Zahlen. **Gerechnet wird jetzt aus der GEOMETRIE**, nicht aus Tabellen: Zahnformfaktor `Y_Fa` und Kerbfaktor `Y_Sa` nach dem Verfahren der 30-Grad-Tangente (ISO 6336-3/DIN 3990-3, Hilfsgroessen E/G/H, Waelzwinkel `theta` iterativ) — eine Tabelle waere hier das Naheliegende und das Falsche gewesen, denn sie gilt nur fuer x = 0 und genau EINE Werkzeugform: gemessen faellt `Y_Fa` bei z = 14 von 3,20 auf 2,26, sobald x = +0,5 gewaehlt wird, und die Auslegung waehlt es, sobald z unter 17 faellt. Ebenso aus ihren Formeln: Profil- und Sprungueberdeckung (daraus `Y_eps`/`Z_eps`), Zonenfaktor `Z_H` (ergibt fuer die Normverzahnung die bekannte 2,495), Elastizitaetsfaktor `Z_E` aus E und nu (189,8 fuer Stahl/Stahl), Zahnverlustgrad `H_V` nach Ohlendorf. **Der Zahnfuss gibt den ersten Wurf, die Flanke entscheidet meistens:** die Fussformel liefert m aus T^(1/3), die Flanke traegt mit m^1,5 — an einer 191-Nm-Stufe haelt der Fuss bei m = 2,0 mit S_F = 1,41, waehrend S_H dort bei 0,91 steht. Deshalb geht die Auslegung ab dem Fussmodul die Normreihe (DIN 780, **aufgerundet**, nie ab) hoch, bis beide Sicherheiten stehen, und gibt aus, **welche gebunden hat**. Zaehnezahlen: das Ritzel bleibt in einem schmalen Fenster ueber der Unterschnittgrenze (ohne diese Schranke gewinnt immer das groesste Ritzel — mit z1 = 41 laesst sich jede Uebersetzung auf ein Promille treffen, und das Rad wird doppelt so gross), teilerfremd wo es nicht mehr als 0,5 % Uebersetzungsfehler kostet, und `i_ist` steht IMMER neben `i_soll`. **Vier Bauarten, und jede sagt, wie weit sie getragen ist:** Stirnrad (ein- und zweistufig, voll), Planetensatz (voll, mit Montagebedingung `(z_s+z_h) % n_p == 0` und Nachbarbedingung; beide Eingriffe gerechnet, der Innen-Eingriff kommt korrekt guenstiger heraus, weil `(u−1)/u` statt `(u+1)/u` zaehlt), Kegelrad (Tragfaehigkeit ueber das **Ersatz-Stirnrad** nach Tredgold — eine Naeherung, und sie steht als solche in der Ausgabe) und Schnecke (**keine** Zahnfussrechnung; dort zaehlen Wirkungsgrad, Selbsthemmung und Erwaermung, und weil der Wirkungsgrad so stark am Reibwert haengt, steht eine SPANNE da und keine Zahl). **Der Einbauort ist ein Tor, keine Beschriftung:** `EINBAU_ERLAUBT` sagt, welche Bauart wo sitzen kann — `in_welle` (Planetensatz in der Hohlwelle des Laeufers, der integrierte Antrieb) geht nur beim Planetensatz, ein Stirnradsatz wird abgewiesen und nicht genaehert. `in_welle_pruefen` stellt drei Zahlen nebeneinander und sagt, **welche bindet**: was der Satz braucht (Hohlrad-Fusskreis + Wand), was gezeichnet ist (`shaftBoreD`) und was **magnetisch zulaessig** ist — letzteres nicht geschaetzt, sondern aus `ema_welle.pruefen` (ein Feldlauf, `bohrung.hoechstens_mm`). Gemessen an einer 90-mm-Welle: gezeichnet 70 mm, magnetisch zulaessig 66 mm, gebraucht 100,6 mm — der Satz passt nicht, die gezeichnete Bohrung ist ueberdies schon zu gross, und beides steht da. Ohne Wellenbefund wird ausdruecklich gesagt, dass die magnetische Grenze UNGEPRUEFT blieb: „passt in die gezeichnete Bohrung" ist nicht „zulaessig". Axial wird gegen Zahnbreite + Lager geprueft. **Werkstoffe sind FREMDWERTE** und stehen deshalb in `ema_referenz.GETRIEBE_WERKSTOFF` — ausdruecklich als **Annahme** (Groessenordnung der Werkstoffklasse mit Spanne), nicht in `MESSPUNKTE`, wo nur Zitiertes hingehoert; jede damit gerechnete Sicherheit traegt `werkstoff_beleg: "annahme"`, und **ohne Kennwert gibt es kein Modul** statt eines Ersatzwerts.  **Im Steckbrief und im Bericht steht es auch** — und aus DERSELBEN Quelle: `ema_steckbrief.getriebe()` liest die juengste abgelegte `getriebe`-Rechnung (`rechnungen/*_getriebe.json`) und rechnet **nichts** nach; das Getriebe gehoert nicht in `results.json`, die gehoert dem Pipelinelauf und wuerde es beim naechsten `run analyse` ueberschreiben. `ema_report._ensure_getriebe_section` haengt daraus den bebilderten Abschnitt an (Muster `_ensure_kuehlung_section`), `_single_md_tables` die Zahlen — die Prosa bleibt wertfrei wie ueberall hier. **Zwei Verwechslungen, die dabei auffielen und behoben sind** (s. `BEFUNDE.md`): `haelt` wurde beim Einbau in der Welle mit `passt` verrechnet, sodass ein Satz mit S_F 2,49 / S_H 1,06 als „traegt nicht" abgelegt wurde, obwohl ihm nur Platz fehlte — das schickt die Suche zu mehr Modul statt zu mehr Platz; und `rechnungen()` sortierte die Zeitmarken als ZEICHENKETTEN, sodass von zwei Rechnungen derselben Sekunde (`…000000` und `…000000-2`) die AELTERE als die juengere galt — im Dateinamen sortiert der Unterstrich ueber den Bindestrich. Zwei Verben in einem Agentenzug sind beide in Millisekunden fertig; das ist der Normalfall. **Bedienbar auch im Browser** (Reiter ⚙ Getriebe, `POST /getriebe` + `/getriebe/status`, eigener `_getriebe_state` nach dem Muster `design_ai`): die Maske ruft dieselbe Kette. `ema_getriebe.lauf(spec, projekt_dir, mit_feld=, mit_cad=, melde=)` fuehrt Feldlauf, Auslegung, Bild, Zeichnung und ist die EINE Stelle, an der das steht — Verb und Route sind sonst zwei Abschriften, die beim ersten Fehlerbericht auseinanderlaufen (derselbe Grund, aus dem alle drei Agentenkoepfe EINE `SKILL.md` lesen); ebenso `bestanden(erg)` fuer das Urteil „haelt UND passt". `ema_welle` und `ema_getriebe_cad` werden darin **erst beim Aufruf** importiert, damit der Rechenkern weiterhin ohne FreeCAD ladbar bleibt. Moment und Drehzahl kommen aus dem Payload des aktiven Projekts, wenn die Felder leer bleiben; fehlen sie ueberall, ist das ein 400 und keine Annahme. **`--uebernehmen` ist der Schritt, der es wirksam macht** (Haekchen im Reiter, `uebernehmen: true` an der Route, `cae_cli._getriebe_uebernehmen` unter allen dreien): die Auslegung wird nach `meta.json` unter `vehicle.getriebe` geschrieben, `gear_ratio` mitgesetzt — und **erst dann** rechnet der Fahrzyklus mit ihr. Bis dahin konnte `compute_drivetrain` den Schluessel zwar LESEN, geschrieben hat ihn niemand (s. `BEFUNDE.md`): jeder Lauf nahm weiter 9,5 und 0,95, waehrend die gerechnete Auslegung danebenlag — genau der Zustand, gegen den der Umbau gebaut war. Uebernommen wird nur, wenn die VERZAHNUNG traegt; wo der Satz sitzt, geht den Fahrzyklus nichts an. Gemessen am Stadt-Land-Zyklus: n_max 7475,5 -> 7488,7 1/min und ein anderes Spitzenmoment, weil eta an der Last haengt und `J_red` beim Beschleunigen wie Masse wirkt. **Zwei Planetenstufen, koaxial hintereinander — auch IN der Welle.** Der Planetensatz war auf EINE Stufe festgenagelt (`if art == "planeten": stufen_n = 1`), und damit endete er bei i = 10. Zwei Stufen tragen 9…100 und liegen auf derselben Achse hintereinander, mittig in der Bohrung. `in_welle_pruefen` nimmt deshalb die ganze **Kette** statt einer Stufe: radial bindet die **dickste** (sie teilen sich dieselbe Bohrung — wer summierte, verboete jede zweistufige Anordnung ohne Grund), axial dagegen die **Summe** der Zahnbreiten plus `STUFENABSTAND_MM` je Fuge plus die Lager, und der Befund sagt, wie viel Luft mittig an jeder Seite bleibt. **Die Aufteilung wird fuer die Bohrung GESUCHT, nicht nach Regel genommen** (`aufteilung_fuer_bohrung`): die uebliche Regel `i1 = sqrt(1,2*i)` ist aufs Bauvolumen gemuenzt und kennt die Bohrung nicht. Der Zusammenhang ist gegenlaeufig — ein groesseres `i1` laesst die zweite Stufe mehr Moment tragen, ein kleineres schiebt Uebersetzung in ihr Hohlrad — und das Optimum liegt bei jedem Moment woanders. Gemessen an i = 25 / 60 Nm: **174,4 mm noetige Bohrung gegen 199,5 mm nach Regel**, und ueber alle Aufteilungen spannt der noetige Durchmesser **174…347 mm**. Gesucht wird durch Abtasten und nicht ueber einen Gradienten, weil die Zielgroesse eine **Treppe** ist: Zaehnezahlen sind ganz und die Modulreihe springt (im Test nachgewiesen, die Kurve ist nicht monoton). Das Bild wird bei mehreren Stufen zum **Laengsschnitt** — von vorn laegen zwei koaxiale Stufen genau uebereinander und man saehe gerade nicht, worum es geht. Test: `test_getriebe.py` |
| `ema_getriebe_cad.py` — der Planetensatz ist eine BAUGRUPPE | **Gezeichnet wurden Sonne, Planeten und Hohlrad -- und sonst nichts** (bis 19.09.2026). Kein Steg, keine Bolzen, keine Wellen, keine Bohrungen in den Raedern, und vor allem: **nie nachgemessen**. Vier Fehler standen darin, jeder still (s. `BEFUNDE.md`): **(1) Die Zahnphase fehlte.** FCGear setzt auf JEDES Rad einen Zahn auf die +x-Achse, zwei so erzeugte Raeder stehen also Zahn auf Zahn. Gemessen ueber einen Versatzlauf: 0 Grad -> 336,7 mm3 Durchdringung Sonne/Planet, **9,00 Grad = genau eine halbe Zahnteilung -> 0,000 mm3**, und zwar am Planeten bei 0 Grad WIE bei 120 Grad; das Hohlrad ebenso um eine halbe Hohlradteilung. Der naheliegende Term `Stellwinkel * z_Sonne/z_Planet` ist die Verdrehung eines ABROLLENDEN Planeten -- Kinematik, nicht Zeichnung -- und laesst 262,9 mm3 stehen. Unbemerkt blieb es, weil der Phasenfehler in Zaehnen gerechnet genau dann verschwindet, wenn `z_Sonne/n_Planeten` aufgeht: der von der eigenen Auslegung erzeugte Satz (24 Zaehne, 3 Planeten) trifft das zufaellig. **(2) Die Raeder hatten keine Bohrung** -- die Sonnenwelle steckte zu 27,0 %, jeder Bolzen zu 58,8 % im Vollmaterial. **(3) Die FCGear-Quellobjekte blieben im Dokument stehen**, am Ursprung, also mitten in der Baugruppe: jedes Rad zweimal. **(4) Es gab keinen Steg** -- und damit keine Antwort auf „passt der Satz in die Hohlwelle". Jetzt: halber Zahnschritt je Rad, Bohrungen, `doc.removeObject` nach dem Kopieren, Planetenbolzen, zwei Stegwangen mit Bohrungen fuer Sonnenwelle und Bolzen, Sonnen- und Stegwelle, jedes Teil als eigenes benanntes Objekt PLUS dem Verbund. Dazu die **Verzahnungsart** (`gerade`/`schraeg`/`pfeil`, `ema_getriebe.VERZAHNUNG_ARTEN`, Vorgabewinkel 15 Grad): `beta` geht in Zahnform, Ueberdeckung, Zonen- und Breitenfaktor laengst ein, GEZEICHNET wurde es nur bei der Stirnradstufe. Erreichbar ueber `cae_cli getriebe --verzahnung`, die Maske im ⚙-Reiter und `POST /getriebe`. Vorbild fuer die Bauteile ist `Tools/planetengetriebe.py` aus `/home/cae/t2g/TextToGeometry` — **portiert, nicht kopiert**: T2G steht unter PolyForm-Noncommercial-NoMilitary und dieses Repo unter MIT (eine woertliche Uebernahme waere eine stille Umlizenzierung), und T2Gs Module importieren FreeCAD auf Modulebene, waehrend dieser Zeichner Text fuer einen fremden Prozess schreibt |
| Nachmessen am Getriebe — und der Schiedsrichter, der keine Boolesche ist | Der gebaute Satz wird paarweise ueber `common().Volume` geprueft (Muster `bau_eesm.py`), Ergebnis in `durchdringung_pct`/`durchdringung_paare`/`durchdringung_unklar`. Dabei kam ein eigener Befund heraus: **OCCs `common()` liefert bei einem gescheiterten Schnitt bitgleich das Volumen eines Operanden** — gemessen 22.364,90 mm3 fuer „Planet 0 steckt vollstaendig im Hohlrad", waehrend die beiden baugleichen Nachbarn 0,0 % zeigten und der Planet im LOCH des Rings sass. Zwei naheliegende Gegenproben versagen: der Huellquader entscheidet nichts (der eines Planeten liegt sehr wohl in dem des Hohlrads), und ein zweites `cut()` scheitert an derselben Stelle und BESTAETIGT den Fehler. Der Schiedsrichter ist deshalb eine **Punktprobe** (600 Wuerfe in den Huellquader des kleineren Koerpers, `Shape.isInside`) mit einem eigenen winzigen Zufallsgenerator, damit jeder Lauf dieselben Punkte nimmt: gemessen 0,0 % aus 346 Proben. Gemeldet wird das als *nicht nachweisbar* und NICHT als Durchdringung — eine 100-%-Meldung, die in Wirklichkeit ein Fehlschlag ist, schickt die Suche ans falsche Ende |
| `ema_getriebe_cad.in_motor_bauen` — der Satz im Motor | Oeffnet das **gebaute** Motordokument und legt den Planetensatz mittig auf die Achse. Ein dritter Erzeuger waere die dritte Abschrift derselben Geometrie gewesen; statt dessen laufen die beiden vorhandenen (`ema_freecad.build_full_motor_script` und `bauen`) unveraendert weiter und das Ergebnis wird zusammengefuegt. **Mittig heisst mittig im BLECHPAKET**, nicht in allem, was das Dokument enthaelt: die Hairpin-Wickelkoepfe ragen ueber beide Stirnseiten hinaus und verschoben den Satz gemessen um 2,9 mm. Der Bohrungsdurchmesser wird am Laeuferkoerper **abgelesen** (kleinster Zylinder um die Drehachse), nicht aus dem Payload geglaubt — eine Zahl aus der Eingabe sagt, was gewollt war, diese sagt, was gezeichnet wurde. Getriebe gegen Motor wird paarweise nachgemessen. Gemessen an einem 280-mm-Motor mit 118-mm-Bohrung und einem i=5-Satz: Bohrung 118,0 mm abgelesen, Satz 100,6 mm, **Durchdringung 0,0 %**. Passt es nicht, ist das ein Befund mit zwei Zahlen (bei 100-mm-Bohrung: 100,6 gegen 100,0 gebraucht) |
...
| `ema_aster.py` | **Code Aster 17.4.0 als DRITTER Löser auf demselben Netz** — aus einem ausgepackten Salome-Meca-SIF-Abbild unter `~/aster-build` (`CAE_ASTER_HEIM`), dessen `env_aster.sh` `LD_LIBRARY_PATH`/`PYTHONPATH`/`ASTER_*` setzt. **Aster 17 läuft als BIBLIOTHEK** (`import code_aster`, `code_aster.Commands`), nicht über `as_run` und nicht über eine `.export` — es gibt in diesem Abbild kein `as_run`. `loese` fährt deshalb `bash -lc 'source env_aster.sh; exec "$ASTER_PYTHON" rechnung.py'` in einem Arbeitsverzeichnis; `_aster_python()` liest den Interpreterpfad AUS dem env-Skript statt ihn nachzubauen, sonst laufen die beiden beim nächsten Abbild auseinander. Geschrieben wird Asters **eigenes** `.mail`-Format (`FORMAT='ASTER'`, `UNITE=20`) und nicht MED — `medcoupling` gibt es im System-Python nicht. Knoten heißen `N<i>`, Elemente `M<i>`, **die Nummern bleiben die des `Netz`**, sonst ließe sich ein Befund zwischen den drei Lösern nicht zuordnen. **Drei gemessene Fallen:** (1) **Aster dualisiert Dirichlet-Randbedingungen mit Lagrange-Multiplikatoren**, und die stehen im selben Vektor wie die Verschiebungen — ein `getValues().reshape(-1,3)` lieferte gemessen eine „Verschiebung" von **1397 mm**, während die Spannungen mit 157,6 MPa völlig plausibel waren; das war eine Reaktionskraft. Gelesen wird über `toSimpleFieldOnNodes()` samt Gültigkeitsmaske, und bei einer Fassungsabweichung bleibt das Feld LEER statt halb gefüllt. (2) Die **Dichte geht in t/mm³** hinein (`density/1e12`) — mit kg/m³ rechnet Aster klaglos weiter. (3) Ein **vorgegebener Arbeitsordner muss nicht existieren**; `loese` legt ihn an und räumt ihn ausdrücklich NICHT weg, weil bei einem Fehlschlag die `.mess` die einzige Spur ist. Der **Sektorlauf bleibt CalculiX** (`NUR_VOLLROTOR` statt einer Näherung). `kennzahlen_aus_lauf` delegiert an `ema_deck.kennzahlen` — eine zweite Auswertung wäre die Stelle, an der drei Löser drei Zahlen liefern, ohne dass der Löser schuld wäre. Test: `test_aster.py` (ohne Aster; `--echt` rechnet zusätzlich gegen CalculiX) |
...
| `ema_maschinenart.py` | **Maschinenart als ausgesprochener Begriff — und als Tor.** **„Traegt die Stufe" und „geht DIESEN Weg" sind zwei Fragen** (seit 18.09.2026 sauber getrennt): die ASM traegt `analytisch`, `feld`, `cad` und `em3d` — aber Feld und 3-D laufen ueber `feld2d`/`feld3d` (Elmer, harmonisch) und nicht ueber die magnetostatische Kette der Pipeline, die sie dort weiterhin abweist. `server._art_optionen` hatte die beiden vermischt: der Hinweis auf den Feldweg stand im Zweig „nicht alle Stufen getragen" und waere mit dem CAD genau fuer die eine Art verschwunden, die ihn braucht. Er haengt jetzt am Weg, nicht an der Vollstaendigkeit. Das Werkzeug ist als PSM mit Hairpins gewachsen, und diese Annahme steht nirgends als Schalter, sondern verteilt in sechs Modulen: `_analytical_Bgap` rechnet aus `Br_NdFeB`/`magThick`, `compute_performance` gibt `Kt = 1.5*p*psi_pm` (reines Magnetmoment), `estimate_dq_currents` teilt durch dieses Kt, `estimate_saliency` legt die Magnetdicke in den d-Pfad, `ema_rotorcheck` prueft Magnettaschen, `ema_referenz` fuehrt Baender je **Magnet**anordnung. Wer dort eine Asynchronmaschine hineinreicht, bekaeme **keine Fehlermeldung, sondern PSM-Zahlen unter fremdem Namen** — derselbe stille Fehler wie WLTP an einem Fahrrad. `ARTEN` fuehrt `pmsm` (Vorgabe) · `asm` · `synrm` · `eesm` mit Erregungsart, Magnet-/Laeuferwicklung/Schlupf-Merkmalen und — der eigentliche Punkt — **welche der vier Stufen (`analytisch`/`feld`/`cad`/`em3d`) die Art heute wirklich traegt**. `pruefe_stufe(code, stufe)` wirft `ArtNichtUnterstuetzt` mit einem Text, der sagt, **was statt dessen geht**; `gilt(code, kennzahl)`/`filtern` trennen „null“ von „nicht anwendbar“ (Magnetmasse 0 kg ist eine Aussage; ein Kurzschlussstrom von 0 A liesse sich als „gemessen und unbedenklich“ lesen). Der Schluessel sitzt in **`geom.machineType`**, weil die analytischen Funktionen nur `geom` bekommen. Die Auswahlliste in `ema_text2ema.SCHEMA` kommt aus diesem Modul (keine zweite handgepflegte Menge). **Und sie hat seit dem 18.09.2026 ein Bedienelement** (Abschnitt „Maschinenart“ im Geometrie-Reiter, VOR der Magnet-Topologie, weil sie die Entscheidung davor ist): `machineType` stand bis dahin nur im Schema und war ueber Parameter-Tabelle und `cae_cli --set` erreichbar — `ema.html` kannte den Schluessel **ueberhaupt nicht** (gemessen null Treffer), `buildPayload` schickte ihn nicht mit, und der Browser rechnete damit **immer eine PSM**, auch nachdem ASM, EESM und GSM zeichenbar geworden sind. Die Auswahl wird aus `/param_schema` geladen und nicht im HTML abgeschrieben (dort steht genau der Vorgabe-Eintrag, bis der Server antwortet); ihre Beschriftung nennt gleich, **wie weit eine Art getragen ist**, damit die Liste nicht zu einer Wahl einlaedt, die der Lauf danach abweist. Bei einer Art ohne Magnete wird die Magnet-Topologie ausgeblendet — ein Regler, der nichts bewegt, liest sich wie einer, der nicht wirkt. Tore: `ema_pipeline._gate_maschinenart` (vor Feld **und** vor CAD) und `ema_screen.screene`. CLI: `cae_cli.py maschinenart [code]`. Test: `test_maschinenart.py` |
| `ema_asm.py` | **Asynchronmaschine, analytisch UND gezeichnet — zwei Laeuferbauformen.** `rotorType` entscheidet: `kaefig` (Vorgabe) oder `schleifring`. Die beiden unterscheiden sich nicht im Stator, sondern darin, was in den Laeufernuten liegt und ob man von aussen drankommt — und deshalb auch in der **Nutzahlregel**: der Kaefig braucht ausdruecklich Unsymmetrie zum Stator (`stabzahl`), die Drehstromwicklung zuerst Symmetrie in sich (`laeufernuten`: `n % 3 == 0`, ganzes `q` bevorzugt aber nicht erzwungen — Bruchlochwicklungen werden gebaut, und ein nicht ganzzahlig treffbares `q` waere sonst ein Nein ohne physikalischen Grund). `laeuferwicklung` bemisst die Nut aus dem **Strom**, nicht aus dem Platz — genau der Fehler, den `kaefig` schon einmal gemacht hat (dort fuellte der Stab den Laeuferraum bis zum Deckel und kam auf 1,14 A/mm² statt 4–8; der Widerstand lag um Faktor 5 daneben und mit ihm der Schlupf). Die Nutformel wird dabei **nicht** neu geschrieben: `ema_wicklung.nutgeometrie` bekommt eine **Schattengeometrie** mit den Zahlen des Laeufers. `anlasswiderstand` ist der ganze Zweck der Bauform, und der Punkt daran ist, **wo die Waerme bleibt**: der Zusatzwiderstand steht ausserhalb der Maschine, zaehlt also in den Schlupf (der IST der Anteil der Luftspaltleistung, der im Laeuferkreis verheizt wird — Definition, keine Naeherung), belastet die Laeuferwicklung aber NICHT; `betriebspunkt` fuehrt beide Anteile getrennt (gemessen: doppelter Laeuferwiderstand → doppelter Schlupf, Verlust IN der Maschine unveraendert). `schleifringe` gibt drei Ringe samt Buerstenflaeche aus der Stromdichte am Kontakt und prueft die **Umfangsgeschwindigkeit** (`V_RING_MAX_MPS`=45) — der eigentliche Deckel dieser Bauform. `schraegung_nutteilungen`/`schraegung_grad` in LAEUFERnutteilungen (Grad sagen ohne die Nutzahl nichts), `-1` = automatisch nach Bauform (Kaefig 1,0, Schleifring 0), `0` = ausdruecklich ohne — der Unterschied ist noetig, weil der frische Payload den Schluessel ohnehin immer traegt. Test: `test_laeufer_cad.py`. **Der analytische Kern (Stufe A der Verallgemeinerung):** Kein zweites Momentgesetz: die ASM speist **dieselbe** `compute_performance` mit dem Feld, das ihr Magnetisierungsstrom erzeugt (`B_m` = `geom.bZielT`, Vorgabe 0,80 T, gedeckelt durch die Zahnsaettigung) — nur so haben `Kt`, `I_s`, `P_verlust` und `T_dauer` dieselbe Bedeutung wie bei der PSM, und nur so ist die Achse `maschinenart` ein Vergleich. Der Unterschied steht dort, wo er hingehoert: `I_s = hypot(i_mag, i_q)` (die ASM traegt den Magnetisierungsstrom **dauernd** mit) und `P_kaefig = s·T·ω_syn` im **Laeufer**, der thermisch schlechtesten Stelle. **Die Normierungsbruecke `k_norm = π·k_w·N_ph/p²`** ist der heikle Punkt und deshalb ausfuehrlich belegt: `compute_performance` gibt eine **normierte** Flussverkettung heraus (`psi_pm = p·(2/π)·B·R·L`, „1 turn per slot“), waehrend `_analytical_Barm` und die `Ld`-Formel in `estimate_dq_currents` mit der **physikalischen** Durchflutung rechnen. Wer `i_mag` physikalisch ausrechnet und ungerechnet neben `i_q = T/Kt_haus` stellt, vergleicht zwei verschiedene Amperes und laesst den Magnetisierungsanteil um genau diesen Faktor zu klein aussehen. Die Rechtfertigung der Bruecke ist eine **Erhaltung**, kein Argument: `1.5·p·psi·i` ist unter ihr invariant — `test_maschinenart.py` rechnet das nach. `i_mag` selbst ist die **Umkehrung von `_analytical_Barm`**, also derselben Funktion, an der auch das FDM-Statorfeld geeicht ist, keine zweite eigene Formel. Weiter: `stabzahl` (Auswahlregel — nie gleich der Statornutzahl, nicht um 0/p/2p/3p daneben, kein Vielfaches der Polzahl), `kaefig` (Nutraum zwischen Steg und Joch, Jochhoehe aus dem Fluss je Pol, Nuttiefe gedeckelt auf Tiefe/Breite ≤ 3 — darueber bestimmt die **Stromverdraengung** den Widerstand, und die ist hier nicht gerechnet), `verluste` (Statorkupfer × `(I_s/i_q)²`, kein Magnetwirbelstrom, dafuer der Kaefig), `dauermoment` (Abschlag aus derselben Stromdichtegrenze: thermisch zulaessig ist `I_s,max`, momentbildend nur `√(I_s,max²−i_mag²)`), `massen_und_kosten` (keine Magnete, dafuer Kaefig; `PREISE_EUR_KG["alu"]`) und `steg_check` (Steg ueber der Kaefignut als **beidseitig eingespannter Balken**; der Zahn haengt nicht daran, er sitzt am Joch — die Bohrungs-Ringspannung prueft weiterhin `rotor_stress_check`). **Kein Feldlauf:** die 2-D-FDM ist reell, linear und magnetostatisch (kein σ, kein ∂A/∂t) und kann einen Kaefiglaeufer grundsaetzlich nicht abbilden; die ASM-Feldstufe braucht Elmers `MagnetoDynamics2DHarmonic` (auf dieser Maschine vorhanden, nachgemessen) und ist noch nicht gebaut. Test: `test_maschinenart.py` |
| `ema_eesm_cad.py` | **Der Schenkelpollaeufer der EESM — als reine Zahlen, in Millimetern.** Er entsteht hier und nicht in `ema_freecad`, weil der Zeichner Python-TEXT fuer einen fremden Prozess schreibt: staende die Geometrie nur dort, gaebe es sie genau einmal (im erzeugten Skript) und weder `render_cross_section` noch ein Test kaemen an sie heran — derselbe Fall, der bei den Wickelkopf-Radien schon einmal eintrat (`ema_wicklung.hairpin_radien`). `koerper(geom, axial_mm)` liest `ema_eesm.polgeometrie`/`erregung`, also den **Magnetkreis, der die Maschine rechnet**, und leitet daraus vier Teile ab: Polschuh (Kreisbogensegment am Rand, Breite aus der Polbedeckung), Polkern (Rechteck darunter, SCHMALER — der Ueberstand ist der Platz, auf dem die Spule sitzt), Erregerspule (zwei Querschnitte links und rechts, Dicke aus dem **Kupferquerschnitt** der Stromdichte statt aus dem verfuegbaren Platz — dieselbe Entscheidung wie bei der ASM-Laeuferwicklung und aus demselben Grund: eine Wicklung, die das Fenster fuellt, kam gemessen auf 0,7 A/mm² und 15,7 kg Kupfer) und Joch. Die Kernbreite wird am KERNradius gemessen und die Polbedeckung am Rand — ohne die Umrechnung waere der Kern bei kleinem Laeufer breiter als die Teilung, die ihm dort zusteht. `passt` prueft Kern + Spule gegen die Polteilung samt Luft zum Nachbarpol; `ungeprueft` sagt ausdruecklich, dass die **Polbefestigung** (Schwalbenschwanz/Bolzen) und die Fliehkraft am Polfuss NICHT gerechnet sind — Schweigen liesse sich als „geprueft" lesen. `zeichenmasse` gibt nur Zahlen heraus: ein Urteil als JSON-`true`/`null` waere im erzeugten **Python**-Skript ein NameError (gemessen genau daran gescheitert). Der Zeichner verschmilzt Joch und Pole mit `POL_UEBERLAPP_MM` Ueberlappung statt auf Stoss — OCCs `fuse` macht aus zwei sich nur beruehrenden Koerpern einen Verbund aus MEHREREN Solids, und gemessen kam der Laeufer so als 7 getrennte Koerper heraus, die die Struktur-FEM als zerfallenen Laeufer vernetzt haette |
| Leiterwerkstoffe: Temperatur und Druckguss (`ema_pipeline.HAIRPIN_MATS`, `rho_bei`, `leitertemperatur`) | **Jeder Leiterwiderstand im Werkzeug rechnete bei 20 °C** — Staenderkupfer, Strangwiderstand, Kaefigstab, Laeuferwicklung, Erregerwicklung, Ankerwicklung **und die Leitfaehigkeit, mit der Elmer den Kaefig loest** (2-D und 3-D). Die Tabelle fuehrte gar keinen Temperaturbeiwert. Nach IEC 60228 (Cu 0,00393/K, Al 0,00403/K) hat eine Klasse-F-Wicklung bei 155 °C **53 % mehr Widerstand**; der ausgewiesene Kupferverlust war also um rund ein Drittel zu klein. `rho_bei(mat, T)` ist die EINE Umrechnung, `leitertemperatur(geom)` die eine Quelle der Temperatur. **Zweiter Befund: der Kaefig rechnete mit einer KNETlegierung** — `KAEFIG_VORGABE` war `al_1350` (61 % IACS), der Kommentar daneben sagte „Aludruckguss“; ein echter Druckgusslaeufer erreicht recherchiert **40–45 % IACS**, der Kaefigwiderstand lag um rund 40 % zu niedrig und mit ihm der Schlupf. Neu `al_guss` (43 %) und `cu_guss` (91 %), als `guss` gekennzeichnet — ein gegossener Kaefig ist kein Staender-Hairpin, und die Hairpin-Auswahl laesst ihn deshalb weg. **`_HAIR` im Schema ist jetzt aus der Tabelle ABGELEITET** statt danebengeschrieben: ein neuer Werkstoff erscheint ohne Zutun in CLI, Parametertabelle und Browser. Neu erreichbar sind `geom.barMat` (war lesbar und in keinem Schema, fuer den Agenten also nicht vorhanden) und `geom.tLeiterC`; der Paarvergleich bekommt die Achse **`kaefigwerkstoff`** (gemessen: Kupferdruckguss 1751 W gegen Aluminium 1928 W, Schlupf 1,01 statt 1,52 %, +1 kg und +14 €). **Das bewegt Altbestand:** Verlustleistung am frischen Payload PSM 638,7 → 833,4 W, ASM 1380 → 1928, SynRM 1070 → 1459, EESM 850,8 → 1077, GSM 5448 → 6810 W. **Und die Temperatur ist eine ANNAHME, die der gerechneten widerspricht:** `T_LEITER_AUSLEGUNG_C = 115 °C` ist der Klasse-F-Auslegungspunkt und **kein Normwert** (belegt ist nur der Beiwert), waehrend das LPTN am selben Punkt 61,2 °C ausgibt — richtig waere eine Iteration Verlust→Temperatur→Widerstand, die es hier nicht gibt. Das steht in `BEFUNDE.md` und in der Maske, statt dass zwei Zahlen unkommentiert nebeneinander stehen. Test: `test_werkstoffe.py` (31 Pruefungen) — und seine **Gegenprobe** war das Wertvollste: sie sucht nach rohen 20-°C-Werten statt zu zaehlen, wo `rho_bei` schon steht, und fand damit die eine Stelle, die beim Umstellen uebersehen worden war |
| Der Feldloeser sieht seit dem 20.09.2026 die RICHTIGE Maschine (`ema_analysis._rasterise`) | **`ema_analysis.py` enthielt die Zeichenkette `machineType` null mal.** Der 2-D-FDM-Rasterer baute den Laeufer aus `magnet_legs` — also Magnete, und sonst nichts. Gemessen ueber alle fuenf Bauarten bei demselben Payload: **1224 Magnetzellen und 15.627 Eisenzellen, Bit fuer Bit dieselbe Maschine**. Eine ASM bekam Permanentmagnete und keinen Kaefig, ein Reluktanzlaeufer Magnete statt leerer Barrieren, ein Schenkelpollaeufer Magnete statt Polen und Erregerspulen. Am Schenkelpol in der Zahl, die den Kern trifft: **2,2 % Luft im Laeuferring, wo 45 % hingehoeren** — der Reluktanzunterschied, der die Bauart AUSMACHT, fehlte. Unbemerkt blieb es, weil die Pipeline die Feldstufe fuer diese Arten abweist (`_gate_maschinenart`) und man dort hinschaut; jeder ANDERE Einstieg in denselben Loeser tat es nicht — `feldbild`, `ema_welle.pruefen` (ein echter Feldlauf, auf dem die Getriebe-Bohrungsentscheidung ruht), `ema_optimize._eval_geom` (und damit Zielwertsuche, Parameterstudie, `screen`, `leistung`, `ausreizen`, die KI-Vorsortierung), der Handy-Pfad. Keines dieser Module nennt die Maschinenart auch nur. **Jetzt kommt der Laeufer aus DERSELBEN Quelle wie die Leinwand:** hat die Art keine Magnete, stempelt `_rasterise` die Teile aus `ema_laeuferbild.teile()` ins Gitter — Bild und Feld koennen damit nicht auseinanderlaufen, sie lesen eine Funktion. Die eingepraegte Erregerdurchflutung geht als Stromdichte mit (Gleichstrom ist magnetostatisch exakt darstellbar, anders als der Kaefig). Gemessen danach: Luft im Laeuferring PSM 1,7 · ASM 13,1 · SynRM 18,8 · EESM 54,7 · GSM 18,1 %, Magnetzellen nur noch bei der PSM. **Die PSM bleibt Ziffer fuer Ziffer dieselbe** (`test_fdm_golden.py`). Zweiter Fund dabei: die Flussbarrieren des SynRM kommen als Form `tasche`, die im ersten Wurf fehlte — gemessen **0,0 % Luft**, also ein Vollzylinder, und ein Reluktanzlaeufer ohne Reluktanzunterschied ist keine Maschine |
| `ema_laeuferbild.rastere`/`_stempel` — der Python-Zwilling | Neu, und es ist bewusst eine zweite Fassung: `rastere`/`_stempel` in `ema_laeufer.js` laeuft im BROWSER fuer die Live-Vorschau, diese hier im 2-D-FDM auf dem Rechner. Die GEOMETRIE kommt nur einmal vor (`teile()`), aber das Eintragen ins Gitter muss jede Seite selbst tun — dieselbe Lage wie `magnet_legs` und sein JS-Spiegel, und sie wird genauso abgesichert: `test_laeuferbild.py` fuehrt die JS-Fassung mit `node` aus und vergleicht **Zelle fuer Zelle**. Gemessen 9156 Zellen beim Schenkelpol, 0 fehlend, 0 zuviel; dazu je Bauart einzeln (EESM `segment`/`trapez`, SynRM `tasche`, ASM und GSM `rechteck`), damit ein roter Test sagt, WELCHE Form auseinanderlaeuft |
| `ema_em3d._tor_layout` — erst die Maschinenart, dann das Layout | Der 3-D-Elmer-Pfad hatte **null** Treffer auf `maschinenart` und elf auf `magnet_legs`/`magnet_rects`: er baut den Laeufer aus den Magneten. Fuer eine Bauart ohne Magnete ist das nicht ungenau, sondern eine ANDERE Maschine. Er weist sie jetzt ab und **nennt den Weg, den es gibt** (fuer die ASM `feld3d`/`ema_em3d_harm`, harmonisch) statt nur abzusagen. Die beiden ASM-Loeser waren sauber — sie rufen `pruefe_stufe` —, sind aber auch nur fuer die ASM gebaut |
| Schenkelpol in MILLIMETERN (`polSchuhBreiteMm`, `polKernBreiteMm`, `erregerSpuleDickeMm`, `erregerSpuleHoeheMm`, `polKegelWinkelGrad`) | Jede Abmessung des Schenkelpols war ein **Verhaeltnis** — richtig fuer einen ersten Wurf und falsch, sobald jemand eine Zeichnung vor sich hat: dort stehen Millimeter. Zwei Entscheidungen, beide der Hausregel folgend: **0 heisst „wie bisher ableiten"** (jede bestehende Auslegung bleibt Ziffer fuer Ziffer dieselbe), und **ein gesetzter Wert wird genommen und NACHGERECHNET, nicht geklemmt** — ein geklemmter Wert sieht fuer den Aufrufer wie ein angenommener aus (dieselbe Regel wie `cae_cli --set`), und bei einer Zeichnung ist das besonders schlimm: wer 24 mm eintraegt und 21,7 zurueckbekommt, ohne dass es jemand sagt, zeichnet die falsche Maschine. Was nicht geht, steht mit BEIDEN Zahlen in `masse_befund` und macht `passt` falsch („Spulendicke: 40,00 mm liegt ausserhalb dessen, was hier darstellbar ist (2,00…5,36 mm)"). Der **Kegelwinkel** ersetzt das Verhaeltnis `KEGEL_VERJUENGUNG`: ein Winkel ist das, was auf einer Zeichnung steht, das Verhaeltnis haengt an der Kernhoehe. Die Kette ist geschlossen und nachgemessen — dieselbe Vorgabe bewegt Zeichnung UND Feld, weil beide `teile()` lesen: Polschuh 55 mm -> Luft im Laeuferring 54,7 -> 37,8 %; Spule 4 x 35 mm -> Zellen mit Spulenstrom 1764 -> 2020; Kegel 5 Grad -> 54,7 -> 56,1 %. **Nebenbefund, mitbehoben:** die Begruendung, warum ein Pol nicht passt, stuerzte ab — `schranken[bindend]` fuehrt nur die vier geometrischen Schranken, `bindend` kann aber auch `kernfluss` oder `vorgabe` sein (`KeyError` an genau der Stelle, die erklaeren soll, warum etwas nicht geht) |
| `ema_laeuferbild.py` / `ema_laeufer.js` (Route `POST /laeuferbild`) | **Die Live-Leinwand zeigt endlich die Maschine, die gerechnet wird.** `drawRotor` zeichnete IMMER Magnete aus der handgespiegelten `magnetLegs` — eine ASM bekam damit einen PM-Laeufer ins Bild, eine EESM auch. **Der naheliegende Weg waere der falsche gewesen:** je Bauart ein JS-Skript, das `ema_asm.kaefig` / `ema_eesm_cad.koerper` / `ema_gsm.ankerwicklung` im Browser NACHBAUT — vier weitere Abschriften von Python-Geometrie, jede mit eigenem Spiegeltest, an genau der Stelle, die diese Datei weiter oben „die gefaehrlichste im ganzen Werkzeug“ nennt. Statt dessen liefert Python **Zeichenteile als reine Zahlen** (`{form: ring|rechteck|segment|kreis|tasche, …, rolle: eisen|pol|luft|kupfer|alu}`, Millimeter, Laeuferbezug), und das JavaScript malt nur noch. **`ema_laeuferbild` rechnet NICHTS** — es wandelt um, was die Funktionen liefern, aus denen auch `ema_freecad` und `render_cross_section` zeichnen; der Test vergleicht Nutzahl, Nutbreite, Nuttiefe und Teilung Ziffer fuer Ziffer gegen sie. **Der Reluktanzlaeufer ist der eine Fall ohne eigene Quelle:** `ema_synrm` kennt nur eine Ersatz-Barrierenhoehe (`magThick`), gezeichnet wird deshalb die Taschenlage aus `magnet_legs` — und das ist die Sache selbst, ein SynRM-Laeufer ist derselbe gestanzte Laeufer wie ein IPM, nur bleiben die Taschen leer; dass die Rechnung davon nur die Hoehe kennt, steht als `hinweis` im Ergebnis. **Die PSM bleibt unberuehrt** auf ihrem alten, getesteten Weg: an der einen Darstellung zu drehen, die seit jeher stimmt, waere Risiko ohne Gewinn. **Gezeichnet UND gerastert aus derselben Liste** — `LAEUFER.rastere` traegt alles Unmagnetische ins mu-Gitter der Feldvorschau ein, Kupfer und Alu eingeschlossen (mu_r ~ 1: die Nut IST magnetisch Luft, ob ein Stab darin liegt oder nicht), und bei der EESM auch die Luft ZWISCHEN den Schenkelpolen. Ohne das zeigte die Leinwand Pole, waehrend die Feldlinien sich verhielten, als waere der Laeufer eine Vollscheibe — ein Bild, das sich selbst widerspricht. **Was NICHT darin steckt, sagt die Leinwand** (`feld_darstellbar`, Zeile unter der Anzeige): der kleine JS-Loeser ist magnetostatisch, ein Kaefigstab fuehrt darin keinen Strom und eine Erregerspule ist nicht eingepraegt — dieselbe Grenze, aus der `feld2d` entstanden ist. Die GSM ist die einzige Bauart mit einem eigenen **Staender** (`staender`-Liste, Schenkelpole nach innen statt Nuten). Abgerufen wird entprellt und nur, wenn sich eine laeufer-relevante Groesse geaendert hat (`_laeuferMarke`); waehrend der Abruf laeuft, gelten die alten Teile ausdruecklich NICHT — sie gehoeren zu einer anderen Maschine. Scheitert die Bauart, steht der Grund in `fehler` und die Leinwand zeichnet den nackten Laeufer weiter: ein Zeichner darf nie der Grund sein, warum die Oberflaeche stehenbleibt. **Vier Fehler, die erst ein BROWSER gezeigt hat** (19.09.2026, s. `BEFUNDE.md`). Geprueft war diese Vorschau bis dahin mit `node` und gestelltem Kontext plus einem aus dem JS-Raster gerenderten PNG — nie in einem Browser. Genau dazwischen lagen die vier: **(1)** Die Magnete haengen jetzt an der BAUART (`hat_magnete` aus `ema_maschinenart`, ueber `/laeuferbild`) und nicht mehr daran, ob Teile ankamen. Eine leere Teileliste heisst „nichts zu zeichnen", niemals „zeichne Magnete" — leer war sie aber genau in den interessanten Faellen (nicht auslegbarer Kaefig, Abruf unterwegs, Serverfehler), und dann stand die V-Anordnung einer PSM auf einem Asynchronlaeufer. **(2)** Ein nicht bemessbarer Kaefig wird GEZEICHNET statt verschwiegen: `ema_asm.kaefig` gibt die Geometrie ausdruecklich auch dann heraus („der Zeichner und das Netz brauchen etwas"), sie ist dann der Fertigungsboden, und genau das steht im Hinweis; das harte Nein bleibt beim Vernetzen (`ema_em2d_harm`) und beim CAD. **(3)** Kern, Spule und Nut sind RECHTECKE, Laeufer und Staender sind KREISE — ein Rechteck der halben Breite `y` mit der Oberkante bei `r` hat seine Ecken bei `sqrt(r²+y²)`. Geprueft wurde die Oberkante, gezeichnet wurden die Ecken; die EESM-Erregerspule stand damit gemessen bei r = 88,63 mm, waehrend die Statorbohrung bei 85,8 mm anfaengt — im Bild als „Rotor und Stator kollidieren" gemeldet. `_im_kreis` ist der Riegel dagegen, an der EINEN Stelle, die alle Bauarten durchlaufen. **(4)** Die GSM bekommt kein Drehfeld mehr: `staenderfeld: False` laesst die Drehstromnuten weg, die Staenderteile werden mitgerastert (Luft zwischen den Polen), und die Erregerspulen sind als GLEICHSTROM eingepraegt (`durchflutung_A` je Spulenseite, Wert aus `ema_eesm.erregung`; dieselbe Regel beim Schenkelpollaeufer der EESM). Das ist keine Erweiterung des Modells, sondern sein eigentlicher Fall — eine eingepraegte Gleichdurchflutung kann ein magnetostatischer Loeser exakt; nicht darstellbar bleibt, was ZEITABHAENGIG ist (Kaefig, kommutierter Anker). Beim Reluktanzlaeufer bleibt die Anordnung ausserdem BEDIENBAR und heisst dort „Flussbarrieren (Anordnung)": sie auszublenden liess einen IPM ohne Magnete stehen, ohne jeden Griff daran. Test: `test_laeuferbild.py` (90 Pruefungen; der Zeichner wird mit `node` und einem gestellten Kontext wirklich AUSGEFUEHRT und gezaehlt, was er malt — Muster `test_laeufer_cad.py`; dazu 56 Faelle ueber Bauart x Polzahl x Spulenform, in denen KEIN Teil mit seiner Ecke aus Laeufer oder Staender tritt, samt Gegenprobe, dass die alte Regel genau dort durchgegangen waere) |
| `ema_eesm_cad.py` — der Polkoerper laeuft bis an die Achse, `wickelraum` bemisst die Spule | **Der Schenkelpol sitzt nicht mehr auf einem Jochring — er IST bis zur Wellenbohrung durchgezogen** (19.09.2026, s. `BEFUNDE.md`). Vorher nahm ein Vollring 62 % des Laeuferhalbmessers ein und der Pol bekam den Rest; die Kernhoehe stieg damit von 26,5 auf **66,0 mm**. Das Joch ist seither das, was die Polkoerper nahe der Bohrung MITEINANDER bilden: zwei Koerper der halben Breite `w`, deren Achsen `2*pi/2p` auseinanderliegen, ueberdecken sich bis `r_nabe = w/sin(pi/2p)`. Das wird **gerechnet und benannt** (`r_nabe_mm`, `h_nabe_mm`, `B_nabe_T`, `nabe_reicht`, `nabe_zusammen`) und NICHT geheilt: gemessen traegt die Nabe den Fluss erst ab 2p = 12 (bei 2p = 8 stehen 9,36 gegen 12,57 mm Bedarf, B = 1,81 T gegen 1,70 T Blechgrenze). `_gate_laeufer` schreibt den Befund ins Protokoll, weist aber nichts ab — ein zu duennes Joch saettigt, es bricht nicht. Drei Fallen stecken in der Zeichnung und sind im Modulkopf festgehalten: die Stirnflaeche eines Arms ist eine **ebene Sehne** und erreicht den Bohrungskreis nur innerhalb von `acos(r0/r_welle)` um die eigene Achse (bei r0 = 18,5 und r_welle = 19 sind das ±13,2° gegen 22,5° halbe Polteilung — heraus kam ein VIELZAHN statt eines Kreises, deshalb wird bis 0,5 mm JENSEITS der Achse gezeichnet und die Bohrung zuletzt abgezogen); die **Spulentasche** darf dabei NICHT bis zur Bohrung durchgezogen werden (ihre halbe Breite ist 17,6 mm, die halbe Polteilung dort 7,9 mm — sie schnitte die Nachbarpole weg und zerstoerte genau die Nabe); und `b_pol` ist eine **Bogenlaenge**, die als kartesische Breite gedeckelt werden muss (bei 2p = 2 sind es 251,8 mm an einem 188,6-mm-Laeufer). **`wickelraum(geom, axial)` ist die EINE Quelle des Wickelraums** und ruft `erregung` ausdruecklich NICHT auf, damit `erregung` sie umgekehrt lesen kann. Drei Schranken, die engste gilt: Polteilung (waechst nach aussen, wird nach innen negativ), Polschuh (er haelt die Spule gegen die Fliehkraft) und Laeuferrand. Weil die ersten beiden linear bzw. konstant sind, ist ihr Minimum **konkav** — eine Sehne zwischen zwei Punkten darauf liegt ueberall darunter, und genau deshalb ist die kegelige Spule hier nicht Zierat, sondern die Form, die der Bauraum vorgibt; der Anfangsradius wird durchgefahren statt gesetzt (369 gegen 444 mm² am selben Pol). **Die analytische Fensterformel wusste von alldem nichts:** `ema_eesm._pol_fenster` rechnet ein Rechteck und kennt weder den Schuh noch das Schrumpfen der Teilung — 2024 mm² gegen 443 mm² zeichenbar |
| `geom.erregerSpuleFuellung` — was die Erregerspule bemisst | **`vorgabe`** (Standard): die Stromdichte (`fieldCurrentDensity`), das Fenster ist nur die Schranke — jede bestehende Auslegung rechnet Ziffer fuer Ziffer weiter. **`max`**: der GEZEICHNETE Bauraum. Das ist kein Schoenheitsschalter, sondern eine Auslegungsentscheidung: `P_f = F_pol^2 * rho * l_w / A_cu` faellt mit 1/A_cu, dafuer steigt die Kupfermasse. Gemessen (2p = 8, Rotor 188,6, Welle 38, Polbedeckung 0,55): Kupfer je Pol **106 → 184 mm²**, Stromdichte **5,00 → 2,87 A/mm²**, Erregerverlust **154,1 → 88,6 W**, Laeuferverlust **218,1 → 152,6 W**, Erregerwicklung **2,32 → 4,04 kg**, Laeufermasse **29,99 → 31,71 kg**; die Spule wird von 19,45 auf 38,16 mm hoch. **Das „Dach" darf mitwachsen — aber nur, soweit es sich rechnet:** `wickelraum_max` faehrt die Polbedeckung ueber ihre erlaubte Spanne und misst. Ein breiterer Schuh ueberdeckt eine dickere Spule, zwingt aber zugleich einen breiteren Kern (er nimmt mehr Fluss auf), und der frisst den Platz fast so schnell, wie der Ueberstand ihn gewinnt: 0,55 → 0,58 bringt 443 → 446 mm², also 0,6 %. Nach unten wird nicht gesucht — die Polbedeckung ist eine magnetische Entscheidung des Menschen, hier darf sie nur FOLGEN. Wo `max` nicht geht (sehr wenige, sehr breite Pole), wird nicht abgewiesen, sondern auf die Stromdichte zurueckgefallen und das gesagt (`fuellung_hinweis`). Bedienbar im Geometrie-Reiter unter **Maschinenart** (`spule_fuellung`, `_ART_FELDER`), ueber `cae_cli --set` und in der Parameter-Tabelle. **Was dabei SCHLECHTER wird, steht mit im Ergebnis:** der kegelige Kern ist am Bohrungsrand schmaler, also wird die Nabe kleiner (9,36 → 4,92 mm, B 1,81 → 3,64 T) |
| `bau_eesm.py` — die Werkbank | **Ein zweiter, unabhaengiger Zeichner fuer denselben Laeufer**, ausserhalb der Pipeline: `python bau_eesm.py --p 4 --spule kegel --fuellung max --gui` baut NUR den Schenkelpollaeufer, misst ihn am fertigen Koerper nach (Solids, Volumen, Radien, Flaechen, **paarweise `common().Volume`** gegen Durchdringung samt Gegenprobe, wieviel Eisen der Spulenschnitt entfernt hat) und oeffnet ihn in FreeCAD. Er rechnet **keine** Geometrie, sondern liest `ema_eesm_cad`/`ema_schenkelpol`/`ema_schleifring` — und genau das macht ihn zur Driftprobe: Werkbank und `ema_freecad` bauen aus derselben Quelle und muessen **Ziffer fuer Ziffer** dasselbe Volumen liefern. Genau dort fiel auf, dass `_pol_koerper` den kegeligen Kern mit der Breite am Spulenanfang statt auf der Kerngeraden baute (775 mm³ Unterschied), und dass die Innenschneider von Spule und Tasche ihre Breite interpolierten statt dieselbe Gerade auszuwerten (102,45 mm³ Kupfer im Blech). `--referenzbild` misst zusaetzlich ein fremdes Schnittbild aus (Farbsegmentierung → dimensionslose Verhaeltnisse) und stellt es daneben |
| `ema_eesm_cad.py` — Deckung, Jochplatz, Kegelwicklung | **Der Polschuh haelt die Spule, und das ist jetzt eine Bedingung statt eines Zufalls.** Die Kernbreite kam aus einem festen Verhaeltnis (0,72 x Schuhbreite); ob darunter noch Platz fuer die Spule blieb, ergab sich nebenbei. Der Schuh hat aber genau diese Aufgabe — „salient pole tips overhang the pole body to secure the field coil against centrifugal force“ (US3089049A). Der Kern gehorcht deshalb **drei Schranken**, und es gilt die engste: **Deckung** (Schuh ueber Spule plus `SCHUH_UEBERSTAND_MM`), **Platz am JOCH** und die alte obere Schranke; `bindend` sagt welche. **Der Jochplatz war der eigentliche Fund:** geprueft wurde nur am Kernradius, wo reichlich Platz ist — die Polteilung ist aber am Joch am kleinsten, und gemessen durchdringen sich ab acht Polen die Spulen benachbarter Pole dort um 3,9 mm. **Kegelige Wicklung** (`geom.erregerSpuleForm = kegel`): der Pol ist an der Jochseite breiter und die Wicklung folgt seiner Neigung (WO2006026200A1); `rechteck` bleibt Vorgabe und laesst jede bisherige Zeichnung Ziffer fuer Ziffer stehen, der Kegel bringt gemessen +3,0 % Laeufereisen. Alle drei Zeichner — CAD, Querschnittsbild, Leinwand — lesen dieselben zwei Breiten. **Und seit dem 19.09.2026 auch denselben SPULENRADIUS:** die Spule sitzt NEBEN dem Kern, ist also ein Rechteck im Kreis, und mit der Kernoberkante als Oberkante laege ihre Ecke ausserhalb des Laeufers — gemessen 88,63 mm gegen eine Statorbohrung bei 85,8 mm, im Bild als „Rotor und Stator kollidieren" gemeldet. `_spule_einpassen` loest Dicke und Hoehe deshalb GEKOPPELT (`d·h` traegt den Wickelraum UND die Ecke liegt auf `r_rot - RAND_LUFT_MM`); die Gleichung ist nicht monoton — eine dickere Spule muss tiefer sitzen und wird dadurch kuerzer —, genommen wird die KLEINERE Wurzel, also die duennere, hoehere Spule naeher am Polschuh, der sie ja halten soll. Gibt es keine Wurzel, passt der Wickelraum ueberhaupt nicht unter den Rand, und das steht als `grund` da statt als gezeichnete Spule im Staender. Dazu eine vierte Schranke `rotorrand` fuer den Kern (`b ≤ 2·sqrt(r_max² − r_kern²)`), die als einzige das Mindestmass `KERN_MIN_ANTEIL` ueberstimmen darf: ein zu schmaler Kern ist ein Befund, ein Kern ausserhalb des Laeufers eine falsche Zeichnung. Neu heraus kommen `r_spule_aussen_mm`, `b_kern_spulenende_mm`, `r_ecke_kern_mm`/`r_ecke_spule_mm` und `im_laeufer`. **Nebenbefund, mitbehoben:** `grund` stand als `(…) + "" if True else (…)` da — der Ausdruck bindet als `(a + "") if True else b`, womit zwei der drei Begruendungen unerreichbar waren und jede Ablehnung denselben Text trug |
| `ema_freecad._pol_spule` — die Spule ist GESCHLOSSEN | Vorher zwei getrennte Quader links und rechts des Kerns: das sind zwei Leiterstaebe, keine Wicklung — die Stuecke an den STIRNSEITEN (die Wickelkoepfe) fehlten ganz. Jetzt Aussenkoerper minus Innenkoerper, nachgemessen in FreeCAD **10 Flaechen, eine Schale, Fuellgrad 0,17** (ein Rahmen mit Loch) und **ein** Festkoerper je Pol statt zweier loser Bloecke. Damit erledigt sich auch der gemeldete Eindruck, „der Rotor bestehe aus verschiedenen Bodies“: `Rotor` war immer schon EIN gueltiger Solid, die losen Koerper waren die Spulen. **Kein Docstring in dieser Funktion** — der Block wird als Text in das FreeCAD-Skript geschrieben, und ein dreifaches Anfuehrungszeichen beendet dort die umschliessende f-Zeichenkette (zum zweiten Mal passiert, steht jetzt als Warnung darueber) |
| `ema_gsm.py` | **Die Gleichstrommaschine — als fuenfte Art, und wirklich gerechnet.** Ein Kommutator, der nur gezeichnet ist, liefert Zahlen unter falschem Namen; das Modul fuehrt deshalb die Hauskonvention (`k_norm`, `betriebspunkt`, `verluste`, `dauermoment`, `massen_und_kosten`) wie `ema_asm`/`ema_eesm`/`ema_synrm`. **`k_norm` gibt 1,0 zurueck und sagt warum:** die Normierungsbruecke `pi*k_w*N_ph/p²` gilt fuer Drehfeldmaschinen, bei denen `compute_performance` auf eine Windung je Nut normiert — hier ist `k = p*z/(2*pi*a)` geometrisch exakt, und dieselbe Konstante traegt `E = k*Phi*omega` UND `T = k*Phi*I_a`, womit `E*I_a = T*omega` **exakt** gilt (was eine Formel wirklich prueft, statt eine Zahl festzunageln). Zweigzahl `a = p` bei Schleifen-, `a = 1` bei Wellenwicklung: gleiche Leistung, anderer Strom/Spannungs-Teiler. **`kommutierung` ist das eigentliche Tor** — nicht das Eisen begrenzt eine Gleichstrommaschine, sondern mittlere Lamellenspannung (~18 V), Spitzenwert (~35 V, Rundfeuer) und Umfangsgeschwindigkeit am Kommutator (~40 m/s); es sagt, **welche der drei bindet**, und traegt je Grenze ihre Ausnutzung. **Die Lamellenspannung haengt NICHT an der Leiterzahl, und das ist eine Aussage ueber die Maschine:** mit `U_l = 2a*E/k`, `E ~ z` und `k ~ z` kuerzt sich z heraus, uebrig bleibt `U_l = 2*r*B*alpha*L*omega` — mehr Lamellen entlasten den Kommutator nicht, nur weniger Fluss, Laenge oder Drehzahl. Welche Grenze bindet, haengt damit an der Bauform, und der Uebergang ist scharf (gemessen, 6000 1/min): bei 100 mm Paket bindet die Umfangsgeschwindigkeit bei 96 %, bei 300 mm reisst die Lamellenspannung mit 38,96 V gegen 35. **Die Ankerrueckwirkung wird BENANNT und mit einem Abschlag gerechnet** (`geschaetzt: True`) — Buerstenverschiebung, Wendepole und Kompensationswicklung sind ausdruecklich NICHT modelliert, damit niemand die Zahl fuer mehr haelt, als sie ist. Der Buerstenabfall kommt aus `ema_schleifring.U_BUERSTE_V`, keine zweite Konstante — und er ist am gemessenen Auslegungspunkt (p=2, 24 Nuten, 4 Leiter/Nut, L=100) mit **1488,6 W groesser als das ganze Ankerkupfer (793,9 W)**: bei 744 A Ankerstrom sind zwei Kohlekontakte kein Nebenposten. **Die GSM ist die EESM von innen nach aussen** — die Schenkelpole sitzen am STAENDER, und `staenderpole` ruft dafuer `ema_eesm.polmasse` mit Statorradien statt einer zweiten Polformel. `stufen = ('analytisch', 'cad')`, `feldweg = 'keiner'`: der FDM ist reell, linear und magnetostatisch und kann einen kommutierten Anker so wenig darstellen wie einen Kaefig, und `ohne_bedeutung` traegt die dq-Groessen — eine Gleichstrommaschine hat keine d- und q-Achse, und eine 0 dort laese sich als Messwert. Gezeichnet wird ueber `ema_freecad` (Anker mit Nuten und Wicklung, Kommutator als **k einzelne Lamellen**, 2p Buersten, Staender aus Joch + einwaerts gerichteten Polen zu EINEM Solid verschmolzen, **keine Statornuten und keine Hairpins**). **Masse und Kosten entstehen aus den EIGENEN Teilen** — Ankereisen abzueglich der Ankernuten, Staenderjoch + Schenkelpole OHNE Nutfaktor, Anker- und Erregerkupfer: die Basis aus `ema_screen` rechnet eine Drehfeldmaschine (Magnete, genuteter Ring, Strangkupfer), und sie zu uebernehmen und nur `magnet_kg` auf 0 zu setzen liess die GSM gemessen mit Ziffer fuer Ziffer derselben Masse und denselben Kosten wie die PSM dastehen. Ebenso traegt `ARTEN['gsm'].ohne_bedeutung` die **`_STAENDERNUT_KENNZAHLEN`**: `ema_saettigung` rechnet die Zahnflussdichte ueber die Nutteilung, und ein Staender aus Schenkelpolen hat keine — der Paarvergleich fragt jetzt `gilt()` und laesst die Spalte leer, statt 8,885 T hinzuschreiben (denselben Wert wie ASM und EESM, weil die Formel nur an `B_gap` und `slotWidthRatio` haengt). Test: `test_gsm.py` (72 Pruefungen ohne Loeser) |
| Der Geometrie-Reiter: was JEDE Maschine hat und was nur EINE (`.art-feld`, `_ART_FELDER`) | **Die Polzahl war bei drei von fuenf Bauarten nicht einstellbar.** Sie stand im Abschnitt „Magnet-Topologie", und der wird bei einer Art ohne Magnete ausgeblendet — fuer ASM, EESM und GSM gab es damit kein Bedienelement fuer `p`. Gemeldet als „wie stelle ich bei der EESM die Anzahl der Pole ein". Sie gehoert aber JEDER Maschine und steht jetzt bei der **Maschinenart**, mit der abgeleiteten Polzahl daneben (`disp_poles`, denn in der Maschine zaehlt 2p). Hauptabmessungen und Stator/Wicklung bleiben unveraendert immer sichtbar. **Alles, was nur EINE Bauart hat, sitzt seither unter der Maschinenart** und wird ueber `data-arten` ein- und ausgeblendet: Ziel-Luftspaltfeld (ASM/EESM/GSM — bei der PSM steht das Feld durch den Magneten fest, bei jeder fremderregten Bauart wird es eingestellt), Laeuferbauform/Laeufernutzahl/Schraegung/Leiter je Laeufernut und die beiden Laeuferwerkstoffe (ASM), Polbedeckung/Polbefestigung/Schrauben/Gewinde/Daempferkaefig samt Stabzahl und Werkstoff (EESM), Ankerwicklungsart und Ankerwerkstoff (GSM), Erregerwicklung und Spulenform (EESM+GSM), Leitertemperatur (alle). **`_ART_FELDER` ist die EINE Tabelle Elementkennung → Schemaschluessel**: gefuellt, gebunden UND in `applyPayload` zurueckgeschrieben aus derselben Zuordnung. Grenzen, Auswahllisten und Vorgaben kommen aus `/param_schema` und stehen nicht ein zweites Mal im HTML — sonst ist ein Wert waehlbar, den `--set` und die Pipeline abweisen; und eine zweite Aufzaehlung in `applyPayload` war schon einmal die Stelle, an der ein Feld gesetzt, aber nie wiederhergestellt wurde (`struct_solver`). `test_maschinenart.py` prueft die LAGE im Dokument (`p_pairs` zwischen Maschinenart und Magnet-Topologie), dass es zu jedem Eintrag ein Bedienelement UND einen Schemaschluessel gibt, und dass die Polbedeckung des Schenkelpols NICHT `poleArcFrac` ist |
| `ema_schenkelpol.py` | **Was den Schenkelpol HAELT und was ihn DAEMPFT** — zwei Dinge, die bis zum 19.09.2026 nur als Luecke benannt waren (`ema_eesm_cad`s `ungeprueft` bzw. „keine Daempferwicklung" im Kopf von `ema_eesm`) und sonst nirgends vorkamen. **`befestigung`**: der Schenkelpol ist das einzige Bauteil dieses Werkzeugs, das NICHT aus dem Vollen kommt — er sitzt auf dem Joch und wird dort gehalten. Gewogen wird der ganze Pol (Schuh + Kern + Spule, aus `ema_eesm_cad.koerper`, also aus derselben Funktion, aus der gezeichnet wird: gemessen 1,49 kg bei Schwerpunkt r = 71,8 mm), gerechnet werden BEIDE Bauarten und benannt wird die bindende: `schwalbenschwanz` (der Hals traegt auf ZUG, `sigma = F/(b_hals·L)·KT_HALS` gegen `yield_mpa/SF`, Kerbfaktor 1,8 nach dem Muster von `ema_rotorcheck.KT_POCKET`) und `bolzen` (Spannungsquerschnitt nach ISO 898-1, Klasse 8.8). Der Unterschied ist keine Feinheit: bei 12.000 1/min zieht derselbe Pol mit **169 kN**, der Hals kommt auf SF 2,62, die Vorgabe-Verschraubung 2 x M10 auf **SF 0,44** — und das Nein nennt „17 x M10" statt nur abzusagen. **`n_zulaessig` ist drehzahlUNABHAENGIG**, und das ist zugleich die Probe: sigma ~ n², also hebt `sqrt(SF)` das n davor exakt auf (gemessen 17.052 1/min bei 3000, 6000 und 12.000). **`daempferkaefig`**: Staebe im Polschuh, bemessen nach einer **benannten Auslegungsregel** (Querschnitt je Pol = 0,25 x Staenderkupfer je Pol, Stabteilung = 0,80 x Staendernutteilung; Say, *Alternating Current Machines* / IEEE Std 115) — sie steht als `regel` im Ergebnis und weist sich als Regel aus, nicht als Messung. Die Teilung ist bewusst VERSCHIEDEN von der Nutteilung, sonst bleibt der Laeufer beim asynchronen Anlauf in einer Oberwelle haengen (Goerges-Sattel); laeuft sie doch in die Naehe von 1 — etwa weil die Stabzahl gedeckelt ist —, steht das als Hinweis da. Widerstand ueber `ema_pipeline.rho_bei` bei der Leitertemperatur wie jede andere Wicklung hier. **Was der Daempfer NUETZT, geht in keine Kennzahl ein**: asynchroner Anlauf und Pendeldaempfung brauchen einen zeitabhaengigen Lauf, den es hier nicht gibt — gerechnet wird, was er KOSTET (Masse, Kosten, Platz im Schuh). Genau diese Asymmetrie steht als ⓘ unter der Paarvergleichsoption, damit „2:0 fuer ohne Daempfer" nicht als Urteil ueber den Daempfer gelesen wird. Angeschlossen: `ema_pipeline._gate_laeufer` (Tor, mit Begruendung und Entlastungsweg), `cae_cli rotor-check`, die Paarvergleichsachsen `polbefestigung`/`daempferkaefig`, `ema_eesm.massen_und_kosten` (der Kaefig ist echtes Kupfer), Leinwand, Querschnittsbild und CAD (`Damper_Cage` samt Bohrungen — aber **nur wenn er passt**: Staebe zu zeichnen, die das Tor abgewiesen hat, waere eine Zeichnung, die der Rechnung widerspricht), `ema_werkzeugstand.PHYSIK`. Test: `test_schenkelpol.py` (46 Pruefungen ohne Loeser — gegen INVARIANTEN statt festgenagelter Zahlen: vierfache Kraft bei doppelter Drehzahl, dreifache Sicherheit bei dreifacher Schraubenzahl, der Querschnitt trifft seine eigene Regel, und das Urteil kippt genau an der ausgerechneten Grenzdrehzahl) |
| Schleifring und Kommutator als BAUTEIL (`ema_eesm.schleifringe`, `ema_gsm.kommutator`) | Beide wurden gerechnet und gezeichnet — aber nur als Nebenwirkung: `ema_freecad` rief `ema_schleifring` direkt, `ema_eesm` hatte gar keine eigene Funktion dafuer, und der Kommutator existierte ausschliesslich als **Grenze** (`kommutierung`), nie als Gegenstand. In keiner Stueckliste, keinem Steckbrief, keinem Paarvergleich. **Die Polbedeckung des Schenkelpols ist seit dem 19.09.2026 einstellbar** (`geom.polbedeckung`, `ema_eesm.polbedeckung` als einzige Quelle, Vorgabe bleibt `POLBEDECKUNG` = 0,68): sie war eine Konstante und damit ein Wert ohne Griff, obwohl sie ueber Grundwelle, Erregerdurchflutung, Kupfermasse und den Platz fuer die Spule entscheidet (0,68 → 0,80 senkt `F_pol` von 459 auf 423 A). Ein EIGENER Parameter und ausdruecklich nicht `poleArcFrac` — das ist die Magnetbedeckung der PSM (0,83), ein Schenkelpol liegt bei 0,65…0,75; dieselbe Zahl fuer beide waere eine Gleichsetzung, die es nicht gibt. **`ema_eesm.schleifringe`** liefert die ZWEI Ringe des Erregerkreises (die ASM fuehrt drei fuer ihre Drehstromwicklung) mit dem Strom aus `erregung` — nicht noch einmal geschaetzt, denn `I_f` haengt an der Wickelentscheidung. **`ema_gsm.kommutator`** ist die Beschreibung neben der Grenze: Lamellenteilung und -breite, Buerstenarme (Schleifenwicklung 2p, Wellenwicklung 2), Kontaktflaeche, Buerstenmasse, Verlust — und die **Baulaenge**. Die war bis dahin `L_KOMM_ANTEIL x Paketlaenge`, also eine ZEICHENregel: gemessen braucht ein Ankerstrom von 800 A bei 10 A/cm² ganze 80 cm² Kontaktflaeche und damit 149,6 mm Buerstenlaenge, waehrend der gezeichnete Kommutator 24 mm lang war — **Faktor 6**, und nichts widersprach. Die Buerstenflaeche bemisst den Kommutator, nicht umgekehrt; die alte Regel bleibt als Boden. Wird er dadurch laenger als das Blechpaket, ist das ein **Befund** und keine Ablehnung (bei Gleichstrommaschinen ueblich, verlaengert die Maschine aber um genau diesen Betrag). `zeichenmasse` liest dieselbe Funktion — sonst saesse die gerechnete Buerste auf einem anderen Kommutator als der gezeichnete. Stromdichte und Kontaktabfall kommen aus `ema_schleifring`: ein Kohlekontakt ist ein Kohlekontakt, ob er auf einem Ring oder auf Lamellen laeuft, und zwei Tabellen waeren zwei verschieden grosse Buersten fuer denselben Strom. Test: `test_schenkelpol.py` §5 |
| `ema_schleifring.py` | **Strom auf einen drehenden Laeufer bringen — EINE Quelle fuer alle, die das brauchen.** Ein Schleifring gehoert weder der Asynchron- noch der Synchronmaschine: die ASM fuehrt **drei** (Drehstromwicklung auf den Anlasswiderstand), die EESM **zwei** (Erregerkreis, Gleichstrom). Die Geometrie ist dieselbe — Ring auf der Welle, Breite aus der **Stromdichte am Buerstenkontakt** (`J_BUERSTE_APCM2`, Auslegungswert) —, und zwei Fassungen waeren zwei verschieden breite Ringe fuer denselben Strom. `V_RING_MAX_MPS` (45 m/s) ist dagegen eine **Grenze**: darueber traegt der Kohlekontakt nicht mehr (Buerstenfeuer, Abbrand), und dann ist ein Schleifringlaeufer die falsche Wahl, egal wie gut er sonst passt. Ohne bekannte Hoechstdrehzahl steht `v_ok: None` da und ausdruecklich nicht „ok". `U_BUERSTE_V` ist derselbe Wert wie in `ema_eesm`; der Test haelt die beiden gleich |
...
| `ema_grenzen.py` | **Die Grenzen, die IMMER gelten — und die Probe, dass CAD und Feld dieselbe Maschine meinen.** `LUFTSPALT_MM = (0.1, 2.0)` ist die einzige Quelle fuer den zulaessigen Luftspalt (vorher vier verschiedene Baender, s. Stufe 0); `pruefe_luftspalt`/`pruefe_hairpin`/`pruefe_nuttiefe` sammeln ALLE Befunde statt beim ersten abzubrechen, `pruefe_hart` wirft `GrenzeVerletzt`. **`cad_gegen_feld(geom, axial)`** stellt die Zahlen BEIDER Geometriewege nebeneinander — was `ema_freecad` zeichnet gegen das, was `ema_analysis._rasterise`/`ema_em3d.slot_rects` rastert — und trennt dabei drei Faelle: gleich · aus EINER Quelle (dort KANN nichts driften, `magnet_legs`, `ema_asm.kaefig` — eine Zeile „stimmt ueberein" waere dort Beruhigung ohne Aussage) · gewollt verschieden. Zwei gewollte stehen benannt da: die Nut ist im CAD ein Rechteck fester Breite und im Feld ein Winkelsektor (am Bohrungsrand 8,290 gegen 8,288 mm, am Nutgrund 8,290 gegen 10,469 — der Zahn ist im Feld schmaler als gezeichnet), und die 3-D-Stufe fuehrt den Statorleiter gerade bis auf den Rand statt als Krone. CLI: `rotor-check --cad-feld`. Test: `test_grenzen.py` |
| `ema_wicklung.hairpin_radien` | **Wie weit die Wicklung radial reicht — importierbar.** Die Staebe koennen die Nut nicht verlassen, die Krone faechert aber auf: Kronenseite `f = wh_flare + H_eff·tan((k+1)·spread)` JE LAGE, Schweissseite `f_weld = H_w·tan((k//2+1)·spread)` PAARWEISE (beide Beine eines Schweisspaares brauchen dieselbe Aufweitung, sonst treffen sich die zu verschweissenden Enden nicht), dazu die Isolierhuelse. Existiert damit **zweimal** — hier als Python, im CAD-Erzeuger als Text fuer einen fremden Prozess; `test_grenzen.py` fuehrt die EMITTIERTE Mathematik wirklich aus und vergleicht sie Lage fuer Lage. Genau das fand einen Fehler: der Erzeuger backte `cond_w`/`layer_h`/`slot_w` mit **vier Nachkommastellen** als Literal ins Skript, die Zeichnung war gegenueber dem Modell also quantisiert (5·10⁻⁶ mm je Lage) und ein exakter Anker unmoeglich. Jetzt `!r`, volle Genauigkeit |
...
| `ema_analysis.py` | 2D-FDM EM field solver + analytical performance (torque, EMF, saliency, d/q currents). **`eckdrehzahl(geom, b_gap_t, v_dc)` ist die EINE Quelle der Eckdrehzahl** (seit 15.09.2026): sie entstand vorher an vier Stellen verschieden — `estimate_dq_currents` rechnete sie inline, `server._rpm_base_von` hatte eine Abschrift, und `ema_pipeline` **erbte** sie aus der ersten Zeile der Bilder-Drehzahlliste (`sweep_rpms[0]`). Die letzte Fassung verstellte auf einem WLTP-Lauf den Eisenverlust um Faktor 0,20 und machte 98,9 % des ausgewiesenen Kupferverlusts zu einem Feldschwächstrom, den es nicht gibt. `ECKDREHZAHL_ERSATZ` ist der benannte Ersatzwert (stand dreimal nackt als `5000.0` da). S. `BEFUNDE.md`, 15.09.2026 |
...
| `ema_drivecycle.py` | Drive cycles (WLTP-3b, full-load, trailer, CSV) + drivetrain + energy integration. **Der Profilabschluss ist EINE Funktion** (`_profil_abschluss`, seit 15.09.2026): alle drei synthetischen Bauer legten `np.zeros(TOTAL_T + 1)` an und füllten die Phasen über `[t_start, t_end)` — der **letzte Abtastwert wurde nie beschrieben** und blieb 0, also −36 m/s² in einer Sekunde; die zweimalige Glättung mit `mode="same"` (nullgepolstert) verteilte den Sprung und machte ihn dadurch unauffällig, nicht kleiner. Gemessen kam daraus die **Momentenspitze einer ganzen Auslegung** (153,1 Nm bei t = 1800 s statt 45,3 Nm im Profil). Jetzt: letzten Wert füllen → randfortgesetzt glätten (`_glaetten`) → mit zulässiger Verzögerung **ausrollen** (`_ausrollen`, ein Zyklus endet im Stand) → auf `BESCHLEUNIGUNG_BAND` klemmen (`_rate_begrenzen`). Die Reihenfolge ist nicht beliebig: glättet man nach dem Ausrollen und setzt danach `v[-1] = 0`, reißt derselbe Sprung wieder auf. WLTP trifft die Sollstrecke danach besser als vorher (23,33 gegen 23,26 km Soll). **`FAHRZEUG_GRENZEN`/`fahrzeug_pruefen`** sind die eine Quelle für die Grenzen des Fahrzeugblocks — Überschreitungen werden **benannt**, nicht geklemmt (geklemmt sähe der Wert wie ein angenommener aus) und nicht abgewiesen (dann wäre ein bestehendes Projekt unrechenbar); das Feld `r_wheel_m` ist ein **Halbmesser** und hieß in `ema.html` bis dahin „Raddurchmesser". `cycle_energy` nimmt `rpm_max_zul` und liefert `speed_warning` — die Überlastprüfung sah bis dahin nur das Moment, und `overload_warning` wurde nirgends ausgegeben. Trailer is user-parametric: `trailer_mountain_cycle(max_grade_pct)` (steepest uphill = that grade %, slope arrays stored in **degrees**) + `trailer_vehicle(base, trailer_mass_kg, n_axles)`; the `/analyse` payload carries `trailer:{mass_kg,n_axles,grade_pct}` |
...
| `ema_chat.py` (Route `/chat`, `/recherche`, `/project/<id>/gespraeche`) | **Ueber die Ergebnisse reden — und nachschlagen lassen.** Der Chat erdet auf einer `_machine_datasheet(meta)`-Spezifikation plus RAG (`_rag_doku`, Projekt-Store zuerst, dann die globale Basis) und kompaktiert `results.json` (`_compact` streicht base64, Frames und lange Zahlenreihen, 1 MB -> wenige kB). Drei Maengel fielen erst auf, als jemand ihn benutzen wollte, um ein Ergebnis zu BEZWEIFELN, und alle drei sind behoben (12.09.2026): **(1) Der Verlauf lebte nur im Browser** — ein Neuladen loeschte das Gespraech, und im Projekt stand davon nichts; jetzt anhaengend unter `<projekt>/gespraeche/<kennung>.jsonl` (eine Datei je Gespraech, damit zwei Fragestraenge nebeneinander stehen koennen), und `/chat` zieht ihn nach, wenn der Browser keinen schickt. **(2) Er sah sechs handverlesene `results`-Zweige** — em3d, Getriebe, Parameterstudien, Sicherheitsbefund und vor allem die **Herkunft** je Kennzahl kamen darin gar nicht vor, auf „woher kommt diese Zahl?" gab es keine Antwort; jetzt ist der **Steckbrief** die Grundlage (`_projektgrundlage`, dazu `AUFTRAG.md`), und die Aufforderung verlangt die Herkunft ausdruecklich mit — gemessen antwortet er darauf jetzt „0,63 T, Herkunft analytisch (geschlossene Formel, kein Feldlauf)". **(3) Er konnte nichts nachsehen** — jetzt eine kleine, bewusst nur **lesende** Werkzeugschleife (`suche`, `hole`, `steckbrief`, `aehnlich`) ueber das Format `WERKZEUG: <name> <argument>`. **Drei Dinge daran sind nicht beliebig:** der Aufruf zaehlt nur, wenn er (nahezu) die GANZE Antwort ist — sonst loeste ein Modell, das seinen eigenen Aufruf bloss erklaert, ungewollt eine Suche aus; ein gescheitertes Werkzeug ist ein ERGEBNIS und keine Ausnahme (das Modell soll erfahren, dass die Seite nicht erreichbar war, statt in eine leere Antwort zu laufen); und kommt das Modell aus der Schleife nicht heraus, wird das GESAGT statt die rohe Protokollzeile hinzustellen — ein oertliches Modell ist im Werkzeugaufruf unzuverlaessig, und das ist das Schlechteste von beidem. Alles RECHNENDE fehlt hier mit Absicht: ein Pipelinelauf dauert Stunden, und der Knopf **„🔬 nachpruefen lassen"** gibt die Frage samt stehendem Auftrag („pruefe das nach, nenne die Belege, erfinde nichts") an einen Agentenkopf — laeuft schon einer, als Zwischenruf (`merken`) statt als zweiter Start. **Und die Antwort wird nachgeprueft:** `deckung_pruefen` laesst `ema_beitrag.ungedeckte_zahlen` ueber sie laufen (runden ist gedeckt, erfinden nicht) — eine Bitte an ein Sprachmodell ist keine Zusicherung. Zwei Messungen daran: Werkzeugergebnisse gehoeren ins Material (was das Werkzeug GELIEFERT hat, ist gedeckt, sonst waere die Pruefung genau dort blind, wo sie am meisten taugt), und eine **Projektkennung ist keine Behauptung** — `20260908_103936` zerfiel in `20260908` und `103936` und wurde als erfundene Zahl gemeldet; eine Pruefung mit sechs Falschmeldungen je Projektliste wird ignoriert, und dann faengt sie auch die echte nicht mehr. `/chat` nimmt jetzt `project_id` statt am global aktiven `_state` zu haengen: wer in einem zweiten Reiter umschaltete, verschob den Gegenstand des Gespraechs unter der Hand. `/recherche` ist die fehlende Route zu `ema_recherche` (es gab es laengst, aber nur ueber einen Agentenkopf erreichbar); was daraus kommt, ist **Fremdtext und keine Zahl**. Test: `test_chat_werkzeuge.py` |
| `ema_optimize.py` | LLM-steered target-value optimisation. **Das Layouttor sitzt im gemeinsamen Bewertungskern** (`_baubar` in `_eval_geom`, seit 13.09.2026): `_violation` kannte nur die vom Menschen gesetzten Kennzahl-Grenzen und **keine geometrische Zulaessigkeit**. Weil `_analytical_Bgap` die Polbedeckung als `n_legs*magWidth/pole_pitch` rechnet — ein VERHAELTNIS —, waechst sie weiter, wenn die Magnettaschen benachbarter Pole einander schon durchdringen: **die Zielgroesse belohnte das Unbaubare**. Gemessen an einer 305-mm-Maschine stieg Kt von 0,314 am letzten baubaren Punkt auf 0,401 jenseits des Tors (+28 %), und eine Suche auf `max Kt` haette genau den Rotor mit ueberlappenden Taschen zum Sieger erklaert. Der Deckel `alpha_i <= 0,92` rettet das nicht verlaesslich — er faellt je nach Geometrie vor oder nach das Tor. Jetzt rangiert `baubar is False` wie eine unerreichbare Auslegung (1e8: unter jeder gueltigen Loesung, ueber einem echten Fehler), und weil die Pruefung im **geteilten** Kern sitzt, sehen sie Parameterstudie, Zielwertsuche und Magnet-Feinschliff zugleich statt eine von dreien. Kosten: reine 2-D-Algebra neben einem FDM-Lauf. S. `BEFUNDE.md`. `evaluate_fast` scores a candidate WITHOUT FreeCAD/FEM (EM at low N + analytical Kt/torque + steady LPTN thermal + analytical struct sweep + analytical mass, ~0.5 s). Its inner geom→metrics core is factored into **`_eval_geom(geom, axial, mats, op, …)`** (works for ANY `magShape` incl. `"custom"`); `evaluate_fast` = `_apply_params` (parametric `FREE_PARAMS`) then `_eval_geom`. The per-magnet optimiser (`ema_design_optimize`) reuses `_eval_geom`/`_fitness`/`_violation`/`_ollama_chat`/`_extract_array`. `optimize(spec)` seeds + lets the LLM propose batches over `FREE_PARAMS`, deterministic clamp + feasibility/fitness pick the best feasible design. Served by `POST /optimize` (threaded, `_opt_state`, body factored into `_optimize_body`) + `GET /optimize/status` + `GET /optimize/meta`; UI is the `🎯 Zielwertoptimierung` modal (Berechnung tab / `#optimize`). "Übernehmen" applies the best params to the form for a final full pipeline run. **Als Job:** „➕ Warteschlange" (`queueOptimize`, Job-Typ `optimize` → `_exec_optimize`) reiht die Optimierung server-seitig ein (läuft bei geschlossenem Browser); das Ergebnis liegt in `_opt_state`, der Jobs-Knopf „🎯 Ansehen" (`openJobOptimize`) öffnet das Modal + rendert es |
| `ema_paramstudy.py` | **Parameterstudie bei fester Drehzahl**: `run_study(payload, param, lo, hi, steps=100, rpm)` sweeps ONE `ema_optimize.FREE_PARAMS` parameter x→y in N steps at a fixed speed and plots EVERY result metric over the parameter (small-multiples grid). Reuses `ema_optimize.evaluate_fast` (FreeCAD/FEM-free, geometry varied per step), so 100 points cost ~50 s. Served by `POST /param_study` (threaded, `_study_state`, body factored into `_param_study_body`) + `GET /param_study/status`; UI is the **„📈 Parameterstudie"** panel (Berechnung tab) — parameter dropdown filled from `/optimize/meta`. **Als Job:** „➕ Warteschlange" (`queueParamStudy`, Job-Typ `param_study` → `_exec_param_study`) reiht die Studie server-seitig ein; das Ergebnis liegt in `_study_state`, der Jobs-Knopf „📈 Ansehen" (`openJobStudy`) öffnet Tab ④ + rendert es (`_studyRenderDone`). **Custom (Designer/KI) designs:** `run_study` honours `geom.customLegs` (they ride through `evaluate_fast` unchanged), so the Designer-Tab button **„📈 Parameterstudie für diesen Entwurf"** (`dsnParamStudy`) runs the same study on the drawn geometry; the UI then restricts the dropdown to the geometry-effective params (`_CUSTOM_STUDY_PARAMS` = axial/airgap/slotDepth/p/magGap — magnet-shape params are no-ops on freehand magnets) and shows a banner (`_studyDesignPayload`, reset via `dsnClearStudyDesign`) | **Jede Studie liegt in ihrem EIGENEN Ordner** (`studie_anlegen`/`ablegen`/`liste`/`laden`/`loeschen`): bis zum 12.09.2026 schrieben ALLE Studien in denselben Ordner (`server.STUDY_FIELD_DIR`), und `_render_field_series` raeumte ihn zu Beginn jedes Laufs leer — die zweite Studie loeschte Bilder und Video der ersten. Schlimmer: das Ergebnis selbst (Zahlen, Diagramm, CSV) lag ueberhaupt nur im Serverspeicher (`_study_state`), war nach einem Neustart weg und liess sich nie wieder ansehen; aus der Sicht dessen, der davorsitzt, ist „gerechnet, aber nicht auffindbar" dasselbe wie „nicht gerechnet" (derselbe Befund wie bei den Agentenlaeufen vor `/agent/laeufe`). Jetzt `<projekt>/parameterstudien/<zeit>_<parameter>/` — bzw. `~/cae_projekte/_paramstudy/` ohne gebundenes Projekt — mit `studie.json` (schlank, ohne base64), `studie.csv`, `verlauf.png`, den Feldbildern, `anim.mp4` **und dem Payload**, aus dem sie gerechnet wurde (ohne die Geometrie ist eine Kurve eine Behauptung). `csv_text` ist die EINE CSV-Quelle fuer Download und Ablage — sie stand vorher ein zweites Mal in `server.param_study_csv`. **Jeder Punkt sagt, ob er BAUBAR ist** (seit 13.09.2026): die Studie fragt ueber `ema_optimize._baubar` das Layouttor und fuehrt `baubar` je Schritt mit — rotes Band im Diagramm, Spalte in der CSV, Hinweis mit dem ersten Grund und dem Schwellwert. Anlass war eine gemeldete Polpaar-Studie, in der `B_gap` **exakt linear mit p** stieg (0,084 bis 0,675 T, `B_gap/p` auf vier Stellen konstant) und `Kt` **exakt mit p^3** — kein Befund ueber die Maschine, sondern die Polbedeckung `magWidth/pole_pitch`, die mit p waechst, waehrend `magWidth` steht: ab p = 7 ueberlappen die Taschen benachbarter Pole (bei p = 8 um 1,25 mm, Stegbreite −1,253 mm), und ausgerechnet diese beiden Zeilen trugen Temperaturen (440 °C gegen 80 °C Magnetgrenze). **Abgebrochen wird die Kurve NICHT** — eine Studie soll zeigen, WO die Grenze liegt, und ein abgeschnittenes Ende sagt nicht warum. S. `BEFUNDE.md`. **`deutungsfalle` — das Gegenstueck zu `wirkungslos_grund`** (13.09.2026): dort steht eine Kurve still und sieht nach einem Rechenfehler aus, hier steigt sie steil und sieht nach einem Gewinn aus, der keiner ist. Gemessener Fall ist die **Polzahl**: das Modell rechnet mit einer Windung je Nut, also waechst der angezeigte `Kt` mit **p²**, ohne dass sich an der Maschine etwas aendert. Bei konstant gehaltener Polbedeckung steht `B_gap` exakt still (0,6747 T auf vier Stellen ueber p = 1…8) und Kt steigt um Faktor 64; mit `ema_asm.k_norm = pi*k_w*N_ph/p²` zurueckgerechnet ist der physikalische Kt **exakt konstant** (ungerundet 0,118232 Nm/A fuer jedes p) — das klassische Ergebnis. **Saettigung kann darin ohnehin nicht stecken:** `_analytical_Bgap` hat keinen Eisenterm, der FDM ist linear (`MU_R_IRON = 500`), und der Saettigungsdurchgang wirkt nur im Anzeigepfad. Der reale Effekt steht daneben und ist exakt aus der Flusserhaltung: Fluss je Pol ~ 1/p (8,687 → 1,448 mWb ueber p = 2…12), noetige Jochhoehe 19,3 → 3,2 mm bei 1,5 T, also 84 % weniger Rueckeneisen — nur loest das Werkzeug es nicht von selbst ein. Und die Saettigung verschwindet nicht, sie WANDERT: die Stege zwischen benachbarten Taschen werden schmaler (22,19 → 3,46 mm ueber p = 4…10) und tragen die Last; die Richtung ist im FDM sichtbar, die Hoehe **nicht belastbar** (Rotorwerte wandern ueber N = 300/420/700 um Faktor 1,18…2,76, nicht konvergiert — deshalb steht dort keine Tesla-Zahl). S. `BEFUNDE.md`. **Wellendurchmesser und Wellenbohrung als Studienparameter** (`shaftD`, `shaftBore`, seit 12.09.2026) — mit dem Merker `haelt_magnete`. Der ist nicht Kosmetik: `ema_topology._common` rechnet `r_pos = r_shaft + (r_rot - r_shaft) * magDepthRel`, die Magnetlage ist also RELATIV zwischen Welle und Rand. Waechst die Welle, wandert der Sitz zwangslaeufig mit — gemessen in `pocketMode="position"` bei JEDER Bauform, im Wandmodus noch bei Speiche, Bar und PMa-SynRM. Eine Wellenstudie zeigte damit die Summe aus zwei Aenderungen, von denen man nur eine wollte. `ema_optimize.magnete_halten` rechnet deshalb die relative Lage gegen die neue Welle zurueck, sodass der ABSOLUTE Sitzradius stehen bleibt. **Ein erster Entwurf fror die Schenkel statt dessen als `customLegs` ein — das war falsch** und faellt nur auf, wenn man nachmisst: fuer `custom` kennt `estimate_saliency` kein Salienzband und `_analytical_Bgap` summiert anders, am selben Ausgangspunkt kamen bei U 0,711 statt 0,630 T heraus und bei der Speiche 0,469 statt 0,551 T; die Studie haette den Modellwechsel gezeigt statt der Welle. **Wo es nicht geht, wird es gesagt:** bei Speiche und Bar spannt der Magnet den Ringraum zwischen Welle und Rand aus, eine dickere Welle MUSS ihn kuerzen — und wo der Sitz innerhalb der neuen Welle laege (gemessen ab shaftD 120 mm bei 58,7 mm Sitzradius), ebenso. Gemessen an der V-Form ueber 30…110 mm: Kt und B_gap stehen still, nur die Masse bewegt sich. Test: `test_paramstudy.py`. **`wirkungslos_grund` benennt flache Kurven:** in einer Testreihe ueber alle elf Parameter (15 Schritte je Studie) lieferten VIER eine vollkommen flache Kurve, und an der Kurve war nicht zu erkennen, ob die Auslegung unempfindlich ist oder das Werkzeug nichts tut — gemessen war es dreimal das Zweite: `magDist`/`magDepthRel` sind im **Wandmodus** (`pocketMode="wand"`, die Vorgabe von `--frisch`) ABGELEITETE Groessen und als Eingabe wirkungslos, `magAsym` gilt nur fuer `magShape="vasym"`; `magGap` bewegt die Tasche, aber nicht die analytischen Kennzahlen. Eine flache Kurve ohne Begruendung sieht wie ein Rechenfehler aus und ist keiner (dasselbe Argument, aus dem `ema_paarvergleich` „bewegt NICHT: alles" ausdruecklich hinschreibt). **Berichte — einzeln und ueber die Reihe.** Den Studienbericht gab es schon, aber nur ueber die ZULETZT gerechnete Studie: er las `_study_state["result"]`, lag also im Serverspeicher und war nach einem Neustart nicht mehr erzeugbar. Weil die Ablage den **Payload mitfuehrt**, braucht ein Bericht ueber eine abgelegte Studie gar keinen Serverzustand mehr — `laden()` liefert Zahlen, Diagramm, Feldbilder und Entwurf. `generate_paramstudy_report` schreibt jetzt in den **Studienordner** (`parameterstudie.pdf`, neben die Zahlen, aus denen er entstand); `liste()` meldet ihn als `bericht`. **Neu ist der REIHENbericht** (`ema_report.generate_studienreihe_report` ueber `ema_paramstudy.reihe_auswerten`/`reihe_chart`): eine einzelne Studie sagt, WIE ein Parameter wirkt — die Reihe sagt, welcher ueberhaupt zuerst anzufassen ist, und das ist die Frage, die vor der Auslegung steht (dasselbe Argument, aus dem `ema_paarvergleich` neben den Paaren die Spannweite je Achse ausgibt). Drei Dinge daran sind nicht beliebig: **(1)** Die Rangliste zeigt Spannweite UND **Faktor** — die relative Spanne ist immer definiert, saettigt aber gegen 100 %, ein Faktor 3 und ein Faktor 143 saehen darin beide nach „fast alles" aus (gemessen an der Polpaar-Achse: 99 % und x143 gegen die Baulaenge mit 66 % und x3). **(2)** Sie prueft ueber `ema_projekt.payload_marke`, ob alle Studien vom **selben Entwurf** ausgehen — Spannweiten verschiedener Maschinen gegeneinanderzustellen ist keine Rangliste, und man sieht es den Zahlen nicht an; ist es nicht so, steht es als Vorbehalt im Bericht statt in einer Fussnote. **(3)** Eine flache Kurve MIT Begruendung ist geklaert, eine ohne ist **offen** und wird als solche gemeldet — „bewegt nichts" ohne Grund ist kein Ergebnis. Der Kopf nennt die Maschine in EINER deterministischen Zeile (`_maschine_zeile`) und ausdruecklich NICHT ueber `ema_chat._machine_datasheet`: das ist ein Datenblatt fuer die AUFFORDERUNG, es beginnt mit „als verbindliche Spezifikation behandeln" und fuellt Fehlendes mit „?" — im ersten erzeugten Reihenbericht stand genau diese Anweisungszeile im Fliesstext. Prosa wie ueberall hier qualitativ und zahlenfrei, Tabellen deterministisch. Der Reihenbericht liegt eine Ebene ueber den Studien (er gehoert keiner einzelnen). Server: `/param_study/gespeichert(/<kennung>(/csv|/video|/delete|/bericht(/download)))`, `/param_study/reihe/bericht(/download)`, UI-Karte **🗂 Gespeicherte Studien** im Berechnungs-Tab. Test: `test_paramstudy.py`
...
| `ema_auftrag.py` (Verb `auftrag`, Route `/project/<id>/auftrag`) | **`AUFTRAG.md` — die ABSICHT eines Projekts, fortschreibbar.** Drei Dokumente mit drei Lebensdauern, und keines ersetzt ein anderes: `AGENTS.md` traegt die **Regeln** (eine Quelle, nie kopiert), `AGENTS.projekt.md` die **Fakten** des Laufs (bei jedem Agentenstart neu erzeugt, kann also nicht driften), und `<projekt>/AUFTRAG.md` die **Absicht** — Ziel, Randbedingungen, Entscheidungen samt Begruendung, offene Punkte, Verweise. Die dritte fehlte: `design.brief` ist EIN Feld aus der Maske, und warum eine Entscheidung so und nicht anders fiel, stand allein im Gespraech — also nirgends, sobald das Fenster zu war; der naechste Lauf faengt die Begruendungen von vorn an und geht einen schon verworfenen Weg ein zweites Mal. **Vier Entwurfsentscheidungen, jede gegen einen konkreten Fehlerfall:** (1) `anlegen` ist **idempotent wie `ema_projekt.init`** — eine vorhandene Datei wird nicht angefasst, sie kann von Hand geschrieben sein, und eine Vorlage darueber waere der eine Schreibvorgang, der Text vernichtet statt welchen hinzuzufuegen. (2) Es gibt **kein `ersetzen`**: `ergaenzen` haengt einen datierten Eintrag unter seinen Abschnitt, eine ueberholte Entscheidung wird als ueberholt vermerkt, nicht getilgt (dieselbe Haltung wie `ema_projekt.zurueck`, das die Geschichte ausdruecklich nicht umschreibt). (3) Abschnitte werden **erkannt, nicht gezaehlt** — wer von Hand umsortiert oder einen eigenen einfuegt, darf das; und der Schluessel (`offen`) trifft dieselbe Ueberschrift wie ihr Titel („Offene Punkte"), sonst entstuende ein zweiter, gleichnamiger Abschnitt und die Haelfte der offenen Punkte staende woanders (gemessen beim Bauen). (4) Ein unbekannter Abschnitt wird **angelegt statt der Eintrag verworfen**. Zwei Fallen im Kleinen: das Kopfmuster darf `\s*$` **nicht** benutzen (mit `re.M` frisst es die Leerzeilen nach der Ueberschrift, und dann steht bei jedem Anhaengen eine Leerzeile mehr da), und ein Platzhalter der Vorlage ist **kein Inhalt** — `ema_steckbrief._auftragsdatei` und die Maske filtern `_noch keine_` heraus, sonst liest man die Vorlage als Aussage. Angelegt wird er in **`ema_pipeline.create_project_dir`** (der einen Stelle, damit `/project/new`, `clone`, `import`, CAD-Vorschau und Pipelinelauf ihn bekommen; `/project/new` reicht Beschreibung und Schlagworte schon dorthin durch, statt die Vorlage hinterher zu flicken), ein **Klon erbt ihn** (er entsteht ja, weil dieselbe Aufgabe weiterverfolgt wird). Gelesen von `ema_agent.projektakte_schreiben` (**vor** dem Steckbrief — die Absicht vor den Fakten), `ema_steckbrief` (`auftragsdatei`, in `als_text` UND `als_markdown`) und der Maske. Die Route gibt die Abschnitte als **Liste** und nicht als Dict zurueck: Flask sortiert Dict-Schluessel alphabetisch, und dann stand „Ziel" in der Maske ganz unten, obwohl es in der Datei oben steht. Test: `test_auftrag.py` |
| `ema_ablage.py` (Verb `papierkorb`, Route `/project/<id>/papierkorb`) | **Der Papierkorb — die EINE Loeschstelle.** Geloescht wurde an rund einem Dutzend Stellen, jede fuer sich, jede endgueltig; gefragt hat **nur die Oberflaeche**, die Routen und Modulfunktionen fuehrten aus, was ihnen gesagt wurde (s. `BEFUNDE.md`). Zwei Regeln, und beide sitzen HIER statt in der Maske: **ohne `bestaetigt=True` passiert nichts** — der Aufruf gibt dann zurueck, WAS entsorgt wuerde und wie gross es ist, ist also zugleich die Rueckfrage (eine Zusicherung in einer Maske ist keine, sobald es einen zweiten Weg zur Funktion gibt, und hier gibt es immer einen: CLI, Route, Agentenkopf); und **entsorgt heisst verschoben** — nach `<projekt>/.papierkorb/<marke>/` (ein ganzes Projekt nach `~/cae_projekte/_papierkorb/`, eigene Funktion `projekt_entsorgen`, weil das die folgenreichste Loeschung im Werkzeug ist und nicht dieselbe Signatur tragen soll wie das Wegwerfen eines Diagramms), mit Journal, Zeile in `evolution` und `wiederherstellen`. Die Grenze wird am **aufgeloesten** Pfad geprueft (`realpath`), nicht an der Zeichenkette — `..` und Symlinks lassen sich sonst verstecken, dasselbe Argument wie in `ema_bericht._rel_ok`; die Projekt- bzw. Bestandswurzel selbst ist unantastbar. `wiederherstellen` **ueberschreibt nichts**: steht am Herkunftsort wieder etwas, landet es daneben und sagt es — der Rueckweg darf nicht selbst zur Loeschung werden. `endgueltig` ist die einzige Stelle, nach der etwas wirklich weg ist, und braucht ein zweites ausdrueckliches Ja. Umgehaengt: `/project/<id>/delete`, `delete_saved_run` (Oel/em3d/FluidX3D), `ema_bericht.loeschen` **und das Kappen der Fassungen**, `ema_paramstudy.loeschen`, `ema_ansichten.loeschen` (samt der `.json` mit den Einstellungen — ein Bild ohne seine Skala ist kein Messwert, auch im Korb nicht), das stille Loeschen des jeweils anderen `bericht*.pdf`. **Bewusst NICHT umgehaengt:** Frame-Ordner, `blendcache_oil`, Elmer-Netz, `eigener_satz`-Loeserdateien — Zwischenstaende, die bei jedem Lauf neu entstehen und den Platz davonlaufen liessen; sie bekommen statt dessen ueber `geraeumt()` eine Zeile in die Zeitleiste, still bleibt nichts. Die Marke zaehlt bei Kollision hoch wie `ema_projekt.knoten_setzen` (zwei Entsorgungen in derselben Millisekunde sind der Normalfall — schon das Kappen der Fassungen entsorgt mehrere in EINER Schleife —, und die Marke ist der Griff, mit dem zurueckgeholt wird), sortiert wird ueber `_marke_key` nach (Zeit, Nummer). Test: `test_ablage.py` |
| `ema_projekt.py` (Projektakte) | **Eine KI-lesbare Quelle der Wahrheit pro Projekt** (`<projekt>/project.json`, `schema_version:1`). Wird in `create_project_dir` **zuerst** angelegt (`init`, neue kwargs `origin`/`parent`) und **laufend** fortgeschrieben: `run_pipeline` setzt zu Beginn `status="rechnet"` und ruft am Save-Block (`ema_pipeline.py:~2275`, direkt nach `ema_training.upsert`) `record_run(...)` → hängt eine **Evolutionsstufe** an (`{ts,action,changed_inputs,key_metrics,note,ref}`, Eingabe-Diff via flachem geom.*-Vergleich, Kennzahlen via `ema_training.build_metrics`) und aktualisiert `datasheet`/`metrics`/`assets`/`design`/`inputs.payload`/`status`. Action = `analyse`/`recompute:<stages>`/`design_ai`. Weitere Schreibpunkte: Rating (`status=bewertet`+Stufe), Report (`status=berichtet`+`assets.report`). **Referenzen statt base64** (Charts als rel. Pfade, gespiegelt aus `ema_training.IMAGE_PAIRS`). **Backward-Compat:** `load_or_synthesize(dir)` rekonstruiert die Akte für Altprojekte aus `meta.json`+`results.json` (kein Pflicht-Migrationslauf), optional Lazy-Write-Back. Jeder Schreibvorgang **soft-fail** (atomar `os.replace`), `load` nutzt `setdefault` je Schlüssel. Felder: `status`(neu/rechnet/gerechnet/bewertet/berichtet/verworfen)·`tags`·`notes`(Entscheidungs-Log)·`lineage{parent,origin}`·`design`·`datasheet`·`inputs.payload`·`metrics`·`assets`·`evolution[]`·`links[]`(persistente Vergleichs-Verknüpfungen, einweg+selbstheilend via `resolved_links`)·`rag.docs[]`·`attachments[]`. Konsumenten lesen dieselbe Quelle: `ema_report.build_context` zieht `evolution`/`links`/`notes` (deterministischer Abschnitt **„Projektverlauf & Verknüpfungen"** via `_evolution_links_md`, nach der Prosa angehängt), `ema_chat` erdet auf `notes` + (bei Altprojekten) das synthetisierte Payload. Server: `/project/<id>/manifest`·`/meta`(Status/Tags/Notizen)·`/links`(GET/POST)·`/links/remove`·`/clone`(ganzes Verzeichnis klonen, `lineage.parent`, RAG mitkopieren — KEINE schweren Ergebnisse)·`/bundle`(Export `.emaproj`-Zip)·`/import_bundle`(Zip-slip-sicher → neues Projekt, `origin=import`)·`/rag`+`/rag/add`+`/rag/<doc>/delete`(**Pro-Projekt-RAG** unter `<projekt>/rag/index.json`)·`/attachments`(Anhänge → automatisch in die Projekt-RAG). UI: Galerie-Badges (Status/Abstammung/⚖Links/⟳Stufen) + **📁 Klonen**/**⬇ Bundle**/**📦 Import**, Ergebnis-Tab-Panel **🗂 Projektakte** (Status/Tags/Notizen + Evolutions-Timeline + Verknüpfungen + Projekt-RAG/Anhänge). **Lauf-Archiv (`lauf_archivieren`/`laeufe_liste`, seit 12.09.2026):** `knoten/<marke>.json` hielt den PAYLOAD jedes Laufs — also die Frage; die ANTWORT wurde bei jedem Lauf ueberschrieben, weil `results.json`, `meta.json` und die `charts/*.png` immer nur den letzten Stand tragen. Wer zwei Laeufe vergleichen wollte, hatte die Kennwerte (aus `evolution`) und die Eingaben (aus `knoten/`), aber kein einziges Bild und keine Zwischengroesse des aelteren — und das faellt erst auf, wenn man es braucht. `record_run` legt jetzt zusaetzlich `laeufe/<marke>/` an (results + meta + alle Diagramme + `lauf.json` mit dem Werkzeugstand), **unter derselben Marke wie der Knoten**, damit Frage und Antwort eines Standes zusammenfindbar bleiben. Nicht kopiert werden Animations-Frames (hunderte PNG, das Video ist ohnehin gebaut), VTU (hunderte MB) und CAD (liegt einmal vor). `verlauf_markdown` rendert die EINE Zeitleiste fuer Menschen — es kommt ausdruecklich **kein zweites Journal** dazu, `evolution` bekommt nur die fehlenden Ereignisarten (`auftrag`, `entsorgt`, `geraeumt`, `zurueckgeholt`). Route `/project/<id>/laeufe`. **Aehnliche Projekte (`aehnlich`, Verb + Route `/project/<id>/aehnlich`):** verknuepfen ging schon immer (`add_link`), aber die Kennung musste man WISSEN — bei 75 abgelegten Projekten verknuepft man dann, woran man sich erinnert, und das ist selten das Aehnlichste. Verglichen wird ueber **dimensionslose** Merkmale (`ema_bilddaten.merkmale` best-effort, dazu Laenge/Bohrung und relativer Spalt, mit einem aus dem Payload gerechneten Rueckfall — das Modul braucht das Layouttor und kann an einer Altgeometrie scheitern, und dann waere ein Projekt unsichtbar statt ungenau): eine 75-mm- und eine 300-mm-Maschine koennen dieselbe Auslegung sein, und wer ueber Absolutmasse vergleicht, findet nur, was zufaellig gleich gross ist (im Test gegen eine 2,5-fach skalierte Maschine nachgemessen). Kategorien (Maschinenart, Anordnung, Kuehlung, Werkstoffe) und Zahlen zaehlen je zur Haelfte; jeder Massstab in `_MASSSTAB` sagt, ab wann ein Unterschied als „ganz anders" gilt — ohne den waere ein Polzahlsprung von 2 auf 12 dasselbe wie ein Nabenanteil von 0,30 auf 0,40. **Die Begruendung gehoert zum Treffer** (worin gleich, worin anders): „87 % aehnlich" sagt nichts darueber, ob es dieselbe Maschinenart mit anderer Kuehlung ist oder umgekehrt. Eine **Dublette** (gleiche `payload_marke`) wird als solche benannt und steht oben — das ist die Auskunft „das hast du schon gerechnet", nicht „das ist aehnlich". Ablagen mit fuehrendem `_` und das eigene Projekt bleiben draussen. Gemessen ueber den ganzen Bestand: 75 Projekte in 0,7 s, Aehnlichkeit 0,28 bis 1,00. Test: `smoke_test.py` `[projektakte]`-Block (init/record/diff/links/synthesize/lineage + `ema_rag.store_dir`-Isolation, ohne Ollama/FreeCAD) |
| Bruecke Agentenkopf ↔ Geometrie-Reiter (`ema_projekt.stand`/`payload_marke`, `GET /project/<id>/stand`, Leiste in `panel-geo`) | **Zwei Leute an einem Werkstueck, von denen einer nicht hinsah.** Agent und Formular arbeiten laengst am selben Projekt — der Agent ueber `cae_cli.py --from-project`, das Formular ueber `/project/<id>/template`. Was fehlte: dass die eine Seite MERKT, wenn die andere etwas geaendert hat. Der Agent schreibt `meta.json`, und der Reiter erfuhr davon erst, wenn jemand von Hand „Als Vorlage verwenden" drueckte. Erkannt wird die Aenderung an der **Payload-Marke** und nicht an der Dateizeit: `meta.json` wird auch neu geschrieben, wenn nur eine Notiz dazukam, und dann ueberschriebe die Bruecke ein Formular, in dem sich nichts geaendert hat. Die Marke geht ueber den GANZEN Payload (kanonisches JSON, nur `cycle_csv` heraus) und **nicht** ueber `_flatten_payload` — das ist fuer die ANZEIGE eines Unterschieds gebaut und steigt nur in `geom` ab, womit ausgerechnet `vehicle.getriebe`, `vehicle.gear_ratio` und `customLegs` herausfielen. **Und sie kommt aus `meta.json`, nicht aus der Akte** (gemessen, beinahe uebersehen): `/agent/vorgabe` und `getriebe --uebernehmen` schreiben NUR dorthin, `inputs.payload` fuehrt allein `record_run` fort — aus der Akte gelesen waere die Bruecke fuer genau die Uebergaben blind, fuer die es sie gibt. **„Automatisch" gilt nur, solange das Formular UNBERUEHRT ist** (`_brkBeruehrt`, ein `input`/`change`-Lauscher auf `panel-geo`): wer gerade selbst tippt, bekommt keinen fremden Wert unter der Hand, dann wird gefragt. Das ist der einzige Punkt, an dem „automatisch" gefaehrlich waere. Die Leiste sagt auch, WELCHER Kopf am Projekt haengt (`kopf.zustand()` je `KOEPFE`) — „etwas hat sich geaendert" ohne das Wer ist die halbe Auskunft. Die Gegenrichtung ist der Knopf **⤒ An den Agenten** auf `/agent/vorgabe`, das es bisher nur im Designer gab. Gefragt wird nur, solange der Reiter offen ist. Test: `test_bruecke.py` |
| Abzweige und der Weg zurueck (`ema_projekt.knoten_setzen`/`knoten_liste`/`zurueck`, `GET|POST /project/<id>/knoten`, `POST /project/<id>/zurueck`) | **Die Evolution war eine Einbahnstrasse.** Jede Stufe haelt nur `changed_inputs` — die NEUEN Werte der geaenderten Schluessel, **gedeckelt auf 60** —, und den vollen Payload gibt es genau einmal (`inputs.payload`, immer den letzten). Der Stand von Stufe 7 liess sich daraus nicht wiederherstellen: die alten Werte stehen nirgends, und was ueber den Deckel hinausging, ist ohnehin fort. Eine Kette gedeckelter Diffs rueckwaerts zu rechnen waere das Naheliegende und das Falsche — sie liefe still daneben, sobald ein Lauf mehr als 60 Schluessel bewegt. Also ein **Schnappschuss je Knoten**, als eigene Datei unter `<projekt>/knoten/<marke>.json`: `project.json` wird bei jedem `stand()` gelesen (die Bruecke fragt im Sekundentakt), fuenfzig eingebettete Payloads machten daraus eine Megabyte-Datei. **Jeder gerechnete Lauf legt automatisch einen an** (`record_run`) — sonst waere ausgerechnet der Stand, dessen Kennwerte man vergleicht, nicht wiederherstellbar; `automatisch` unterscheidet die von denen, die ein Mensch ausdruecklich gesetzt hat („hier zweige ich ab"), denn nur die sind eine Aussage. **`zurueck` schreibt die Geschichte NICHT um:** der Rueckweg ist selbst eine Stufe, die Stufen dazwischen bleiben stehen — dass ein Zweig probiert wurde und sich nicht bewaehrt hat, ist selbst eine Auskunft. Der VERLASSENE Stand wird vorher automatisch gesichert, sonst waere der Rueckweg der einzige Schritt, den man nicht rueckgaengig machen kann. Zurueckgesetzt wird **`meta.json` UND die Akte** — die Akte allein braechte Formular, Verb und Steckbrief nicht mit. Bedienung: „⑂ Abzweig" und „↩ Zurueck zu …" in der Brueckenleiste. Test: `test_bruecke.py` | **Die Marke traegt Millisekunden und seit dem 12.09.2026 auch einen Kollisionszusatz** (`-2`, `-3`, …): ein gerechneter Lauf legt automatisch einen Rueckkehrpunkt an, und wer unmittelbar danach von Hand abzweigt, traf dieselbe Millisekunde und **ueberschrieb ihn** — `zurueck` fuehrte danach an den falschen Stand, der eigentliche Punkt war weg. Gemessen war `test_bruecke` dadurch in rund jedem vierten Lauf rot, ohne dass sich am Code etwas geaendert haette. Dazu gehoert zwingend die Markenpruefung in `knoten_holen`: sie liess nur `[0-9_]` zu, der Zusatz waere daran gescheitert und der Knoten waere geschrieben und trotzdem unerreichbar gewesen. Dieselbe Absicherung wie in `ema_steckbrief.ablegen` und `ema_paramstudy.studie_anlegen`.
...
| `ema_fluidx3d.py` / `fluidx3d_runner.py` | **Spritzoel als Lattice-Boltzmann auf der GPU** (FluidX3D, freie Oberflaeche mit PLIC + Oberflaechenspannung) — s. Architektur-Abschnitt. `run_fluidx3d` (STL-Ausschnitt → Fensterlage → C++-Fall erzeugen → uebersetzen → Lauf → Schnittbilder/Video/Diagramme → `results["fluidx3d"]` + Varianten-Store), `aufloesung`/`vollmaschine_kosten` (was geht und was nicht — als Zahl, nicht als Behauptung), `fenster_bestimmen`/`stl_ins_fenster` (Fenster aus der Geometrie, Ausschnitt auf die +x-Achse gedreht), `setup_code` (der Fall als Quelltext, mit `@@NAME@@` gesetzt statt `str.format` — C++ ist voller geschweifter Klammern). `fluidx3d_runner` = Arbeitskopie anlegen, `defines.hpp` setzen, uebersetzen (SHA1-Cache), laufen, abbrechen. **Dritter Loeser auf demselben `oil`-Satz** wie 💧 und 🌊; **isotherm** — der HTC bleibt bei `ema_cfd`. Server `/fluidx3d`(+`/status`,`/abort`,`/kosten`) + `/project/<id>/fluidx3d/saved…`, Job-Typ `fluidx3d`, UI-Tab **🌀 FluidX3D (LBM)**. Test: `test_fluidx3d.py` |
| `ema_ansichten.py` (Routen `/project/<id>/ansicht…`) | **Festgehaltene Ansichten aus dem Browser-Betrachter.** Der 3-D-Betrachter zeichnet mit vtk.js; was man dort einstellt — Blickrichtung, Schnittebene, Farbskala, Feldlinien, Lastfall — ist oft **genau die Ansicht, die in den Bericht gehoert**, und sie existierte nur auf dem Schirm: die Berichtsbilder kommen aus matplotlib und `render_field_3d` mit fester Kamera. Der Knopf **📷 Festhalten** in der Viewer-Leiste greift das WebGL-Bild ueber vtk.js' `captureNextImage()` ab (erst anmelden, dann rendern — ein `canvas.toDataURL()` liefert ohne `preserveDrawingBuffer` ein leeres Bild) und legt es unter `<projekt>/ansichten/` ab. **Die Einstellungen kommen mit** (`einstellungen`: Skala, Lastfall, Schnitt): ein |B|-Bild ohne seine Skala ist kein Messwert, sondern ein Muster, und im Bericht stuende es sonst ohne Massstab neben gerechneten Zahlen. Bewusst getrennt von `charts/` — dort liegt, was ein Lauf erzeugt und beim naechsten ueberschreibt; eine festgehaltene Ansicht ist eine Entscheidung des Menschen. Test: `test_bericht.py` |
| `ema_report.generate_em3d_report` (Route `/em3d/bericht`, Karte im 🧲-Reiter) | **Elmer-Auswertung mit Schwerpunkt auf dem LOESER.** Der Projektbericht hat laengst einen 3-D-Abschnitt (`_ensure_em3d_section`), aber der ordnet den Lauf in eine Maschinenauslegung ein. Hier geht es um den LAUF: womit gerechnet wurde — **aus der `case.sif` GELESEN** (`sif_loeser`), nicht aus `ema_em3d.write_sif` abgeschrieben, denn die Einstellungen dort haengen an Netzart und Anregung (Piola nur bei Hex, kein Tree-Gauge dazu, MUMPS nur auf Simplizes) und das Modul beschriebe die Moeglichkeiten statt den Lauf. Dazu die Netztabelle (Knoten, Taschen zugeordnet/gewollt, Zonen-Zellgroessen, Klebespalt, Staffelung), der 2-D-Vergleich samt Orientierungspruefung und die **Warnungen des Laufs im Wortlaut** — sie tragen die Geltungsgrenzen (etwa dass im Lastfall nur die Mittelebene belastbar ist). Die festgehaltenen Ansichten werden angehaengt. Zwei Fallen, beide gemessen: der Solverblock endet bei `End` (ohne das landete die `Equation = 1` eines Body-Blocks im letzten Solver), und Anzahlen werden ohne Nachkommastellen gesetzt (`164 912.000 Knoten` liest sich wie eine Messgroesse). Test: `test_bericht.py` |
| `ema_bericht.py` (Reiter 📝 Bericht, Routen `/project/<id>/berichte…`) | **Ein Bericht, den der Mensch selbst schreibt.** Alle uebrigen Berichte hier sind erzeugt; es gab nichts, worin man selbst etwas festhaelt — eine Einordnung, eine Entscheidung, ein Vergleich zweier Laeufe, ein Video neben einem Feldbild. Ein Bericht ist eine Liste **Bloecke** (`text` als Markdown, `bild`, `video`, `tabelle`, `umbruch`) in der Reihenfolge, in der sie stehen sollen. **Verwiesen, nicht kopiert:** ein Block nennt den projektrelativen Pfad, die Datei bleibt, wo sie entstand — ein Bericht, der Bilder kopiert, zeigt beim naechsten Lauf den alten Stand, ohne dass es jemand merkt (dasselbe Argument wie bei `ema_agent.sichern`). `_rel_ok` prueft am **aufgeloesten** Pfad, nicht an der Zeichenkette — `..` laesst sich sonst verstecken. **Video im PDF gibt es nicht, und das wird nicht verschwiegen:** ein Videoblock wird zweimal ausgegeben — im PDF als Standbild (ffmpeg, aus der **Mitte** des Films: der erste Frame eines Simulationsvideos ist der Zustand VOR dem Ereignis) samt Dateiname, und in einer **HTML-Fassung** als echtes `<video>`. Beide aus derselben Blockliste; die HTML traegt `<base href="../../">`, weil sie zwei Ebenen unter dem Projekt liegt und die Blockpfade projektrelativ sind. `bestand()` sammelt, was sich einfuegen laesst (charts, cad_images, ansichten, Parameterstudien, alle `frames_*/anim.mp4` und die Varianten-Stores) — Einzelframes einer Animation ausdruecklich NICHT, sonst stuenden hunderte in der Auswahl. **Vorschau, Fassungen, Varianten, Bausteine** (12.09.2026). Die **Vorschau** wird SERVERseitig mit `als_html` gebaut — derselben Funktion, die auch die ausgelieferte HTML-Fassung schreibt; eine zweite Darstellung im Browser waere genau die Stelle, an der Vorschau und Ergebnis auseinanderlaufen (im `iframe` zeigt `base href` auf `/project/<id>/datei/` statt auf `../../`). **Fassungen:** jedes Speichern legt eine ab, unveraendert aber keine neue (`_inhalt_marke`); `zurueck` holt eine zurueck und schreibt die Geschichte NICHT um — der verlassene Stand bleibt auffindbar, die Rueckkehr ist selbst eine Fassung. **Varianten:** `abzweigen` legt einen neuen Bericht an, der seine Herkunft (`eltern`) mitfuehrt; `baum()` macht daraus die Ansicht, und die Herkunft ueberlebt ein Speichern, auch wenn die Oberflaeche sie nicht mitschickt. **Die Marken werden nach (Zeit, Nummer) sortiert, nicht als Zeichenkette** — zwei Fassungen derselben Sekunde heissen `…038` und `…038-2`, und im Dateinamen sortiert der Bindestrich UNTER den Punkt; derselbe Fehler steht schon in `ema_getriebe.rechnungen()`, hier fiel er an der Dubletten-Erkennung auf. **Bausteine** (`BAUSTEINE`, Blockart `baustein`) setzen GERECHNETE Abschnitte ein — Steckbrief, Getriebe, Elmer, Studienreihe, Sicherheit —, und zwar beim Rendern FRISCH aus der Ablage: eine eingefrorene Zahl, die von der Ablage abweicht, sieht richtig aus und ist es nicht. Damit ist dieser Bericht der **Gesamtbericht ueber die ganze Maschine**, Getriebe eingeschlossen. Was es nicht gibt, sagt das auch — das Fehlen ist die Auskunft. **`alle_berichte()`** listet jedes PDF/HTML des Projekts, egal welche Stufe es erzeugt hat: sie liegen alle im Projektordner, nur an fuenf Stellen, weil jede Stufe ihren Bericht neben ihre Zahlen legt (das ist richtig so — ein Bericht neben seinen Zahlen ist zuzuordnen); was fehlte, war die eine Liste. Test: `test_bericht.py` |
...
- `BEFUNDE.md` — **what is wrong with the tool itself**: observed, measured, with the
  line reference. Dated entries, newest first; a fixed point stays and gets its status.
  This is where an agent writes a defect instead of silently repairing it mid-run — a
  silent repair means the figures before and after come from two different tools and
  nothing about them says so (see `ema_werkzeugstand.py`).
