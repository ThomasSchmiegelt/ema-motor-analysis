...
| `ema_getriebe_cad.py` | **Die Zeichnung — mit FCGear, sonst ehrliche Ersatzkoerper.** `freecad.gears` laeuft kopflos, und das ist gemessen und nicht geglaubt: `InvoluteGear` z = 20, m = 3, h = 40 -> 110.236 mm^3 in **0,12 s**, `InternalInvoluteGear` z = 72 -> 35.748 mm^3 in 0,17 s (FreeCAD 1.1.1 unter `FreeCADCmd`). **Zwei Fallen, beide gemessen:** das Addon gehoert nach `~/.local/share/FreeCAD/`**`v1-1`**`/Mod` — dieser Bau meldet `v1-1/` als Nutzerzweig, im Elternverzeichnis wird es schlicht nicht gefunden (`ModuleNotFoundError`, ohne Hinweis darauf, dass es danebenliegt); und die Eigenschaft heisst **`num_teeth`**, nicht `teeth` — `g.teeth = 20` scheitert mit `AttributeError` an einem Objekt, das gerade eben erzeugt wurde. **`Part.makeCompound`, nicht `fuse`:** der Verschnitt zweier Raeder, deren Kopfkreise sich beruehren, gab gemessen **0,00 mm^3** zurueck, waehrend die Einzelkoerper 16.677 und 209.396 mm^3 hatten — still gescheitert; ein Getriebe ist ohnehin eine Baugruppe und kein verschweisster Klumpen. **Der Rueckfall zeichnet KEINE Verzahnung.** Hier stand zuerst ein eigener Evolventen-Erzeuger; an derselben Stufe (z 18/63, m = 1,5, b = 30) lieferte er 12.092.260 statt 226.073 mm^3 — **Faktor 60**, und dem Bild sieht man das nicht an. Ein Rad, das wie ein Zahnrad aussieht und die falsche Groesse hat, ist schlimmer als keines: wer eine Fertigungszeichnung daraus zieht, merkt es zu spaet. Ohne FCGear stehen daher Zylinder auf dem Teilkreis (`zeichner: "ersatz"` + `vorbehalt`) — unverkennbar keine Zaehne, und genau das, was die Einbaufrage braucht: passt es hinein und wo sitzt es. Kegelrad und Schnecke haben **keinen** Rueckfall, dann steht das da. Heraus kommen `getriebe.FCStd` + `getriebe.step` im Projekt (gemessen: zweistufig 4 Koerper / 3.429.312 mm^3, Planetensatz in der Welle 5 Koerper / 100.257 mm^3) und — **ohne FreeCAD, aus den Zahlen** — ein Querschnittsbild nach `charts/getriebe.png`, das die verfuegbare Bohrung samt der bindenden Grenze eintraegt und ueber den vorhandenen Bilderpfad von selbst in der rechten Spalte der Agentenseiten erscheint. CLI: `getriebe --cad` (das Bild kommt immer, `--ohne-cad` laesst auch das weg). Test: `test_getriebe.py` [12] — er prueft die drei Fallen **am Quelltext und ohne FreeCAD**, weil jede von ihnen still scheitert |
