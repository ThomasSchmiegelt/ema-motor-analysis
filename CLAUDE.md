...
| `cae_orchestrator/cae_cli.py` | the CLI itself, sixteen verbs: nine over HTTP on `:5000` (`status/health/geom/run/wait/results/projects/raw/routes`), seven local (`rotor-check`, `screen`, `struktur`, `topopt`, `db`, `lernen`, `recherche`) |
